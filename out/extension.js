...
`).map(a=>a.replace("*","").trim()).filter(a=>a);if(n.length===0){Ee.window.showInformationMessage(`Ricwiz: No branches found matching "${e}"`);return}let r=n.map(a=>({label:`$(git-branch) ${a}`,branchName:a})),p=await Ee.window.showQuickPick(r,{placeHolder:`Select a branch for ${e}`});p&&await ze(p.branchName)}catch{Ee.window.showErrorMessage("Ricwiz: Failed to search branches")}}var Ee,So=k(()=>{"use strict";Ee=v(require("vscode"));$();qe()});async function Bo(){let t=ye.window.activeTextEditor;if(!t)return ye.window.showErrorMessage("Ricwiz: Please open a file in the editor to check blame."),null;let e=t.document.fileName,i=Po.basename(e),n=y();if(!n)return ye.window.showErrorMessage("Ricwiz: Workspace is not a git repository."),null;let r=[];try{let{stdout:o}=await f(`git log -5 --pretty=format:"%an|%ar|%s|%h" -- "${e}"`,{cwd:n}),s=o.trim().split(`
