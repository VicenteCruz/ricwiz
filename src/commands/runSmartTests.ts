...
export async function runSmartTests(): Promise<void> {
    ...
    await vscode.window.withProgress({
        ...
        try {
            const { stdout } = await exec(`git diff --name-status origin/${sourceBranch}...HEAD`, { cwd });
            
            const lines = stdout.split('\n').map(l => l.trim()).filter(l => l.length > 0);
            
            const testClassesToRun = new Set<string>();
            const modifiedClasses = new Set<string>();

            for (const line of lines) {
                const parts = line.split(/\s+/);
                if (parts[0].startsWith('D')) continue; // skip deleted
                const file = parts[1];
                if (!file) continue;
            ...
            const items: vscode.QuickPickItem[] = Array.from(testClassesToRun).map(t => ({
                label: `$(beaker) ${t}`,
