...
`).map(a=>a.trimRight()).filter(a=>a.length>2).filter(a=>{let m=a.substring(0,2);return["UU","AA","UD","DU","AU","UA","DD"].includes(m)}).map(a=>{let m=a.substring(0,2);return{file:a.substring(3).trim(),state:n(m)}})}catch{return[]}},p=async()=>{if(r)return;let i=await d(),n=await u(),{webviewProvider:a}=(_e(),O(Ve));a&&a.setConflictState({isConflict:!0,sourceStr:e,targetStr:o,deletionsCount:i.length,files:n})},l=ae.commands.registerCommand("ricwiz.conflictAction",async i=>{if(i==="abortDeploy")c=!0;else if(i==="resolveDeletions"){try{let a=(await d()).map(g=>({label:g})),m=await ae.window.showQuickPick(a,{canPickMany:!0,placeHolder:"Select conflicted files to DELETE",title:"Ricwiz: Delete Conflicted Files"});if(m&&m.length>0){for(let g of m)try{await h(`git rm --force "${g.label}"`,{cwd:t})}catch{}ae.window.showInformationMessage(`Ricwiz: Deleted ${m.length} conflicted file(s).`)}}catch(n){ae.window.showErrorMessage(`Ricwiz: Error. (${n.message})`)}p()}else if(i==="commitAndContinue")try{let a=(await d()).filter(g=>At.existsSync(Lt.join(t,g)));if(a.length>0&&await ae.window.showWarningMessage(`Wait! There are ${a.length} file(s) with deletion conflicts that are still on your disk.

If you commit now, you will KEEP them in the project.

Are you sure you want to KEEP them?`,{modal:!0},"Yes, KEEP them","No, let me DELETE them")!=="Yes, KEEP them"){p();return}let m=!1;try{let{stdout:g}=await h('git grep -E "^<<<<<<< "',{cwd:t});g.trim().length>0&&(m=!0)}catch{}if(m){ae.window.showErrorMessage("Ricwiz: You still have unresolved conflict markers (<<<<<<<) in your files. Please resolve them first!"),p();return}await h("git add .",{cwd:t}),await h("git commit --no-edit",{cwd:t})}catch(n){ae.window.showErrorMessage(`Ricwiz: Could not commit automatically. (${n.message})`),p()}});for(p();;){if(c){r=!0,l.dispose(),(_e(),O(Ve)).webviewProvider?.setConflictState(null);try{await h("git merge --abort",{cwd:t})}catch{}return!1}try{let{stdout:i}=await h("git status --porcelain",{cwd:t});if(i.trim().length===0)return r=!0,l.dispose(),(_e(),O(Ve)).webviewProvider?.setConflictState(null),ae.window.showInformationMessage("Ricwiz: Changes committed!"),!0}catch{}await new Promise(i=>setTimeout(i,2e3))}}var ae,At,Lt,Qe=z(()=>{"use strict";ae=b(require("vscode")),At=b(require("fs")),Lt=b(require("path"));S()});var rt={};De(rt,{fetchMergeRequestStatus:()=>Ke,hasGitlabToken:()=>Ze,ricwizLogger:()=>j});async function Ze(){let t=await st();return!!(t&&t.trim())}async function si(t,e){let o=Ye.workspace.getConfiguration("ricwiz"),s=(await st())?.trim();if(!s)throw new Error("No GitLab token");let r=e?e.getConfig("gitlabUrlOverride",""):o.get("gitlabUrlOverride",""),c=[];if(r&&r.trim()!=="")c.push(r.trim());else try{let{stdout:u}=await h("git remote",{cwd:t}),p=u.split(`
`).map(i=>i.trim()).filter(i=>i),l=[];e&&e.upstreamRemote&&p.includes(e.upstreamRemote)&&l.push(e.upstreamRemote),e&&e.originRemote&&e.originRemote!==e.upstreamRemote&&p.includes(e.originRemote)&&l.push(e.originRemote),p.includes("upstream")&&!l.includes("upstream")&&l.push("upstream"),p.includes("origin")&&!l.includes("origin")&&l.push("origin"),l.length===0&&p.length>0&&l.push(...p);for(let i of l)try{let{stdout:n}=await h(`git remote get-url ${i}`,{cwd:t}),a=n.trim();a.endsWith(".git")&&(a=a.slice(0,-4)),a.startsWith("git@")&&(a=a.replace("git@","").replace(":","/"),a=`https://${a}`),c.push(a)}catch(n){j.appendLine(`[GitLab API] Error getting remote URL for ${i}: ${n.message}`)}}catch(u){j.appendLine(`[GitLab API] Error getting remotes: ${u.message}`)}if(c.length===0)throw j.appendLine("[GitLab API] No candidate URLs found in getGitlabTargets!"),new Error("Could not get any remote origin URL.");return c.map(u=>{let p=new URL(u),l=`${p.protocol}//${p.host}`,i=p.pathname;i.startsWith("/")&&(i=i.substring(1)),i.endsWith("/")&&(i=i.slice(0,-1)),i.endsWith(".git")&&(i=i.slice(0,-4));let n=encodeURIComponent(i);return{baseUrl:l,token:s,projectPath:n}})}async function It(t,e,o,s,r){let c=new URL(`${e}${r}`);return j.appendLine(`[GitLab API] ${s} ${c.toString()}`),new Promise((d,u)=>{let p=Ot.request(c,{method:s,timeout:5e3,headers:{"PRIVATE-TOKEN":o,Accept:"application/json"}},l=>{let i="";l.on("data",n=>i+=n),l.on("end",()=>{if(j.appendLine(`[GitLab API] Response Code: ${l.statusCode}`),l.statusCode&&l.statusCode>=400)return j.appendLine(`[GitLab API] Error Data: ${i}`),u(new Error(`GitLab API error: ${l.statusCode}`));if(!i)return d({});try{let n=JSON.parse(i);Array.isArray(n)?j.appendLine(`[GitLab API] Returned array with ${n.length} items`):n&&typeof n=="object"&&j.appendLine(`[GitLab API] Returned object with id ${n.id||n.iid||"unknown"}`),d(n)}catch(n){j.appendLine(`[GitLab API] Parse Error: ${n.message}`),u(n)}})});p.on("timeout",()=>{p.destroy(),u(new Error("GitLab request timed out"))}),p.on("error",l=>{j.appendLine(`[GitLab API] Request Failed: ${l.message}`),u(l)}),p.end()})}async function Ke(t,e,o,s){j.appendLine(`[GitLab API] fetchMergeRequestStatus called for source: ${e}, target: ${o||"any"}`);let r=`${t}:${e}:${o||"any"}`,c=Ft.get(r);if(c&&Date.now()-c.timestamp<ri)return c.data;try{let d=await si(t,s);for(let u of d)try{let p=`/api/v4/projects/${u.projectPath}/merge_requests?source_branch=${encodeURIComponent(e)}&order_by=updated_at&sort=desc`;o&&(p+=`&target_branch=${encodeURIComponent(o)}`);let l=await It(t,u.baseUrl,u.token,"GET",p);if(l&&l.length>0){let i=l[0];try{let m=await It(t,u.baseUrl,u.token,"GET",`/api/v4/projects/${u.projectPath}/merge_requests/${i.iid}`);m&&(i=m)}catch{}let n="none";if(i.head_pipeline&&i.head_pipeline.status){let m=i.head_pipeline.status;m==="success"||m==="failed"||m==="canceled"||m==="skipped"?n=m:n="running"}let a={isMerged:i.state==="merged",isOpen:i.state==="opened",pipelineStatus:n,webUrl:i.web_url};return Ft.set(r,{data:a,timestamp:Date.now()}),a}}catch(p){j.appendLine(`[GitLab API] Error inside target loop: ${p.message}`)}return null}catch(d){return j.appendLine(`[GitLab API] Failed to fetch MR status: ${d.message}`),null}}var Ot,Ye,j,Ft,ri,Xe=z(()=>{"use strict";Ot=b(require("https")),Ye=b(require("vscode"));ze();S();j=Ye.window.createOutputChannel("Ricwiz Debug");Ft=new Map,ri=30*1e3});var ke={};De(ke,{findRelatedBranches:()=>mt,getCurrentBranchMergeStatus:()=>ct,getRecentCommits:()=>dt,getRecentTickets:()=>lt,getRelatedBranchesStatus:()=>at,resolveExistingBranchName:()=>ai});function Nt(t,e){return e.find(o=>t.endsWith(`-to-${o.name}`))}async function at(t,e,o,s,r){let c=await Ze();return await Promise.all(e.map(async u=>{let p=Nt(u,s);if(c){let l=p?p.sourceBranch:void 0,i=await Ke(t,u,l,r);if(i)return{name:u,isMerged:i.isMerged,pipelineStatus:i.pipelineStatus,mrUrl:i.webUrl}}else{let{ricwizLogger:l}=(Xe(),O(rt));l.appendLine(`[GitLab API] Skipping MR check for ${u} because hasGitlabToken() is false`)}return{name:u,isMerged:!1}}))}async function ct(t,e,o,s){let r=Nt(e,o);if(!r)return!1;if(await Ze()){let c=await Ke(t,e,r.sourceBranch,s);if(c)return c.isMerged}else{let{ricwizLogger:c}=(Xe(),O(rt));c.appendLine(`[GitLab API] Skipping MR check for current branch ${e} because hasGitlabToken() is false`)}return!1}async function dt(t,e=10){try{let{stdout:o}=await h(`git log --oneline -${e} --format="%h|||%s|||%ar"`,{cwd:t});return o.split(`
...
This cannot be undone!`,{modal:!0},"Yes, delete them")!=="Yes, delete them"){N.window.showInformationMessage("Ricwiz: Deletion cancelled.");return}let i=0;for(let n of p){let a=n.label;if(a===e){let m=o.get("ticketSourceBranch","main");try{await h(`git checkout ${m}`,{cwd:t}),e=m}catch{N.window.showWarningMessage(`Ricwiz: Could not switch away from ${a}. Skipping.`);continue}}try{await h(`git branch -D ${a}`,{cwd:t}),i++}catch{N.window.showWarningMessage(`Ricwiz: Could not delete local branch ${a}.`)}}N.window.showInformationMessage(`Ricwiz: \u{1F5D1}\uFE0F Cleaned up ${i} unused local branch(es).`)})}var N,xo=z(()=>{"use strict";N=b(require("vscode"));S()});async function Me(t){let e=y();e&&await ne.window.withProgress({location:ne.ProgressLocation.Notification,title:`Ricwiz: Switching to ${t}...`,cancellable:!1},async()=>{try{let o=await F(e),s=!1;try{let{stdout:c}=await h("git status --porcelain",{cwd:e});s=c.trim().length>0}catch{}if(s&&o)try{await h(`git stash push --include-untracked -m "ricwiz-auto:${o}"`,{cwd:e}),ne.window.showInformationMessage(`Ricwiz: \u{1F4E6} Stashed changes from ${o}`)}catch{ne.window.showWarningMessage("Ricwiz: Could not stash changes. Checkout may fail if there are conflicts.")}let r=t;t.includes("/")&&(r=t.split("/").slice(1).join("/"));try{await h(`git checkout ${r}`,{cwd:e})}catch{let d="";if(t.includes("/"))d=t.split("/")[0];else{let{stdout:u}=await h("git branch -r",{cwd:e}),p=u.split(`
`).map(i=>i.trim()).filter(i=>i),l=[];for(let i of p){let n=i.split(" ")[0];n.endsWith(`/${r}`)&&l.push(n.substring(0,n.lastIndexOf("/")))}if(l.length===0){ne.window.showErrorMessage(`Ricwiz: A branch "${r}" n\xE3o existe localmente nem em nenhuma remote!`);return}else if(l.length===1)d=l[0];else{let i=await D.initialize(e);l.includes("origin")?d="origin":i&&l.includes(i.upstreamRemote)?d=i.upstreamRemote:d=l[0]}}try{await h(`git fetch ${d} ${r}`,{cwd:e}),await h(`git checkout -b ${r} --track ${d}/${r}`,{cwd:e})}catch{ne.window.showErrorMessage(`Ricwiz: Encontrou na remote ${d} mas falhou a fazer checkout.`);return}}try{let{stdout:c}=await h("git stash list",{cwd:e}),d=c.split(`
`);for(let u=0;u<d.length;u++)if(d[u].includes(`ricwiz-auto:${r}`)){let p=d[u].match(/stash@\{(\d+)\}/);p&&(await h(`git stash pop stash@{${p[1]}}`,{cwd:e}),ne.window.showInformationMessage(`Ricwiz: \u{1F4E6} Restored stashed changes on ${r}`));break}}catch{ne.window.showWarningMessage(`Ricwiz: Could not restore stashed changes on ${r}. You may need to resolve conflicts manually (check git stash list).`)}}catch{ne.window.showErrorMessage(`Ricwiz: Could not checkout branch ${t}.`)}})}var ne,et=z(()=>{"use strict";ne=b(require("vscode"));S();te()});async function ko(){let t=y();if(t)try{let{stdout:e}=await h("git branch --show-current",{cwd:t}),o=e.trim();o&&(await Fe.env.clipboard.writeText(o),Fe.window.showInformationMessage(`Ricwiz: \u{1F4CB} Copied "${o}" to clipboard`))}catch{Fe.window.showErrorMessage("Ricwiz: Could not get the current branch name.")}}var Fe,Co=z(()=>{"use strict";Fe=b(require("vscode"));S()});async function Ro(){let t=y();if(!t){Q.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=Q.workspace.getConfiguration("ricwiz"),o=e.get("ticketSourceBranch","main"),r=e.get("packageXmlCommand",'sf sgd source delta --to "HEAD" --from "origin/{baseBranch}" --output-dir "."').replace("{baseBranch}",o);await Q.window.showWarningMessage("Are you sure you want to generate the package.xml? This may overwrite existing local manifest files.",{modal:!0},"Yes, Generate")==="Yes, Generate"&&await Q.window.withProgress({location:Q.ProgressLocation.Notification,title:"Ricwiz: Generating package.xml using Salesforce CLI...",cancellable:!1},async()=>{try{await h(r,{cwd:t,maxBuffer:10*1024*1024}),Q.window.showInformationMessage("Ricwiz: Successfully generated package.xml!");let d=tt.join(t,"package","package.xml"),u=tt.join(t,"package.xml"),p=tt.join(t,"manifest","package.xml");for(let l of[d,u,p])if($o.existsSync(l)){let i=await Q.workspace.openTextDocument(l);await Q.window.showTextDocument(i);break}}catch(d){Q.window.showErrorMessage(`Ricwiz: Error running sf command - ${d.message}`)}})}var Q,tt,$o,zo=z(()=>{"use strict";Q=b(require("vscode")),tt=b(require("path")),$o=b(require("fs"));S()});async function So(){let t=y();if(!t){Y.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let o=Y.workspace.getConfiguration("ricwiz").get("deployCommand","sf project deploy start --manifest package/package.xml --post-destructive-changes destructiveChanges/destructiveChanges.xml -c -l NoTestRun -g");await Y.window.showWarningMessage("Are you sure you want to deploy to Salesforce? This action modifies your org and cannot be easily undone.",{modal:!0},"Yes, Deploy")==="Yes, Deploy"&&await Y.window.withProgress({location:Y.ProgressLocation.Notification,title:"Ricwiz: Deploying package...",cancellable:!1},async()=>{try{let{stdout:r,stderr:c}=await h(o,{cwd:t,maxBuffer:52428800}),d=Y.window.createOutputChannel("Ricwiz Deploy");d.appendLine(`Executing: ${o}`),d.appendLine(r),c&&(d.appendLine("--- STDERR ---"),d.appendLine(c)),d.show(),Y.window.showInformationMessage("Ricwiz: Successfully ran deploy command!")}catch(r){let c=Y.window.createOutputChannel("Ricwiz Deploy");c.appendLine(`Error executing: ${o}`),r.stdout&&c.appendLine(r.stdout),r.stderr&&c.appendLine(r.stderr),c.appendLine(r.message),c.show(),Y.window.showErrorMessage("Ricwiz: Error running deploy command. See output channel for details.")}})}var Y,Po=z(()=>{"use strict";Y=b(require("vscode"));S()});async function Bo(){let t=y();if(!t){Z.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let o=Z.workspace.getConfiguration("ricwiz").get("importDataCommand","sfdx force:data:tree:import --plan data/plan.json");await Z.window.showWarningMessage("Are you sure you want to import data into Salesforce? This action modifies records in your org.",{modal:!0},"Yes, Import")==="Yes, Import"&&await Z.window.withProgress({location:Z.ProgressLocation.Notification,title:"Ricwiz: Importing data...",cancellable:!1},async()=>{try{let{stdout:r,stderr:c}=await h(o,{cwd:t,maxBuffer:52428800}),d=Z.window.createOutputChannel("Ricwiz Import Data");d.appendLine(`Executing: ${o}`),d.appendLine(r),c&&(d.appendLine("--- STDERR ---"),d.appendLine(c)),d.show(),Z.window.showInformationMessage("Ricwiz: Successfully ran import data command!")}catch(r){let c=Z.window.createOutputChannel("Ricwiz Import Data");c.appendLine(`Error executing: ${o}`),r.stdout&&c.appendLine(r.stdout),r.stderr&&c.appendLine(r.stderr),c.appendLine(r.message),c.show(),Z.window.showErrorMessage("Ricwiz: Error running import data command. See output channel for details.")}})}var Z,Eo=z(()=>{"use strict";Z=b(require("vscode"));S()});async function To(){let t=y();if(!t){K.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e="";try{e=await F(t)}catch{}let s=K.workspace.getConfiguration("ricwiz").get("ticketSourceBranch","main"),r=await K.window.showInputBox({prompt:`Enter the branch name to list modified files (compared to ${s})`,value:e,placeHolder:"SFPSCA-1234"});r&&await K.window.withProgress({location:K.ProgressLocation.Notification,title:`Ricwiz: Finding files for ${r}...`,cancellable:!1},async()=>{try{let c=r.replace(/-to-[a-zA-Z0-9]+$/i,""),d=[];try{let g="";try{let{stdout:f}=await h(`git merge-base origin/${s} ${r}`,{cwd:t});g=f.trim()}catch{let{stdout:f}=await h(`git merge-base ${s} ${r}`,{cwd:t});g=f.trim()}if(g){let{stdout:f}=await h(`git diff --name-only ${g} ${r}`,{cwd:t,maxBuffer:10485760});d=f.split(`
...
`).map(f=>f.trim()).filter(f=>f.length>0)}catch{}let p=[...d,...u];if(p.length===0){K.window.showInformationMessage(`Ricwiz: No modified files found for ${r}.`);return}let l=Array.from(new Set(p)).sort(),i={};for(let g of l){let f=g.match(/default\/([^/]+)/),w=f&&f[1]?f[1].toUpperCase():"OUTROS";i[w]||(i[w]=[]),i[w].push(g)}let n=`Files modified in branch ${r}:
`,a=Object.keys(i).sort();for(let g of a)n+=`
=== ${g} ===
`,n+=i[g].join(`
`)+`
`;let m=await K.workspace.openTextDocument({content:n,language:"plaintext"});await K.window.showTextDocument(m)}catch(c){K.window.showErrorMessage(`Ricwiz: Error running git log - ${c.message}`)}})}var K,Mo=z(()=>{"use strict";K=b(require("vscode"));S()});async function Do(){let t=y();if(!t){se.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let o=se.workspace.getConfiguration("ricwiz").get("resetTrackingCommand","sf org disable tracking && sf project reset tracking --no-prompt");await se.window.withProgress({location:se.ProgressLocation.Notification,title:"Ricwiz: Resetting tracking...",cancellable:!1},async()=>{try{let{stdout:s,stderr:r}=await h(o,{cwd:t,maxBuffer:52428800}),c=se.window.createOutputChannel("Ricwiz Reset Tracking");c.appendLine(`Executing: ${o}`),c.appendLine(s),r&&(c.appendLine("--- STDERR ---"),c.appendLine(r)),c.show(),se.window.showInformationMessage("Ricwiz: Successfully reset tracking!")}catch(s){let r=se.window.createOutputChannel("Ricwiz Reset Tracking");r.appendLine(`Error executing: ${o}`),s.stdout&&r.appendLine(s.stdout),s.stderr&&r.appendLine(s.stderr),r.appendLine(s.message),r.show(),se.window.showErrorMessage("Ricwiz: Error resetting tracking. See output channel for details.")}})}var se,Ao=z(()=>{"use strict";se=b(require("vscode"));S()});async function Lo(){let t=y();if(!t){X.window.showErrorMessage("Ricwiz: Open a workspace first.");return}let e=["ApexClass","ApexTrigger","CustomObject","CustomField","LightningComponentBundle","AuraDefinitionBundle","Flow","CustomLabel","CustomMetadata","StaticResource","Profile","PermissionSet","PermissionSetGroup","Layout","ValidationRule","RecordType","ListView","Report","EmailTemplate","Other (Type manually)..."],o=await X.window.showQuickPick(e,{placeHolder:"Select Metadata Type to extract (e.g., ApexClass)",ignoreFocusOut:!0});if(!o||o==="Other (Type manually)..."&&(o=await X.window.showInputBox({prompt:"Enter Metadata Type (e.g., CustomApplication, Queue)",ignoreFocusOut:!0}),!o))return;let s={ApexClass:"**/*.cls",ApexTrigger:"**/*.trigger",CustomObject:"**/*.{object,object-meta.xml}",CustomField:"**/*.field-meta.xml",LightningComponentBundle:"**/lwc/*/*.js",AuraDefinitionBundle:"**/aura/*/*.cmp",Flow:"**/*.flow-meta.xml",CustomLabel:"**/*.labels-meta.xml",CustomMetadata:"**/*.md-meta.xml",StaticResource:"**/*.resource-meta.xml",Profile:"**/*.profile-meta.xml",PermissionSet:"**/*.permissionset-meta.xml",PermissionSetGroup:"**/*.permissionsetgroup-meta.xml",Layout:"**/*.layout-meta.xml",ValidationRule:"**/*.validationRule-meta.xml",RecordType:"**/*.recordType-meta.xml",ListView:"**/*.listView-meta.xml"},r=[],c=s[o];if(c)try{r=(await X.workspace.findFiles(c,"**/node_modules/**")).map(p=>{let l=p.fsPath.split(/[\\/]/).pop()||"";if(o==="LightningComponentBundle"||o==="AuraDefinitionBundle"){let i=p.fsPath.split(/[\\/]/);return i[i.length-2]||l.split(".")[0]}return l.split(".")[0]}),r=[...new Set(r)].sort()}catch{}let d=await new Promise(u=>{let p=X.window.createQuickPick();p.title=`Extract ${o}`,p.placeholder="Type name (e.g. MyComponent) or * for all",p.ignoreFocusOut=!0,p.matchOnDescription=!0;let l=()=>{let i=p.value.trim(),n=[];i?n.push({label:`$(cloud-download) Extract "${i}"`,description:"Extract exact name from Salesforce",alwaysShow:!0}):n.push({label:'$(cloud-download) Extract "*" (All)',description:`Extract all ${o}s`,alwaysShow:!0}),r.forEach(a=>{(!i||a.toLowerCase().includes(i.toLowerCase()))&&n.push({label:a,description:"Local workspace component"})}),p.items=n};p.onDidChangeValue(()=>l()),p.onDidAccept(()=>{let i=p.selectedItems[0];if(i){let n=i.label;n.startsWith('$(cloud-download) Extract "')?n=n.replace('$(cloud-download) Extract "',"").replace('" (All)',"").replace('"',""):n==='$(cloud-download) Extract "*" (All)'&&(n="*"),p.hide(),u(n)}}),p.onDidHide(()=>{p.dispose(),u(void 0)}),l(),p.show()});d&&await X.window.withProgress({location:X.ProgressLocation.Notification,title:`Ricwiz: Extracting ${o}:${d} from Salesforce...`,cancellable:!0},async(u,p)=>{try{I.show(!0);let l=`${o}:${d}`,{stdout:i,stderr:n}=await h(`sf project retrieve start -m "${l}"`,{cwd:t});i&&I.appendLine(i),n&&I.appendLine(n),X.window.showInformationMessage(`Ricwiz: Successfully extracted ${l}.`)}catch(l){I.appendLine(`ERROR: ${l.message}`),l.stdout&&I.appendLine(l.stdout),l.stderr&&I.appendLine(l.stderr),X.window.showErrorMessage("Ricwiz: Extraction failed. See Output channel for details.")}})}var X,Io=z(()=>{"use strict";X=b(require("vscode"));S()});async function Oo(){let t=W.window.activeTextEditor;if(!t){W.window.showErrorMessage("Ricwiz: Please open the file you want to deploy in the editor first.");return}let e=t.document.uri.fsPath,o=y();if(!o)return;let s="";if(await W.window.withProgress({location:W.ProgressLocation.Notification,title:"Ricwiz: Fetching available Salesforce orgs...",cancellable:!1},async()=>{try{let{stdout:p}=await h("sf org list --json",{cwd:o});s=p}catch(p){s=p.stdout||""}}),!s){W.window.showErrorMessage("Ricwiz: Failed to fetch orgs. Is Salesforce CLI installed?");return}let r=[];try{let p=JSON.parse(s),l=p.result?.nonScratchOrgs||[],i=p.result?.scratchOrgs||[];r=[...l,...i]}catch{W.window.showErrorMessage("Ricwiz: Failed to parse org list.");return}if(r.length===0){W.window.showInformationMessage("Ricwiz: No authenticated orgs found.");return}let c=r.map(p=>({label:p.alias||p.username,description:p.alias?p.username:"",picked:p.isDefaultUsername})),d=await W.window.showQuickPick(c,{placeHolder:"Select the org(s) to deploy this file to",canPickMany:!0,ignoreFocusOut:!0});if(!d||d.length===0)return;let u=Fo.basename(e);await W.window.withProgress({location:W.ProgressLocation.Notification,title:`Ricwiz: Deploying ${u} to ${d.length} org(s)...`,cancellable:!1},async()=>{I.show(!0),I.appendLine(`--- Starting Parallel Deploy of ${u} ---`);let p=d.map(async a=>{let m=a.label;I.appendLine(`[${m}] Deploying...`);try{let{stdout:g,stderr:f}=await h(`sf project deploy start -d "${e}" -o "${m}"`,{cwd:o});return I.appendLine(`[${m}] \u2705 Success`),g&&I.appendLine(g),{org:m,success:!0}}catch(g){return I.appendLine(`[${m}] \u274C Failed`),g.stdout&&I.appendLine(g.stdout),g.stderr&&I.appendLine(g.stderr),{org:m,success:!1}}}),l=await Promise.all(p),i=l.filter(a=>a.success).length,n=l.filter(a=>!a.success).length;n===0?W.window.showInformationMessage(`Ricwiz: Successfully deployed to all ${i} orgs!`):W.window.showErrorMessage(`Ricwiz: Deploy finished with errors (${i} success, ${n} failed). Check Output channel.`)})}var W,Fo,No=z(()=>{"use strict";W=b(require("vscode")),Fo=b(require("path"));S()});async function Uo(){let t=y();if(!t){A.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=A.workspace.getConfiguration("ricwiz"),o=e.get("auditUsername",""),s=e.get("auditHours",8),r=await A.window.showInputBox({prompt:"Enter your Salesforce Username to query in SetupAuditTrail",value:o,placeHolder:"admin@tuaorg.com"});if(!r)return;let c=await A.window.showInputBox({prompt:"How many hours back do you want to search?",value:s.toString(),placeHolder:"8"});if(!c)return;let d=parseFloat(c);if(isNaN(d)||d<=0){A.window.showErrorMessage("Ricwiz: Invalid hours specified.");return}let u=new Date(Date.now()-d*60*60*1e3).toISOString(),l=`sf data query -q "${`SELECT Action, Display, Section FROM SetupAuditTrail WHERE CreatedBy.Username = '${r}' AND CreatedDate >= ${u}`}" --json`;await A.window.withProgress({location:A.ProgressLocation.Notification,title:"Ricwiz: Interrogating Setup Audit Trail...",cancellable:!1},async()=>{try{let{stdout:i}=await h(l,{cwd:t,maxBuffer:52428800}),n=JSON.parse(i);if(!n.result||n.result.records.length===0){A.window.showInformationMessage(`Ricwiz: No changes found for ${r} in the last ${d} hours.`);return}let a=n.result.records,m=[],g=new Set;for(let E of a){let L=di(E.Action,E.Display,E.Section);if(L){let ee=`${L.isDelete?"DEL":"ADD"}-${L.metadataFormat}`;if(!g.has(ee)){g.add(ee);let V=L.isDelete?"$(trash)":"$(plus)";m.push({label:`${V} ${L.metadataFormat}`,description:`${E.Action} -> ${E.Display}`,metadataFormat:L.metadataFormat,isDelete:L.isDelete})}}}if(m.length===0){A.window.showInformationMessage(`Ricwiz: No extractable metadata changes found for ${r} in the last ${d} hours (ignored passwords/logins).`);return}let f=await A.window.showQuickPick(m,{canPickMany:!0,placeHolder:"Select the changes you want to extract to GitLab",ignoreFocusOut:!0});if(!f||f.length===0){A.window.showInformationMessage("Ricwiz: No changes selected.");return}let w=f.filter(E=>E.isDelete),v=f.filter(E=>!E.isDelete),x=A.window.createOutputChannel("Ricwiz Admin Bridge");if(x.show(),w.length>0){let{stdout:E}=await h("git ls-files",{cwd:t}),L=E.split(`
`).map(V=>V.trim()),ee=0;for(let V of w){let de=V.metadataFormat.split(":"),je=de[0],Je=de[1],ve=Je;je==="CustomField"&&(ve=Je.split(".")[1]);let nt=L.filter(Re=>{let T=it.basename(Re);return T.startsWith(ve+".")&&T.includes(je==="CustomField"?".field":"")});for(let Re of nt){let T=it.join(t,Re);ot.existsSync(T)&&(ot.unlinkSync(T),x.appendLine(`Deleted local file: ${Re}`),ee++)}}A.window.showInformationMessage(`Ricwiz: Deleted ${ee} local files from Git workspace.`)}if(v.length===0)return;let R=v.map(E=>E.metadataFormat).filter(E=>E!=="").join(", "),B=await A.window.showInputBox({prompt:"Review and adjust the metadata components to retrieve",value:R,ignoreFocusOut:!0});if(!B)return;let U=`sf project retrieve start -m "${B}"`;x.appendLine(`Executing: ${U}`),A.window.showInformationMessage(`Ricwiz: Extracting ${v.length} components...`);let ge=await h(U,{cwd:t});x.appendLine(ge.stdout),ge.stderr&&(x.appendLine("--- STDERR ---"),x.appendLine(ge.stderr)),A.window.showInformationMessage("Ricwiz: Changes extracted successfully! Ready for Git Commit.")}catch(i){A.window.showErrorMessage(`Ricwiz: Error capturing changes - ${i.message}`)}})}function di(t,e,o){if(!t||!e||!o)return null;let s=t.toLowerCase(),r=o.toLowerCase();if(["security controls","network access","session settings","data export","login history","password policies","identity verification","delegated administration"].includes(r)||s.includes("login")||s.includes("password")||s.includes("oauth")||s.includes("session"))return null;let d=s.includes("delete"),u=null;if(s==="permissionsetgroupcomponentadd"||s==="permissionsetgroupcomponentdelete")return null;let p=(l,i=!1)=>{let n=l.replace(/\(.*\)/g,"").trim();n.includes(":")&&!s.includes("calculation")&&(n=n.split(":")[0]);let a=["disabled","deleted","removed","created","changed","updated","from","to","on","assigned","assign","assignment","permission","set","group","apex","class","trigger","custom","field","object","layout","validation","rule","flow","profile"],m=n.split(/\s+/);if(i){for(;m.length>0&&a.includes(m[m.length-1].toLowerCase());)m.pop();for(;m.length>0&&a.includes(m[0].toLowerCase());)m.shift();return m.join(" ").trim().replace(/[^a-zA-Z0-9_ ]/g,"")}else return m.filter(w=>!a.includes(w.toLowerCase())).join("_").replace(/[^a-zA-Z0-9_]/g,"")||n.replace(/\s+/g,"")};if(s.includes("profile"))u=`Profile:${p(e,!0)}`;else if(s.includes("permissionsetgroupcalculation")){let l=e.split(":");u=`PermissionSetGroup:${l.length>1?l[l.length-1].trim():p(e,!1)}`}else if(s.includes("permission set group")||s.includes("permissionsetgroup"))u=`PermissionSetGroup:${p(e,!1)}`;else if(s.includes("permission set")||s.includes("permissionset"))u=`PermissionSet:${p(e,!1)}`;else if(s.includes("apexclass"))u=`ApexClass:${p(e,!1)}`;else if(s.includes("apextrigger")||s.includes("apex trigger"))u=`ApexTrigger:${p(e,!1)}`;else if(s.includes("customfield")){let l=e.match(/([A-Za-z0-9_]+__c)/),i=e.match(/(?:on|na|for)\s+([A-Za-z0-9_]+)/i);l&&i?u=`CustomField:${i[1]}.${l[1]}`:u=`CustomField:${p(e,!1)}`}else if(s.includes("layout"))u=`Layout:${p(e,!0)}`;else if(s.includes("validation"))u=`ValidationRule:${p(e,!1)}`;else if(s.includes("flow"))u=`Flow:${p(e,!1)}`;else if(s.includes("customobject")){let l=e.match(/([A-Za-z0-9_]+__c)/);u=l?`CustomObject:${l[1]}`:`CustomObject:${p(e,!1)}`}else if(!s.includes("created")&&!s.includes("changed")&&!s.includes("deleted"))return null;return u?{metadataFormat:u,isDelete:d}:null}var A,ot,it,jo=z(()=>{"use strict";A=b(require("vscode")),ot=b(require("fs")),it=b(require("path"));S()});async function Jo(){let t=y();if(t)try{let{stdout:e}=await h('git for-each-ref --sort=-committerdate --format="%(refname:short)|||%(committerdate:relative)|||%(subject)" refs/heads/',{cwd:t}),o=e.split(`
`).filter(r=>r.trim()).map(r=>{let c=r.split("|||");return{label:`$(git-branch) ${c[0]}`,description:c[1],detail:c[2],branchName:c[0]}}),s=await ut.window.showQuickPick(o,{placeHolder:"Select a branch from history to checkout",matchOnDescription:!0,matchOnDetail:!0});s&&await Me(s.branchName)}catch{ut.window.showErrorMessage("Ricwiz: Failed to get branch history")}}var ut,Wo=z(()=>{"use strict";ut=b(require("vscode"));S();et()});async function qo(){let t=y();if(!t)return;let e=await Oe.window.showInputBox({prompt:"Enter ticket number or name (e.g., 48934)",placeHolder:"48934"});if(e)try{let{stdout:o}=await h(`git branch --list "*${e}*"`,{cwd:t}),s=o.split(`
`).map(d=>d.replace("*","").trim()).filter(d=>d);if(s.length===0){Oe.window.showInformationMessage(`Ricwiz: No branches found matching "${e}"`);return}let r=s.map(d=>({label:`$(git-branch) ${d}`,branchName:d})),c=await Oe.window.showQuickPick(r,{placeHolder:`Select a branch for ${e}`});c&&await Me(c.branchName)}catch{Oe.window.showErrorMessage("Ricwiz: Failed to search branches")}}var Oe,Go=z(()=>{"use strict";Oe=b(require("vscode"));S();et()});async function Vo(){let t=$e.window.activeTextEditor;if(!t)return $e.window.showErrorMessage("Ricwiz: Please open a file in the editor to check blame."),null;let e=t.document.fileName,o=Ho.basename(e),s=y();if(!s)return $e.window.showErrorMessage("Ricwiz: Workspace is not a git repository."),null;let r=[];try{let{stdout:i}=await h(`git log -5 --pretty=format:"%an|%ar|%s|%h" -- "${e}"`,{cwd:s}),n=i.trim().split(`
`);for(let a of n){let m=a.split("|");m.length>=4&&r.push({author:m[0],time:m[1],message:m.slice(2,-1).join("|"),hash:m[m.length-1]})}}catch(i){console.error("Git blame error:",i)}let c="Unknown",d="Unknown",u="Unknown",p=[],l=li(e);if(l)try{await $e.window.withProgress({location:$e.ProgressLocation.Notification,title:`Ricwiz: Analyzing ${l.name} in Salesforce...`,cancellable:!1},async()=>{let i="";if(l.type==="CustomField"){let n=l.name.split(".");n.length===2&&(i=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM CustomField WHERE DeveloperName = '${n[1].replace("__c","")}' AND TableEnumOrId = '${n[0]}'`)}else l.type==="LightningComponentBundle"?i=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM LightningComponentBundle WHERE DeveloperName = '${l.name}'`:i=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM ${l.type} WHERE Name = '${l.name}'`;if(i)try{let{stdout:n}=await h(`sf data query -t -q "${i}" --json`,{cwd:s,maxBuffer:52428800}),a=JSON.parse(n);if(a&&a.result&&a.result.records&&a.result.records.length>0){let m=a.result.records[0];c=m.LastModifiedBy?m.LastModifiedBy.Name:"Unknown",u=m.CreatedBy?m.CreatedBy.Name:"Unknown",d=new Date(m.LastModifiedDate).toLocaleString()}else c="Not found in Org",d="N/A",u="N/A"}catch{c="Query Error",d="N/A",u="N/A"}try{let n="SELECT Action, Display, CreatedBy.Name, CreatedDate FROM SetupAuditTrail ORDER BY CreatedDate DESC LIMIT 1500",{stdout:a}=await h(`sf data query -q "${n}" --json`,{cwd:s,maxBuffer:52428800}),m=JSON.parse(a);if(m&&m.result&&m.result.records){let g=l.name.replace("__c","");p=m.result.records.filter(w=>w.Display&&w.Display.includes(g)).map(w=>({action:w.Action,display:w.Display,author:w.CreatedBy?w.CreatedBy.Name:"Unknown",time:new Date(w.CreatedDate).toLocaleString()})).slice(0,10)}}catch(n){console.error("Audit trail query error:",n)}})}catch(i){console.error("Salesforce query error:",i)}else c="Unsupported Metadata Type",d="N/A";return{fileName:o,gitHistory:r,sfAuthor:c,sfTime:d,sfCreatedBy:u,auditHistory:p}}function li(t){let e=t.replace(/\\/g,"/");if(e.includes("/classes/")){let o=e.match(/\/classes\/([^/.]+)\.cls/);if(o)return{type:"ApexClass",name:o[1]}}if(e.includes("/triggers/")){let o=e.match(/\/triggers\/([^/.]+)\.trigger/);if(o)return{type:"ApexTrigger",name:o[1]}}if(e.includes("/lwc/")){let o=e.match(/\/lwc\/([^/]+)\//);if(o)return{type:"LightningComponentBundle",name:o[1]}}if(e.includes("/aura/")){let o=e.match(/\/aura\/([^/]+)\//);if(o)return{type:"AuraDefinitionBundle",name:o[1]}}if(e.includes("/objects/")&&e.includes("/fields/")){let o=e.match(/\/objects\/([^/]+)\//),s=e.match(/\/fields\/([^/.]+)\.field/);if(o&&s)return{type:"CustomField",name:`${o[1]}.${s[1]}`}}return null}var $e,Ho,_o=z(()=>{"use strict";$e=b(require("vscode")),Ho=b(require("path"));S()});function Qo(t,e,o){t.subscriptions.push(C.commands.registerCommand("ricwiz.generateDestructiveChanges",Rt),C.commands.registerCommand("ricwiz.runSmartTests",St),C.commands.registerCommand("ricwiz.refreshWebview",()=>{e&&C.commands.executeCommand("workbench.action.webview.reloadWebviewAction")}),C.commands.registerCommand("ricwiz.createBranches",Mt),C.commands.registerCommand("ricwiz.prepareDeploy",Ut),C.commands.registerCommand("ricwiz.createMergeRequests",Wt),C.commands.registerCommand("ricwiz.createMergeRequestsVSCode",qt),C.commands.registerCommand("ricwiz.openJiraTicket",Vt),C.commands.registerCommand("ricwiz.openJiraTicketVSCode",_t),C.commands.registerCommand("ricwiz.showJiraDetails",()=>{e&&io(e)}),C.commands.registerCommand("ricwiz.openJiraDashboard",s=>{e&&so(e,s)}),C.commands.registerCommand("ricwiz.openJiraDetailsForId",s=>{e&&ro(e,s)}),C.commands.registerCommand("ricwiz.toggleDashboardBranches",s=>{e&&(e.setDashboardShowBranches(s),C.commands.executeCommand("ricwiz.openJiraDashboard"))}),C.commands.registerCommand("ricwiz.changeJiraStatus",co),C.commands.registerCommand("ricwiz.addJiraComment",lo),C.commands.registerCommand("ricwiz.addJiraLabel",mo),C.commands.registerCommand("ricwiz.setJiraToken",po),C.commands.registerCommand("ricwiz.setGitlabToken",go),C.commands.registerCommand("ricwiz.syncAll",ho),C.commands.registerCommand("ricwiz.updateBases",vo),C.commands.registerCommand("ricwiz.deleteUnusedBranches",yo),C.commands.registerCommand("ricwiz.checkoutBranch",Me),C.commands.registerCommand("ricwiz.copyBranchName",ko),C.commands.registerCommand("ricwiz.generatePackageXml",Ro),C.commands.registerCommand("ricwiz.deployPackage",So),C.commands.registerCommand("ricwiz.importData",Bo),C.commands.registerCommand("ricwiz.listTicketFiles",To),C.commands.registerCommand("ricwiz.resetTracking",Do),C.commands.registerCommand("ricwiz.extractComponent",Lo),C.commands.registerCommand("ricwiz.deployMultiOrg",Oo),C.commands.registerCommand("ricwiz.captureAdminChanges",Uo),C.commands.registerCommand("ricwiz.openHistory",Jo),C.commands.registerCommand("ricwiz.searchTicket",qo),C.commands.registerCommand("ricwiz.whoToBlame",async()=>{let s=await Vo();s&&e&&(e.setBlameData(s),e.setPage("blame"))}),C.commands.registerCommand("ricwiz.manualRefresh",()=>{o&&o()}),C.commands.registerCommand("ricwiz.toggleAutoRefresh",()=>{if(e){let s=!e.isAutoRefreshEnabled();e.setAutoRefresh(s),C.workspace.getConfiguration("ricwiz").update("autoRefresh",s,C.ConfigurationTarget.Global)}}),C.commands.registerCommand("ricwiz.openSettings",()=>{C.commands.executeCommand("workbench.action.openSettings","ricwiz")}))}var C,Yo=z(()=>{"use strict";C=b(require("vscode"));zt();Pt();Dt();jt();Gt();Qt();no();ao();uo();fo();wo();bo();xo();et();Co();zo();Po();Eo();Mo();Ao();Io();No();jo();Wo();Go();_o()});function Zo(t,e,o){let s,r=ce.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(r),t.subscriptions.push(ce.workspace.onDidChangeConfiguration(d=>{if(d.affectsConfiguration("ricwiz.autoRefresh")){let u=ce.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(u)}}));async function c(){let d=ce.extensions.getExtension("vscode.git");if(d){let l=function(i){let n="",a;async function m(){let f=ce.workspace.workspaceFolders;if(!f)return;let w=f[0].uri.fsPath,v=await F(w);if(v&&v!==n){n=v;let x=ce.workspace.getConfiguration("ricwiz"),R=x.get("ticketPrefix","SFPSCA-");if(!v.includes(R)){let T=v.match(/([A-Z]+-)\d+/i);T&&(R=T[1].toUpperCase())}let B=[],U=[],ge=[],E=[],L=await D.initialize(w,{skipPrompt:!0}),ee=L?.environments||x.get("environments",[{name:"Qual",sourceBranch:"quality"},{name:"Val",sourceBranch:"validation"},{name:"Prod",sourceBranch:"main"}]);try{let T=x.get("workspaceCheckoutButtons",["main","quality","validation"]);ge=Array.from(new Set(T))}catch{}let V="",de=v.match(new RegExp(`(${R}\\d+(?:-\\d+)?)`,"i"));if(de){let T=de[1].toUpperCase();V=T;let be=x.get("commitMessageSuffix","- "),gt=/^[A-Z]+-\d+(?:-\d+)?\s*(?:-\s*|:\s*|\s+)?/i;gt.test(i.inputBox.value)?i.inputBox.value.toUpperCase().startsWith(T)||(i.inputBox.value=i.inputBox.value.replace(gt,`${T}${be}`)):i.inputBox.value=`${T}${be}`+i.inputBox.value,o.text=`$(bookmark) ${T}`,o.tooltip=`Branch: ${v}
Click to open Jira ticket`,o.show();try{let ft=await mt(w,T,"");B=await at(w,ft,T,ee,L)}catch{}}else{o.hide();try{E=await lt(w)}catch{}}let[je,Je,ve]=await Promise.all([dt(w,10),ct(w,v,ee,L),V?Te(V).catch(T=>{let be=T.message;return(be.includes("ENOTFOUND")||be.includes("network"))&&(be="No Internet or Invalid URL"),{summary:`\u26A0\uFE0F Jira Error: ${be}`,description:"",status:""}}):Promise.resolve(null)]);U=je;let nt=ve?ve.summary:"",Re=ve&&ve.status||"";e?.updateBranch(v,Je,B,U,ge,E,nt,Re)}}function g(){e?.isAutoRefreshEnabled()&&(a&&clearTimeout(a),a=setTimeout(()=>{n="",m()},300))}s=()=>{n="",m()},m(),i.state.onDidChange(()=>g()),ce.window.onDidChangeWindowState(f=>{f.focused&&g()})};var u=l;d.isActive||await d.activate();let p=d.exports.getAPI(1);p.repositories.length>0&&p.repositories.forEach(i=>l(i)),p.onDidOpenRepository(i=>l(i))}}return c(),()=>{s&&s()}}var ce,Ko=z(()=>{"use strict";ce=b(require("vscode"));S();we();Ie();te()});var Ve={};De(Ve,{activate:()=>mi,deactivate:()=>pi,webviewProvider:()=>Ne});module.exports=O(Ve);function mi(t){vt(t),Ne=new qe(t.extensionUri),t.subscriptions.push(Ue.window.registerWebviewViewProvider("ricwiz-webview",Ne));let e=Ue.window.createStatusBarItem(Ue.StatusBarAlignment.Left,100);e.command="ricwiz.openJiraTicket",t.subscriptions.push(e);let o=Zo(t,Ne,e);Qo(t,Ne,o)}function pi(){}var Ue,Ne,_e=z(()=>{Ue=b(require("vscode"));wt();ze();Yo();Ko()});_e();0&&(module.exports={activate,deactivate,webviewProvider});
