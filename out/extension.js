...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('ricwiz.checkoutBranch', async (branchName) => {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders)
            return;
        const cwd = workspaceFolders[0].uri.fsPath;
    ...
    let disposable = vscode.commands.registerCommand('ricwiz.createBranches', async () => {
        ...
        const cwd = workspaceFolders[0].uri.fsPath;
        const config = vscode.workspace.getConfiguration('ricwiz');
        const prefix = config.get('ticketPrefix', 'SFPSCA-');
        let currentBranch = '';
        try {
            const { stdout } = await exec('git branch --show-current', { cwd });
            currentBranch = stdout.trim();
        ...
        if (!currentBranch.includes(prefix)) {
            const guessMatch = currentBranch.match(/([A-Z]+-)\d+/i);
            if (guessMatch)
                actualPrefix = guessMatch[1].toUpperCase();
        }
        let suggestedTicket = '';
        ...
        try {
            ...
            const checkBranchExists = async (b) => {
                try {
            ...
            for (const env of environments) {
                ...
                if (await checkBranchExists(envBranchName)) {
                    ...
                    try {
                        ...
                        try {
                            await exec(`git pull origin ${sourceBranch}`, { cwd });
                        }
                        catch (e) { }
    ...
    const doCreateMergeRequests = async (openInVSCode = false) => {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders)
            return;
        const cwd = workspaceFolders[0].uri.fsPath;
    ...
    const doOpenJiraTicket = async (openInVSCode = false) => {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders)
            return;
        const cwd = workspaceFolders[0].uri.fsPath;
