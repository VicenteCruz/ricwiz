...
async function prepareDeploy() {
    ...
    await vscode.window.withProgress({
        ...
        const handleMergeConflict = async (sourceStr, targetStr) => {
            progress.report({ message: `CONFLICT! Resolve & COMMIT to auto-resume.` });
            vscode.window.showWarningMessage(`Ricwiz: CONFLICT! Merging ${sourceStr} into ${targetStr}. Please resolve the conflicts and COMMIT. The deploy will automatically resume.`);
            while (true) {
                if (abortRequested) {
                    try {
