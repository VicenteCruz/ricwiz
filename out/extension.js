...
async function getGitlabTargets(cwd, ctx) {
  ...
  const targets = candidateUrls.map((webUrl) => {
    ...
    let path9 = urlObj.pathname;
    if (path9.startsWith("/")) path9 = path9.substring(1);
    if (path9.endsWith("/")) path9 = path9.slice(0, -1);
    if (path9.endsWith(".git")) path9 = path9.slice(0, -4);
    const projectPath = encodeURIComponent(path9);
...
async function fetchMergeRequestStatus(cwd, sourceBranch, targetBranch, ctx) {
  ...
  try {
    ...
    for (const target of targets) {
      try {
        let path9 = `/api/v4/projects/${target.projectPath}/merge_requests?source_branch=${encodeURIComponent(sourceBranch)}&order_by=updated_at&sort=desc`;
        if (targetBranch) {
          path9 += `&target_branch=${encodeURIComponent(targetBranch)}`;
        }
        const mrs = await gitlabRequest(target.baseUrl, target.token, "GET", path9);
        if (mrs && mrs.length > 0) {
          let mr = mrs[0];
          try {
    ...
    for (const target of targets) {
      try {
        const path9 = `/api/v4/projects/${target.projectPath}/pipelines?ref=${encodeURIComponent(sourceBranch)}&order_by=updated_at&sort=desc`;
        const pipelines = await gitlabRequest(target.baseUrl, target.token, "GET", path9);
...
async function jiraRequest(method, path9, body) {
  ...
  try {
    ...
    if (!baseUrl) {
      ...
      for (const remote of allRemotes) {
        ...
        if (path9.startsWith("/")) path9 = path9.substring(1);
        if (path9.endsWith("/")) path9 = path9.slice(0, -1);
        if (encodeURIComponent(path9) === projectPath || path9 === projectPath) {
...
function registerAllCommands(context, webviewProvider2, forceUpdate) {
  context.subscriptions.push(
    vscode36.commands.registerCommand("ricwiz.conflictAction", executeConflictAction),
    vscode36.commands.registerCommand("ricwiz.generateDestructiveChanges", async () => {
      try {
        await generateDestructiveChanges();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
      }
    }),
    vscode36.commands.registerCommand("ricwiz.runSmartTests", async () => {
      try {
        await runSmartTests();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.refreshWebview", () => {
      if (webviewProvider2) vscode36.commands.executeCommand("workbench.action.webview.reloadWebviewAction");
    }),
    vscode36.commands.registerCommand("ricwiz.createBranches", async (ticket) => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.prepareDeploy", async () => {
      try {
        await prepareDeploy();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.createMergeRequests", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.createMergeRequestsVSCode", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.openJiraTicket", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.openJiraTicketVSCode", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.showJiraDetails", () => {
      if (webviewProvider2) showJiraDetails(webviewProvider2);
    }),
    vscode36.commands.registerCommand("ricwiz.openJiraDashboard", (indexOverride) => {
      if (webviewProvider2) openJiraDashboard(webviewProvider2, indexOverride);
    }),
    vscode36.commands.registerCommand("ricwiz.openJiraDetailsForId", (ticketId) => {
      if (webviewProvider2) openJiraDetailsForId(webviewProvider2, ticketId);
    }),
    vscode36.commands.registerCommand("ricwiz.toggleDashboardBranches", (show) => {
      if (webviewProvider2) {
        webviewProvider2.setDashboardShowBranches(show);
        vscode36.commands.executeCommand("ricwiz.openJiraDashboard");
      }
    }),
    vscode36.commands.registerCommand("ricwiz.changeJiraStatus", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.addJiraComment", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.addJiraLabel", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.setJiraToken", setJiraTokenCommand),
    vscode36.commands.registerCommand("ricwiz.setGitlabToken", setGitlabTokenCommand),
    vscode36.commands.registerCommand("ricwiz.syncAll", async () => {
      try {
        await syncAll();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.updateBases", async () => {
      try {
        await updateBases();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.deleteUnusedBranches", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.checkoutBranch", async (branchName) => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.copyBranchName", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.generatePackageXml", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.deployPackage", async () => {
      try {
        await deployPackage();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.importData", async () => {
      try {
        await importData();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.listTicketFiles", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.resetTracking", async () => {
      try {
        await resetTracking();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.extractComponent", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.deployMultiOrg", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.captureAdminChanges", async () => {
      try {
    ...
    vscode36.commands.registerCommand("ricwiz.openHistory", async () => {
      try {
        await openHistory();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.searchTicket", async () => {
      try {
        await searchTicket();
      } finally {
        vscode36.commands.executeCommand("ricwiz.manualRefresh");
    ...
    vscode36.commands.registerCommand("ricwiz.whoToBlame", async () => {
    ...
    vscode36.commands.registerCommand("ricwiz.showPipelineLogs", (projectPath, pipelineId) => showPipelineLogs(projectPath, pipelineId)),
    vscode36.commands.registerCommand("ricwiz.manualRefresh", () => {
      if (forceUpdate) {
        forceUpdate();
      }
    }),
    vscode36.commands.registerCommand("ricwiz.toggleAutoRefresh", () => {
      if (webviewProvider2) {
        const newState = !webviewProvider2.isAutoRefreshEnabled();
        webviewProvider2.setAutoRefresh(newState);
        vscode36.workspace.getConfiguration("ricwiz").update("autoRefresh", newState, vscode36.ConfigurationTarget.Global);
      }
    }),
    vscode36.commands.registerCommand("ricwiz.openSettings", () => {
      vscode36.commands.executeCommand("workbench.action.openSettings", "ricwiz");
    })
  );
}
...
function initializeGitMonitor(context, webviewProvider2, statusBarItem) {
  let forceUpdate;
  const initialAutoRefresh = vscode37.workspace.getConfiguration("ricwiz").get("autoRefresh", true);
  ...
  context.subscriptions.push(
    vscode37.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("ricwiz.autoRefresh")) {
        const enabled = vscode37.workspace.getConfiguration("ricwiz").get("autoRefresh", true);
        webviewProvider2?.setAutoRefresh(enabled);
      }
    })
  );
  async function initGit() {
    const gitExtension = vscode37.extensions.getExtension("vscode.git");
    if (gitExtension) {
      let setupRepo2 = function(repo) {
        ...
        async function update() {
          ...
          if (currentBranch && currentBranch !== lastBranch) {
            ...
            const config = vscode37.workspace.getConfiguration("ricwiz");
...
Click to open Jira ticket`;
        ...
        context.subscriptions.push(vscode37.window.onDidChangeWindowState((e) => {
          if (e.focused) {
            scheduleUpdate();
          }
        }));
...
var AiSkills = {
   ...
   */
  get_tickets_batch: async (ticketIds) => {
...
function setupCliEnvironment(port) {
  const ricwizDir = path8.join(os.homedir(), ".ricwiz");
  if (!fs6.existsSync(ricwizDir)) {
    fs6.mkdirSync(ricwizDir, { recursive: true });
  }
  fs6.writeFileSync(path8.join(ricwizDir, "port.txt"), port.toString(), "utf8");
  const cliScript = `
...
`;
  ;
  fs6.writeFileSync(path8.join(ricwizDir, "jira-cli.js"), cliScript.trim(), "utf8");
}
...
async function activate(context) {
  ...
  context.subscriptions.push(
    vscode38.window.registerWebviewViewProvider("ricwiz-webview", webviewProvider)
  );
  ...
  return {
    getJiraCredentials: async () => ({
      email: vscode38.workspace.getConfiguration("ricwiz").get("jiraEmail", ""),
