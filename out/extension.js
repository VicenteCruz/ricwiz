"use strict";var Uo=Object.create;var qe=Object.defineProperty;var Oo=Object.getOwnPropertyDescriptor;var jo=Object.getOwnPropertyNames;var No=Object.getPrototypeOf,Jo=Object.prototype.hasOwnProperty;var Wo=(t,e)=>{for(var o in e)qe(t,o,{get:e[o],enumerable:!0})},vt=(t,e,o,s)=>{if(e&&typeof e=="object"||typeof e=="function")for(let i of jo(e))!Jo.call(t,i)&&i!==o&&qe(t,i,{get:()=>e[i],enumerable:!(s=Oo(e,i))||s.enumerable});return t};var k=(t,e,o)=>(o=t!=null?Uo(No(t)):{},vt(e||!t||!t.__esModule?qe(o,"default",{value:t,enumerable:!0}):o,t)),Go=t=>vt(qe({},"__esModule",{value:!0}),t);var ei={};Wo(ei,{activate:()=>Zo,deactivate:()=>Xo,webviewProvider:()=>ne});module.exports=Go(ei);var Me=k(require("vscode"));var P=k(require("vscode"));function C(t){return t?t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function Ve(t){let e=(t||"").toLowerCase().trim();return e==="open"?"#888888":e==="in progress"?"#007acc":e==="waiting for deploy"?"#d7a500":e==="close"||e==="done"||e==="closed"?"#238636":"var(--vscode-badge-background)"}function be(t){return t?t==="running"?"\u{1F7E1}":t==="success"?"\u{1F7E2}":t==="failed"?"\u{1F534}":t==="canceled"||t==="skipped"?"\u26AA":"":""}function H(){return`
        <script>
    ...
    `}function yt(t,e){let o=q(),s=(e.files||[]).map(i=>`
    ...
    </html>`}function xt(t){let e=q(),o=t?.ticketId||"Jira",s=t?.summary||"No Title",i=t?.description||"No description provided.",p=t?.relatedBranches||[];return`<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    </html>`}function kt(t){let{data:e,showBranches:o}=t,s=q(),i=e||{queries:[],selectedIndex:0,results:[],error:null},p=i.queries.map((c,a)=>`
                ...
                ${i.results.map(c=>`
                    <tr style="border-bottom: ${c.detailedBranches&&c.detailedBranches.length>0?"none":"1px solid var(--vscode-panel-border)"}; cursor: pointer;" class="tr-hover" onclick="sendCommand('openJiraDetailsForId', '${C(c.key)}', this)">
                        ...
                        <td style="padding: 6px; white-space: nowrap;">
                            <span style="background: ${Ve(c.status)}; color: white; padding: 2px 4px; border-radius: 3px; font-size: 9px; font-weight: bold; text-transform: uppercase;">${C(c.status)}</span>
                        </td>
    ...
    </html>`}function $t(t){let{logoUri:e,currentBranch:o,currentBranchIsMerged:s,relatedBranches:i,commits:p,baseBranches:n,recentTickets:c,ticketTitle:a,ticketStatus:d,autoRefreshEnabled:r}=t,m=q(),l=p.length>0?`
...
`).map(l=>l.trimEnd()).filter(l=>l.length>2).filter(l=>{let u=l.substring(0,2);return["UU","AA","UD","DU","AU","UA","DD"].includes(u)}).map(l=>{let u=l.substring(0,2);return{file:l.substring(3).trim(),state:r(u)}})}catch{return[]}},d=async()=>{if(p)return;let r=await c(),m=await a();ne&&ne.setConflictState({isConflict:!0,sourceStr:e,targetStr:o,deletionsCount:r.length,files:m})};for(at(async r=>{if(r==="abortDeploy")n=!0;else if(r==="resolveDeletions"){try{let l=(await c()).map(f=>({label:f})),u=await ve.window.showQuickPick(l,{canPickMany:!0,placeHolder:"Select conflicted files to DELETE",title:"Ricwiz: Delete Conflicted Files"});if(u&&u.length>0){for(let f of u)try{await g(`git rm --force "${f.label}"`,{cwd:t})}catch{}ve.window.showInformationMessage(`Ricwiz: Deleted ${u.length} conflicted file(s).`)}}catch(m){ve.window.showErrorMessage(`Ricwiz: Error. (${m.message})`)}d()}else if(r==="commitAndContinue")try{let l=(await c()).filter(f=>Oe.existsSync(je.join(t,f)));if(l.length>0&&await ve.window.showWarningMessage(`Wait! There are ${l.length} file(s) with deletion conflicts that are still on your disk.

If you commit now, you will KEEP them in the project.

Are you sure you want to KEEP them?`,{modal:!0},"Yes, KEEP them","No, let me DELETE them")!=="Yes, KEEP them"){d();return}let u=!1;try{let{stdout:f}=await g('git grep -E "^<<<<<<< "',{cwd:t});f.trim().length>0&&(u=!0)}catch{}if(u){ve.window.showErrorMessage("Ricwiz: You still have unresolved conflict markers (<<<<<<<) in your files. Please resolve them first!"),d();return}await g("git add .",{cwd:t}),await g("git commit --no-edit",{cwd:t})}catch(m){ve.window.showErrorMessage(`Ricwiz: Could not commit automatically. (${m.message})`),d()}}),d();;){if(n){p=!0,at(void 0),ne?.setConflictState(null);try{await g("git merge --abort",{cwd:t})}catch{}return!1}try{let{stdout:r}=await g("git status --porcelain",{cwd:t}),m=r.split(`
`).some(w=>{let $=w.substring(0,2);return["UU","AA","UD","DU","AU","UA","DD"].includes($)}),l=je.join(t,".git","MERGE_HEAD"),u=je.join(t,".git","REBASE_HEAD"),f=je.join(t,".git","CHERRY_PICK_HEAD");if(!(m||Oe.existsSync(l)||Oe.existsSync(u)||Oe.existsSync(f)))return p=!0,at(void 0),ne?.setConflictState(null),ve.window.showInformationMessage("Ricwiz: Changes committed!"),!0}catch{}await new Promise(r=>setTimeout(r,2e3))}}var Xe=k(require("https")),It=k(require("vscode"));async function mt(){let t=await xe();return!!(t&&t.trim())}async function Vo(t,e){let o=It.workspace.getConfiguration("ricwiz"),s=(await xe())?.trim();if(!s)throw new Error("No GitLab token");let i=e?e.getConfig("gitlabUrlOverride",""):o.get("gitlabUrlOverride",""),p=[];if(i&&i.trim()!=="")p.push(i.trim());else try{let{stdout:c}=await g("git remote",{cwd:t}),a=c.split(`
`).map(r=>r.trim()).filter(r=>r),d=[];e&&e.upstreamRemote&&a.includes(e.upstreamRemote)&&d.push(e.upstreamRemote),e&&e.originRemote&&e.originRemote!==e.upstreamRemote&&a.includes(e.originRemote)&&d.push(e.originRemote),a.includes("upstream")&&!d.includes("upstream")&&d.push("upstream"),a.includes("origin")&&!d.includes("origin")&&d.push("origin"),d.length===0&&a.length>0&&d.push(...a);for(let r of d)try{let{stdout:m}=await g(`git remote get-url ${r}`,{cwd:t}),l=m.trim();l.endsWith(".git")&&(l=l.slice(0,-4)),l.startsWith("git@")&&(l=l.replace("git@","").replace(":","/"),l=`https://${l}`),p.push(l)}catch(m){R.appendLine(`[GitLab API] Error getting remote URL for ${r}: ${m.message}`)}}catch(c){R.appendLine(`[GitLab API] Error getting remotes: ${c.message}`)}if(p.length===0)throw R.appendLine("[GitLab API] No candidate URLs found in getGitlabTargets!"),new Error("Could not get any remote origin URL.");return p.map(c=>{let a=new URL(c),d=`${a.protocol}//${a.host}`,r=a.pathname;r.startsWith("/")&&(r=r.substring(1)),r.endsWith("/")&&(r=r.slice(0,-1)),r.endsWith(".git")&&(r=r.slice(0,-4));let m=encodeURIComponent(r);return{baseUrl:d,token:s,projectPath:m}})}var _o=new Xe.Agent({keepAlive:!0,maxSockets:10});async function dt(t,e,o,s){let i=new URL(`${t}${s}`);return R.appendLine(`[GitLab API] ${o} ${i.toString()}`),new Promise((p,n)=>{let c=Xe.request(i,{method:o,timeout:5e3,agent:_o,headers:{"PRIVATE-TOKEN":e,Accept:"application/json"}},a=>{let d="";a.on("data",r=>d+=r),a.on("end",()=>{if(R.appendLine(`[GitLab API] Response Code: ${a.statusCode}`),a.statusCode&&a.statusCode>=400)return R.appendLine(`[GitLab API] Error Data: ${d}`),n(new Error(`GitLab API error: ${a.statusCode}`));if(!d)return p({});try{let r=JSON.parse(d);Array.isArray(r)?R.appendLine(`[GitLab API] Returned array with ${r.length} items`):r&&typeof r=="object"&&R.appendLine(`[GitLab API] Returned object with id ${r.id||r.iid||"unknown"}`),p(r)}catch(r){R.appendLine(`[GitLab API] Parse Error: ${r.message}`),n(r)}})});c.on("timeout",()=>{c.destroy(),n(new Error("GitLab request timed out"))}),c.on("error",a=>{R.appendLine(`[GitLab API] Request Failed: ${a.message}`),n(a)}),c.end()})}var lt=new Map,Qo=30*1e3;async function pt(t,e,o,s){R.appendLine(`[GitLab API] fetchMergeRequestStatus called for source: ${e}, target: ${o||"any"}`);let i=`${t}:${e}:${o||"any"}`,p=lt.get(i);if(p&&Date.now()-p.timestamp<Qo)return p.data;try{let n=await Vo(t,s),c=null,a=-1;for(let d of n)try{let r=`/api/v4/projects/${d.projectPath}/merge_requests?source_branch=${encodeURIComponent(e)}&order_by=updated_at&sort=desc`;o&&(r+=`&target_branch=${encodeURIComponent(o)}`);let m=await dt(d.baseUrl,d.token,"GET",r);if(m&&m.length>0){let l=m[0];try{let w=await dt(d.baseUrl,d.token,"GET",`/api/v4/projects/${d.projectPath}/merge_requests/${l.iid}`);w&&(l=w)}catch{}let u="none";if(l.head_pipeline&&l.head_pipeline.status){let w=l.head_pipeline.status;w==="success"||w==="failed"||w==="canceled"||w==="skipped"?u=w:u="running"}let f={isMerged:l.state==="merged",isOpen:l.state==="opened",pipelineStatus:u,webUrl:l.web_url,projectPath:d.projectPath,pipelineId:l.head_pipeline?l.head_pipeline.id:void 0},h=0;f.isOpen?h=2:f.isMerged&&(h=1),h>a&&(c=f,a=h)}}catch(r){R.appendLine(`[GitLab API] Error inside target loop: ${r.message}`)}if(c)return lt.set(i,{data:c,timestamp:Date.now()}),c;for(let d of n)try{let r=`/api/v4/projects/${d.projectPath}/pipelines?ref=${encodeURIComponent(e)}&order_by=updated_at&sort=desc`,m=await dt(d.baseUrl,d.token,"GET",r);if(m&&m.length>0){let l=m[0],u="none";if(l.status){let h=l.status;h==="success"||h==="failed"||h==="canceled"||h==="skipped"?u=h:u="running"}let f={isMerged:!1,isOpen:!1,pipelineStatus:u,webUrl:l.web_url,projectPath:d.projectPath,pipelineId:l.id};return lt.set(i,{data:f,timestamp:Date.now()}),f}}catch{}return null}catch(n){return R.appendLine(`[GitLab API] Failed to fetch MR status: ${n.message}`),null}}function At(t,e){return e.find(o=>t.endsWith(`-to-${o.name}`))}async function ze(t,e,o,s,i){let p=await mt(),n=e.map(async c=>{let a=At(c,s);if(p){let d=a?a.sourceBranch:void 0,r=await pt(t,c,d,i);if(r)return{name:c,isMerged:r.isMerged,pipelineStatus:r.pipelineStatus,mrUrl:r.webUrl,projectPath:r.projectPath,pipelineId:r.pipelineId}}else R.appendLine(`[GitLab API] Skipping MR check for ${c} because hasGitlabToken() is false`);return{name:c,isMerged:!1,pipelineStatus:"none"}});return await Promise.all(n)}async function Ft(t,e,o,s){let i=At(e,o);if(!i)return!1;if(await mt()){let p=await pt(t,e,i.sourceBranch,s);if(p)return p.isMerged}else R.appendLine(`[GitLab API] Skipping MR check for current branch ${e} because hasGitlabToken() is false`);return!1}async function Ut(t,e=10){try{let{stdout:o}=await g(`git log --oneline -${e} --format="%h|||%s|||%ar"`,{cwd:t});return o.split(`
`).filter(s=>s.trim()).map(s=>{let i=s.split("|||");return{hash:i[0]||"",message:i.length>=3?i.slice(1,-1).join("|||"):i[1]||"",timeAgo:i.length>=3?i[i.length-1]:""}})}catch{return[]}}async function Ot(t,e=3){try{let{stdout:o}=await g('git for-each-ref --sort=-committerdate --format="%(refname:short)" refs/heads/',{cwd:t}),s=o.split(`
...
`,w+=h[b].join(`
`)+`
`;let v=await G.workspace.openTextDocument({content:w,language:"plaintext"});await G.window.showTextDocument(v)}catch(c){G.window.showErrorMessage(`Ricwiz: Error running git log - ${c.message}`)}})}var oe=k(require("vscode"));async function xo(){let t=z();if(!t){oe.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let o=oe.workspace.getConfiguration("ricwiz").get("resetTrackingCommand","sf org disable tracking && sf project reset tracking --no-prompt");await oe.window.withProgress({location:oe.ProgressLocation.Notification,title:"Ricwiz: Resetting tracking...",cancellable:!1},async()=>{try{let{stdout:s,stderr:i}=await g(o,{cwd:t,maxBuffer:52428800}),p=oe.window.createOutputChannel("Ricwiz Reset Tracking");p.appendLine(`Executing: ${o}`),p.appendLine(s),i&&(p.appendLine("--- STDERR ---"),p.appendLine(i)),p.show(),oe.window.showInformationMessage("Ricwiz: Successfully reset tracking!")}catch(s){let i=oe.window.createOutputChannel("Ricwiz Reset Tracking");i.appendLine(`Error executing: ${o}`),s.stdout&&i.appendLine(s.stdout),s.stderr&&i.appendLine(s.stderr),i.appendLine(s.message),i.show(),oe.window.showErrorMessage("Ricwiz: Error resetting tracking. See output channel for details.")}})}var Z=k(require("vscode"));async function ko(){let t=z();if(!t){Z.window.showErrorMessage("Ricwiz: Open a workspace first.");return}let e=["ApexClass","ApexTrigger","CustomObject","CustomField","LightningComponentBundle","AuraDefinitionBundle","Flow","CustomLabel","CustomMetadata","StaticResource","Profile","PermissionSet","PermissionSetGroup","Layout","ValidationRule","RecordType","ListView","Report","EmailTemplate","Other (Type manually)..."],o=await Z.window.showQuickPick(e,{placeHolder:"Select Metadata Type to extract (e.g., ApexClass)",ignoreFocusOut:!0});if(!o||o==="Other (Type manually)..."&&(o=await Z.window.showInputBox({prompt:"Enter Metadata Type (e.g., CustomApplication, Queue)",ignoreFocusOut:!0}),!o))return;let s={ApexClass:"**/*.cls",ApexTrigger:"**/*.trigger",CustomObject:"**/*.{object,object-meta.xml}",CustomField:"**/*.field-meta.xml",LightningComponentBundle:"**/lwc/*/*.js",AuraDefinitionBundle:"**/aura/*/*.cmp",Flow:"**/*.flow-meta.xml",CustomLabel:"**/*.labels-meta.xml",CustomMetadata:"**/*.md-meta.xml",StaticResource:"**/*.resource-meta.xml",Profile:"**/*.profile-meta.xml",PermissionSet:"**/*.permissionset-meta.xml",PermissionSetGroup:"**/*.permissionsetgroup-meta.xml",Layout:"**/*.layout-meta.xml",ValidationRule:"**/*.validationRule-meta.xml",RecordType:"**/*.recordType-meta.xml",ListView:"**/*.listView-meta.xml"},i=[],p=s[o];if(p)try{i=(await Z.workspace.findFiles(p,"**/node_modules/**")).map(a=>{let d=a.fsPath.split(/[\\/]/).pop()||"";if(o==="LightningComponentBundle"||o==="AuraDefinitionBundle"){let r=a.fsPath.split(/[\\/]/);return r[r.length-2]||d.split(".")[0]}return d.split(".")[0]}),i=[...new Set(i)].sort()}catch{}let n=await new Promise(c=>{let a=Z.window.createQuickPick();a.title=`Extract ${o}`,a.placeholder="Type name (e.g. MyComponent) or * for all",a.ignoreFocusOut=!0,a.matchOnDescription=!0;let d=()=>{let r=a.value.trim(),m=[];r?m.push({label:`$(cloud-download) Extract "${r}"`,description:"Extract exact name from Salesforce",alwaysShow:!0}):m.push({label:'$(cloud-download) Extract "*" (All)',description:`Extract all ${o}s`,alwaysShow:!0}),i.forEach(l=>{(!r||l.toLowerCase().includes(r.toLowerCase()))&&m.push({label:l,description:"Local workspace component"})}),a.items=m};a.onDidChangeValue(()=>d()),a.onDidAccept(()=>{let r=a.selectedItems[0];if(r){let m=r.label;m.startsWith('$(cloud-download) Extract "')?m=m.replace('$(cloud-download) Extract "',"").replace('" (All)',"").replace('"',""):m==='$(cloud-download) Extract "*" (All)'&&(m="*"),a.hide(),c(m)}}),a.onDidHide(()=>{a.dispose(),c(void 0)}),d(),a.show()});n&&await Z.window.withProgress({location:Z.ProgressLocation.Notification,title:`Ricwiz: Extracting ${o}:${n} from Salesforce...`,cancellable:!0},async(c,a)=>{try{R.show(!0);let d=`${o}:${n}`,{stdout:r,stderr:m}=await g(`sf project retrieve start -m "${d}"`,{cwd:t});r&&R.appendLine(r),m&&R.appendLine(m),Z.window.showInformationMessage(`Ricwiz: Successfully extracted ${d}.`)}catch(d){R.appendLine(`ERROR: ${d.message}`),d.stdout&&R.appendLine(d.stdout),d.stderr&&R.appendLine(d.stderr),Z.window.showErrorMessage("Ricwiz: Extraction failed. See Output channel for details.")}})}var j=k(require("vscode")),Co=k(require("path"));async function $o(){let t=j.window.activeTextEditor;if(!t){j.window.showErrorMessage("Ricwiz: Please open the file you want to deploy in the editor first.");return}let e=t.document.uri.fsPath,o=z();if(!o)return;let s="";if(await j.window.withProgress({location:j.ProgressLocation.Notification,title:"Ricwiz: Fetching available Salesforce orgs...",cancellable:!1},async()=>{try{let{stdout:a}=await g("sf org list --json",{cwd:o});s=a}catch(a){s=a.stdout||""}}),!s){j.window.showErrorMessage("Ricwiz: Failed to fetch orgs. Is Salesforce CLI installed?");return}let i=[];try{let a=JSON.parse(s),d=a.result?.nonScratchOrgs||[],r=a.result?.scratchOrgs||[];i=[...d,...r]}catch{j.window.showErrorMessage("Ricwiz: Failed to parse org list.");return}if(i.length===0){j.window.showInformationMessage("Ricwiz: No authenticated orgs found.");return}let p=i.map(a=>({label:a.alias||a.username,description:a.alias?a.username:"",picked:a.isDefaultUsername})),n=await j.window.showQuickPick(p,{placeHolder:"Select the org(s) to deploy this file to",canPickMany:!0,ignoreFocusOut:!0});if(!n||n.length===0)return;let c=Co.basename(e);await j.window.withProgress({location:j.ProgressLocation.Notification,title:`Ricwiz: Deploying ${c} to ${n.length} org(s)...`,cancellable:!1},async()=>{R.show(!0),R.appendLine(`--- Starting Parallel Deploy of ${c} ---`);let a=n.map(async l=>{let u=l.label;R.appendLine(`[${u}] Deploying...`);try{let{stdout:f,stderr:h}=await g(`sf project deploy start -d "${e}" -o "${u}"`,{cwd:o});return R.appendLine(`[${u}] \u2705 Success`),f&&R.appendLine(f),{org:u,success:!0}}catch(f){return R.appendLine(`[${u}] \u274C Failed`),f.stdout&&R.appendLine(f.stdout),f.stderr&&R.appendLine(f.stderr),{org:u,success:!1}}}),d=await Promise.all(a),r=d.filter(l=>l.success).length,m=d.filter(l=>!l.success).length;m===0?j.window.showInformationMessage(`Ricwiz: Successfully deployed to all ${r} orgs!`):j.window.showErrorMessage(`Ricwiz: Deploy finished with errors (${r} success, ${m} failed). Check Output channel.`)})}var I=k(require("vscode")),ot=k(require("fs")),it=k(require("path"));async function Ro(){let t=z();if(!t){I.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=I.workspace.getConfiguration("ricwiz"),o=e.get("auditUsername",""),s=e.get("auditHours",8),i=await I.window.showInputBox({prompt:"Enter your Salesforce Username to query in SetupAuditTrail",value:o,placeHolder:"admin@tuaorg.com"});if(!i)return;i=ke(i);let p=await I.window.showInputBox({prompt:"How many hours back do you want to search?",value:s.toString(),placeHolder:"8"});if(!p)return;let n=parseFloat(p);if(isNaN(n)||n<=0){I.window.showErrorMessage("Ricwiz: Invalid hours specified.");return}let c=new Date(Date.now()-n*60*60*1e3).toISOString(),d=`sf data query -q "${`SELECT Action, Display, Section FROM SetupAuditTrail WHERE CreatedBy.Username = '${i}' AND CreatedDate >= ${c}`}" --json`;await I.window.withProgress({location:I.ProgressLocation.Notification,title:"Ricwiz: Interrogating Setup Audit Trail...",cancellable:!1},async()=>{try{let{stdout:r}=await g(d,{cwd:t,maxBuffer:52428800}),m=JSON.parse(r);if(!m.result||m.result.records.length===0){I.window.showInformationMessage(`Ricwiz: No changes found for ${i} in the last ${n} hours.`);return}let l=m.result.records,u=[],f=new Set;for(let B of l){let U=Yo(B.Action,B.Display,B.Section);if(U){let N=`${U.isDelete?"DEL":"ADD"}-${U.metadataFormat}`;if(!f.has(N)){f.add(N);let ce=U.isDelete?"$(trash)":"$(plus)";u.push({label:`${ce} ${U.metadataFormat}`,description:`${B.Action} -> ${B.Display}`,metadataFormat:U.metadataFormat,isDelete:U.isDelete})}}}if(u.length===0){I.window.showInformationMessage(`Ricwiz: No extractable metadata changes found for ${i} in the last ${n} hours (ignored passwords/logins).`);return}let h=await I.window.showQuickPick(u,{canPickMany:!0,placeHolder:"Select the changes you want to extract to GitLab",ignoreFocusOut:!0});if(!h||h.length===0){I.window.showInformationMessage("Ricwiz: No changes selected.");return}let w=h.filter(B=>B.isDelete),$=h.filter(B=>!B.isDelete),v=I.window.createOutputChannel("Ricwiz Admin Bridge");if(v.show(),w.length>0){let{stdout:B}=await g("git ls-files",{cwd:t}),U=B.split(`
`).map(ce=>ce.trim()),N=0;for(let ce of w){let Ue=ce.metadataFormat.split(":"),Ge=Ue[0],He=Ue[1],ye=He;Ge==="CustomField"&&(ye=He.split(".")[1]);let nt=U.filter(Le=>{let L=it.basename(Le);return L.startsWith(ye+".")&&L.includes(Ge==="CustomField"?".field":"")});for(let Le of nt){let L=it.join(t,Le);ot.existsSync(L)&&(ot.unlinkSync(L),v.appendLine(`Deleted local file: ${Le}`),N++)}}I.window.showInformationMessage(`Ricwiz: Deleted ${N} local files from Git workspace.`)}if($.length===0)return;let b=$.map(B=>B.metadataFormat).filter(B=>B!=="").join(", "),x=await I.window.showInputBox({prompt:"Review and adjust the metadata components to retrieve",value:b,ignoreFocusOut:!0});if(!x)return;let D=`sf project retrieve start -m "${x}"`;v.appendLine(`Executing: ${D}`),I.window.showInformationMessage(`Ricwiz: Extracting ${$.length} components...`);let T=await g(D,{cwd:t});v.appendLine(T.stdout),T.stderr&&(v.appendLine("--- STDERR ---"),v.appendLine(T.stderr)),I.window.showInformationMessage("Ricwiz: Changes extracted successfully! Ready for Git Commit.")}catch(r){I.window.showErrorMessage(`Ricwiz: Error capturing changes - ${r.message}`)}})}function Yo(t,e,o){if(!t||!e||!o)return null;let s=t.toLowerCase(),i=o.toLowerCase();if(["security controls","network access","session settings","data export","login history","password policies","identity verification","delegated administration"].includes(i)||s.includes("login")||s.includes("password")||s.includes("oauth")||s.includes("session"))return null;let n=s.includes("delete"),c=null;if(s==="permissionsetgroupcomponentadd"||s==="permissionsetgroupcomponentdelete")return null;let a=(d,r=!1)=>{let m=d.replace(/\(.*\)/g,"").trim();m.includes(":")&&!s.includes("calculation")&&(m=m.split(":")[0]);let l=["disabled","deleted","removed","created","changed","updated","from","to","on","assigned","assign","assignment","permission","set","group","apex","class","trigger","custom","field","object","layout","validation","rule","flow","profile"],u=m.split(/\s+/);if(r){for(;u.length>0&&l.includes(u[u.length-1].toLowerCase());)u.pop();for(;u.length>0&&l.includes(u[0].toLowerCase());)u.shift();return u.join(" ").trim().replace(/[^a-zA-Z0-9_ ]/g,"")}else return u.filter(w=>!l.includes(w.toLowerCase())).join("_").replace(/[^a-zA-Z0-9_]/g,"")||m.replace(/\s+/g,"")};if(s.includes("profile"))c=`Profile:${a(e,!0)}`;else if(s.includes("permissionsetgroupcalculation")){let d=e.split(":");c=`PermissionSetGroup:${d.length>1?d[d.length-1].trim():a(e,!1)}`}else if(s.includes("permission set group")||s.includes("permissionsetgroup"))c=`PermissionSetGroup:${a(e,!1)}`;else if(s.includes("permission set")||s.includes("permissionset"))c=`PermissionSet:${a(e,!1)}`;else if(s.includes("apexclass"))c=`ApexClass:${a(e,!1)}`;else if(s.includes("apextrigger")||s.includes("apex trigger"))c=`ApexTrigger:${a(e,!1)}`;else if(s.includes("customfield")){let d=e.match(/([A-Za-z0-9_]+__c)/),r=e.match(/(?:on|na|for)\s+([A-Za-z0-9_]+)/i);d&&r?c=`CustomField:${r[1]}.${d[1]}`:c=`CustomField:${a(e,!1)}`}else if(s.includes("layout"))c=`Layout:${a(e,!0)}`;else if(s.includes("validation"))c=`ValidationRule:${a(e,!1)}`;else if(s.includes("flow"))c=`Flow:${a(e,!1)}`;else if(s.includes("customobject")){let d=e.match(/([A-Za-z0-9_]+__c)/);c=d?`CustomObject:${d[1]}`:`CustomObject:${a(e,!1)}`}else if(!s.includes("created")&&!s.includes("changed")&&!s.includes("deleted"))return null;return c?{metadataFormat:c,isDelete:n}:null}var ft=k(require("vscode"));async function zo(){let t=z();if(t)try{let{stdout:e}=await g('git for-each-ref --sort=-committerdate --format="%(refname:short)|||%(committerdate:relative)|||%(subject)" refs/heads/',{cwd:t}),o=e.split(`
`).filter(i=>i.trim()).map(i=>{let p=i.split("|||");return{label:`$(git-branch) ${p[0]}`,description:p[1],detail:p[2],branchName:p[0]}}),s=await ft.window.showQuickPick(o,{placeHolder:"Select a branch from history to checkout",matchOnDescription:!0,matchOnDetail:!0});s&&await Fe(s.branchName)}catch{ft.window.showErrorMessage("Ricwiz: Failed to get branch history")}}var Je=k(require("vscode"));async function Po(){let t=z();if(!t)return;let e=await Je.window.showInputBox({prompt:"Enter ticket number or name (e.g., 48934)",placeHolder:"48934"});if(!e)return;let o=ke(e);try{let{stdout:s}=await g(`git branch --list "*${o}*"`,{cwd:t}),i=s.split(`
`).map(c=>c.replace("*","").trim()).filter(c=>c);if(i.length===0){Je.window.showInformationMessage(`Ricwiz: No branches found matching "${e}"`);return}let p=i.map(c=>({label:`$(git-branch) ${c}`,branchName:c})),n=await Je.window.showQuickPick(p,{placeHolder:`Select a branch for ${e}`});n&&await Fe(n.branchName)}catch{Je.window.showErrorMessage("Ricwiz: Failed to search branches")}}var De=k(require("vscode")),Bo=k(require("path"));async function So(){let t=De.window.activeTextEditor;if(!t)return De.window.showErrorMessage("Ricwiz: Please open a file in the editor to check blame."),null;let e=t.document.fileName,o=Bo.basename(e),s=z();if(!s)return De.window.showErrorMessage("Ricwiz: Workspace is not a git repository."),null;let i=[];try{let{stdout:r}=await g(`git log -5 --pretty=format:"%an|%ar|%s|%h" -- "${e}"`,{cwd:s}),m=r.trim().split(`
`);for(let l of m){let u=l.split("|");u.length>=4&&i.push({author:u[0],time:u[1],message:u.slice(2,-1).join("|"),hash:u[u.length-1]})}}catch(r){R.appendLine(`[WhoToBlame] Git blame error: ${r.message}`)}let p="Unknown",n="Unknown",c="Unknown",a=[],d=Ke(e);if(d)try{await De.window.withProgress({location:De.ProgressLocation.Notification,title:`Ricwiz: Analyzing ${d.name} in Salesforce...`,cancellable:!1},async()=>{let r="";if(d.type==="CustomField"){let m=d.name.split(".");m.length===2&&(r=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM CustomField WHERE DeveloperName = '${m[1].replace("__c","")}' AND TableEnumOrId = '${m[0]}'`)}else d.type==="LightningComponentBundle"?r=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM LightningComponentBundle WHERE DeveloperName = '${d.name}'`:r=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM ${d.type} WHERE Name = '${d.name}'`;if(r)try{let{stdout:m}=await g(`sf data query -t -q "${r}" --json`,{cwd:s,maxBuffer:52428800}),l=JSON.parse(m);if(l&&l.result&&l.result.records&&l.result.records.length>0){let u=l.result.records[0];p=u.LastModifiedBy?u.LastModifiedBy.Name:"Unknown",c=u.CreatedBy?u.CreatedBy.Name:"Unknown",n=new Date(u.LastModifiedDate).toLocaleString()}else p="Not found in Org",n="N/A",c="N/A"}catch(m){p="Query Error",n="N/A",c="N/A",R.appendLine(`[WhoToBlame] Query error: ${m.message}`)}try{let m="SELECT Action, Display, CreatedBy.Name, CreatedDate FROM SetupAuditTrail ORDER BY CreatedDate DESC LIMIT 1500",{stdout:l}=await g(`sf data query -q "${m}" --json`,{cwd:s,maxBuffer:52428800}),u=JSON.parse(l);if(u&&u.result&&u.result.records){let f=d.name.replace("__c","");a=u.result.records.filter(w=>w.Display&&w.Display.includes(f)).map(w=>({action:w.Action,display:w.Display,author:w.CreatedBy?w.CreatedBy.Name:"Unknown",time:new Date(w.CreatedDate).toLocaleString()})).slice(0,10)}}catch(m){R.appendLine(`[WhoToBlame] Audit trail query error: ${m.message}`)}})}catch(r){R.appendLine(`[WhoToBlame] Salesforce query error: ${r.message}`)}else p="Unsupported Metadata Type",n="N/A";return{fileName:o,gitHistory:i,sfAuthor:p,sfTime:n,sfCreatedBy:c,auditHistory:a}}var fe=k(require("vscode"));var We=k(require("https"));async function Do(t,e){let o=z();if(!o)return;let s=(await xe())?.trim();if(!s){fe.window.showErrorMessage("Ricwiz: GitLab token is not configured.");return}try{let i=await S.initialize(o,{skipPrompt:!0});if(!i)return;let n=i.getConfig("gitlabUrlOverride","");if(n){let c=new URL(n);n=`${c.protocol}//${c.host}`}else{let{stdout:c}=await g("git remote",{cwd:o}),a=c.split(`
`).map(r=>r.trim()).filter(r=>r),d=!1;for(let r of a){let{stdout:m}=await g(`git remote get-url ${r}`,{cwd:o}),l=m.trim();l.endsWith(".git")&&(l=l.slice(0,-4)),l.startsWith("git@")&&(l=l.replace("git@","").replace(":","/"),l=`https://${l}`);let u=new URL(l),f=u.pathname;if(f.startsWith("/")&&(f=f.substring(1)),f.endsWith("/")&&(f=f.slice(0,-1)),encodeURIComponent(f)===t||f===t){n=`${u.protocol}//${u.host}`,d=!0;break}}if(!d){fe.window.showErrorMessage(`Ricwiz: Could not determine base URL for project ${t}`);return}}await fe.window.withProgress({location:fe.ProgressLocation.Notification,title:`Fetching failed jobs for Pipeline #${e}...`,cancellable:!1},async()=>{let c=new We.Agent({keepAlive:!0}),a=new URL(`${n}/api/v4/projects/${t}/pipelines/${e}/jobs?scope[]=failed`),d=await new Promise(h=>{We.get(a,{headers:{"PRIVATE-TOKEN":s},agent:c},w=>{let $="";w.on("data",v=>$+=v),w.on("end",()=>{if(w.statusCode===200)try{h(JSON.parse($))}catch{h([])}else h([])})}).on("error",()=>h([]))});if(!d||d.length===0){fe.window.showInformationMessage("Ricwiz: No failed jobs found for this pipeline.");return}let r=d[0],m=new URL(`${n}/api/v4/projects/${t}/jobs/${r.id}/trace`),u=(await new Promise(h=>{We.get(m,{headers:{"PRIVATE-TOKEN":s},agent:c},w=>{let $="";w.on("data",v=>$+=v),w.on("end",()=>h($))}).on("error",w=>h(`Failed to fetch log: ${w.message}`))})).replace(/\x1B\[[0-9;]*[mK]/g,""),f=fe.window.createOutputChannel(`Pipeline #${e} - Job ${r.name}`);f.appendLine(`Pipeline ID: ${e}`),f.appendLine(`Job Name: ${r.name}`),f.appendLine(`Status: ${r.status}`),f.appendLine(`URL: ${r.web_url}`),f.appendLine("========================================"),f.appendLine(u),f.show()})}catch(i){fe.window.showErrorMessage(`Ricwiz: Error fetching pipeline logs - ${i.message}`)}}function Eo(t,e,o){t.subscriptions.push(y.commands.registerCommand("ricwiz.conflictAction",Lt),y.commands.registerCommand("ricwiz.generateDestructiveChanges",async()=>{try{await Et()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.runSmartTests",async()=>{try{await Tt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.refreshWebview",()=>{e&&y.commands.executeCommand("workbench.action.webview.reloadWebviewAction")}),y.commands.registerCommand("ricwiz.createBranches",async s=>{try{await Mt(s)}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.prepareDeploy",async()=>{try{await jt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.createMergeRequests",async()=>{try{await Jt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.createMergeRequestsVSCode",async()=>{try{await Wt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.openJiraTicket",async()=>{try{await Ht()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.openJiraTicketVSCode",async()=>{try{await qt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.showJiraDetails",()=>{e&&oo(e)}),y.commands.registerCommand("ricwiz.openJiraDashboard",s=>{e&&io(e,s)}),y.commands.registerCommand("ricwiz.openJiraDetailsForId",s=>{e&&ro(e,s)}),y.commands.registerCommand("ricwiz.toggleDashboardBranches",s=>{e&&(e.setDashboardShowBranches(s),y.commands.executeCommand("ricwiz.openJiraDashboard"))}),y.commands.registerCommand("ricwiz.changeJiraStatus",async()=>{try{await no()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.addJiraComment",async()=>{try{await so()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.addJiraLabel",async()=>{try{await ao()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.setJiraToken",co),y.commands.registerCommand("ricwiz.setGitlabToken",mo),y.commands.registerCommand("ricwiz.syncAll",async()=>{try{await po()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.updateBases",async()=>{try{await uo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.deleteUnusedBranches",async()=>{try{await go()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.checkoutBranch",async s=>{try{await Fe(s)}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.copyBranchName",async()=>{try{await fo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.generatePackageXml",async()=>{try{await wo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.deployPackage",async()=>{try{await vo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.importData",async()=>{try{await yo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.listTicketFiles",async()=>{try{await bo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.resetTracking",async()=>{try{await xo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.extractComponent",async()=>{try{await ko()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.deployMultiOrg",async()=>{try{await $o()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.captureAdminChanges",async()=>{try{await Ro()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.openHistory",async()=>{try{await zo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.searchTicket",async()=>{try{await Po()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.whoToBlame",async()=>{let s=await So();s&&e&&(e.setBlameData(s),e.setPage("blame"))}),y.commands.registerCommand("ricwiz.showPipelineLogs",(s,i)=>Do(s,i)),y.commands.registerCommand("ricwiz.manualRefresh",()=>{o&&o()}),y.commands.registerCommand("ricwiz.toggleAutoRefresh",()=>{if(e){let s=!e.isAutoRefreshEnabled();e.setAutoRefresh(s),y.workspace.getConfiguration("ricwiz").update("autoRefresh",s,y.ConfigurationTarget.Global)}}),y.commands.registerCommand("ricwiz.openSettings",()=>{y.commands.executeCommand("workbench.action.openSettings","ricwiz")}))}var ae=k(require("vscode"));function To(t,e,o){let s,i=ae.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(i),t.subscriptions.push(ae.workspace.onDidChangeConfiguration(n=>{if(n.affectsConfiguration("ricwiz.autoRefresh")){let c=ae.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(c)}}));async function p(){let n=ae.extensions.getExtension("vscode.git");if(n){let d=function(r){let m="",l;async function u(){let h=ae.workspace.workspaceFolders;if(!h)return;let w=h[0].uri.fsPath,$=await A(w);if($&&$!==m){m=$;let v=ae.workspace.getConfiguration("ricwiz"),b=v.get("ticketPrefix","SFPSCA-");if(!$.includes(b)){let L=$.match(/([A-Z]+-)\d+/i);L&&(b=L[1].toUpperCase())}let x=[],D=[],T=[],B=[],U=await S.initialize(w,{skipPrompt:!0}),N=U?.environments||v.get("environments",[{name:"Qual",sourceBranch:"quality"},{name:"Val",sourceBranch:"validation"},{name:"Prod",sourceBranch:"main"}]);try{let L=v.get("workspaceCheckoutButtons",["main","quality","validation"]);T=Array.from(new Set(L))}catch{}let ce="",Ue=$.match(new RegExp(`(${b}\\d+(?:-\\d+)?)`,"i"));if(Ue){let L=Ue[1].toUpperCase();ce=L;let he=v.get("commitMessageSuffix","- "),ht=/^[A-Z]+-\d+(?:-\d+)?\s*(?:-\s*|:\s*|\s+)?/i;ht.test(r.inputBox.value)?r.inputBox.value.toUpperCase().startsWith(L)||(r.inputBox.value=r.inputBox.value.replace(ht,`${L}${he}`)):r.inputBox.value=`${L}${he}`+r.inputBox.value,o.text=`$(bookmark) ${L}`,o.tooltip=`Branch: ${$}
Click to open Jira ticket`,o.show();try{let wt=await Pe(w,L,"");x=await ze(w,wt,L,N,U)}catch{}}else{o.hide();try{B=await Ot(w)}catch{}}let[Ge,He,ye]=await Promise.all([Ut(w,10),Ft(w,$,N,U),ce?Ae(ce).catch(L=>{let he=L.message;return he&&(he.includes("ENOTFOUND")||he.includes("network"))&&(he="No Internet or Invalid URL"),{summary:`\u26A0\uFE0F Jira Error: ${he}`,description:"",status:""}}):Promise.resolve(null)]);D=Ge;let nt=ye?ye.summary:"",Le=ye&&ye.status||"";e?.updateBranch($,He,x,D,T,B,nt,Le)}}function f(){e?.isAutoRefreshEnabled()&&(l&&clearTimeout(l),l=setTimeout(()=>{m="",u()},300))}s=()=>{m="",u()},u(),t.subscriptions.push(r.state.onDidChange(()=>f())),t.subscriptions.push(ae.window.onDidChangeWindowState(h=>{h.focused&&f()}))};var c=d;n.isActive||await n.activate();let a=n.exports.getAPI(1);a.repositories.length>0&&a.repositories.forEach(r=>d(r)),a.onDidOpenRepository(r=>d(r))}}return p(),()=>{s&&s()}}var Mo={get_tickets_batch:async t=>{let e=await et(t);return JSON.stringify(e)}};var Lo=k(require("http")),Te=k(require("fs")),rt=k(require("path")),Io=k(require("os"));var Ee;function Ao(){Ee||(Ee=Lo.createServer(async(t,e)=>{try{let o=new URL(t.url||"",`http://${t.headers.host}`);if(o.pathname==="/tickets_batch"&&t.method==="GET"){let s=o.searchParams.get("ids");if(!s)return e.writeHead(400),e.end(JSON.stringify({error:"Missing ids parameter"}));let i=s.split(",").map(n=>n.trim()).filter(Boolean),p=await et(i);return e.writeHead(200,{"Content-Type":"application/json"}),e.end(JSON.stringify(p))}e.writeHead(404),e.end()}catch(o){e.writeHead(500),e.end(JSON.stringify({error:o.message||"Internal error"}))}}),Ee.listen(0,"127.0.0.1",()=>{let t=Ee?.address();t&&typeof t!="string"&&Ko(t.port)}))}function Ko(t){let e=rt.join(Io.homedir(),".ricwiz");Te.existsSync(e)||Te.mkdirSync(e,{recursive:!0}),Te.writeFileSync(rt.join(e,"port.txt"),t.toString(),"utf8"),Te.writeFileSync(rt.join(e,"jira-cli.js"),`
...
req.end();
`.trim(),"utf8")}function Fo(){Ee&&(Ee.close(),Ee=void 0)}var ne;function Zo(t){Rt(t),Ao(),ne=new _e(t.extensionUri),t.subscriptions.push(Me.window.registerWebviewViewProvider("ricwiz-webview",ne));let e=Me.window.createStatusBarItem(Me.StatusBarAlignment.Left,100);e.command="ricwiz.openJiraTicket",t.subscriptions.push(e);let o=To(t,ne,e);return Eo(t,ne,o),{getJiraCredentials:async()=>({email:Me.workspace.getConfiguration("ricwiz").get("jiraEmail",""),token:await Qe()}),getGitLabToken:async()=>xe(),AiSkills:Mo}}function Xo(){Fo()}0&&(module.exports={activate,deactivate,webviewProvider});
