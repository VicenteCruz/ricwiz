...
async function generatePackageXml() {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            let diffOutput = '';
            try {
                const { stdout: mergeBase } = await (0, git_1.exec)(`git merge-base origin/${sourceBranch} HEAD`, { cwd });
                const baseCommit = mergeBase.trim();
                const { stdout } = await (0, git_1.exec)(`git diff --name-only ${baseCommit}..HEAD`, { cwd });
                diffOutput = stdout;
            }
            catch (e) {
                // Fallback to local source branch
                try {
                    const { stdout: mergeBase } = await (0, git_1.exec)(`git merge-base ${sourceBranch} HEAD`, { cwd });
                    const baseCommit = mergeBase.trim();
                    const { stdout } = await (0, git_1.exec)(`git diff --name-only ${baseCommit}..HEAD`, { cwd });
                    diffOutput = stdout;
                }
