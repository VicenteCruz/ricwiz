...
async function deleteUnusedBranches() {
    ...
    let currentBranch = await (0, git_1.getCurrentBranch)(cwd);
    ...
    await vscode.window.withProgress({
        ...
        let localBranches = [];
        try {
            const { stdout } = await (0, git_1.exec)(`git branch --format="%(refname:short)"`, { cwd });
        ...
        try {
            const { stdout } = await (0, git_1.exec)(`git branch -r --format="%(refname:short)"`, { cwd });
            remoteBranchNames = stdout.split('\n')
                .map((b) => b.trim().replace(/^origin\//, '').replace(/^[^\/]+\//, '')) // handle origin/ and other remotes
        ...
        let goneBranches = [];
        try {
            const { stdout } = await (0, git_1.exec)(`git for-each-ref --format="%(refname:short)|||%(upstream:track)" refs/heads/`, { cwd });
        ...
        const items = orphanedBranches.map(name => {
            ...
            if (isGone)
                description = 'Deleted on remote [gone]';
            if (isCurrent)
                description += ' (Current branch - will checkout main first)';
            return {
                label: name,
                description,
