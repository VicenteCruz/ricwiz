...
async function createBranches(prefilledTicket) {
  ...
  if (environments.length > 0) {
    const selectedOption = await vscode6.window.showQuickPick(quickPickOptions, {
  ...
  for (const env5 of environments) {
    if (!Security.isValidShellArg(env5.name)) {
      vscode6.window.showErrorMessage(`Invalid format for environment name in settings: ${env5.name}`);
      return;
    }
    if (!Security.isValidShellArg(env5.sourceBranch)) {
      vscode6.window.showErrorMessage(`Invalid format for environment sourceBranch in settings: ${env5.sourceBranch}`);
      return;
    }
  }
  try {
...
async function prepareDeploy() {
  ...
  try {
    const { stdout } = await exec2(`git config branch.${ticketId}.ricwiz-reviewers`, { cwd });
  ...
  if (defaultReviewers.trim()) {
    ...
    try {
      if (reviewerInput.trim()) {
        await exec2(`git config branch.${ticketId}.ricwiz-reviewers "${reviewerInput.trim()}"`, { cwd });
      } else if (currentSavedReviewers) {
        await exec2(`git config --unset branch.${ticketId}.ricwiz-reviewers`, { cwd });
      }
    } catch (e) {
    }
  }
  await vscode9.window.withProgress({
    ...
    try {
      ...
      for (const env5 of environments) {
        try {
          ...
          progress.report({ message: `Fetching ${env5.sourceBranch}...`, increment: envSyncStep });
          const fetchRemote = ctx.getFetchRemote(env5.sourceBranch);
          const fetchBranch = ctx.getFetchBranch(env5.sourceBranch);
          await exec2(`git fetch ${fetchRemote} ${fetchBranch}:${fetchBranch}`, { cwd });
        } catch (e) {
        }
      }
    } catch (e) {
    ...
    for (const env5 of environments) {
      ...
      try {
        ...
        const performSafeMerge = async (branchToMerge) => {
          try {
...
async function doCreateMergeRequests(openInVSCode = false) {
  ...
  vscode10.window.showInformationMessage(`Ricwiz: Opening ${openInVSCode ? "VS Code browser" : "external browser"} for Merge Requests!`);
}
