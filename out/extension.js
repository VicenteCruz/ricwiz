...
async function runGeminiCLI(prompt, workspacePath, channel) {
  return new Promise((resolve, reject) => {
    const child = cp2.spawn("gemini", ["-y", "-p", prompt, "--output-format", "text"], {
      cwd: workspacePath,
      shell: true
    });
    let stdout = "";
...
async function generateCommitMessage() {
  ...
  try {
    const diff = await new Promise((resolve, reject) => {
      cp2.exec("git diff --cached", { cwd, maxBuffer: 1024 * 1024 * 10 }, (err, stdout, stderr) => {
...
async function askCodeContext(question) {
  if (!question) {
    question = await vscode36.window.showInputBox({
  ...
  try {
    await vscode36.window.withProgress({
      ...
      if (editor) {
        const filePath = vscode36.workspace.asRelativePath(editor.document.uri);
        const selection = editor.selection;
        const start = selection.start.line + 1;
        const end = selection.end.line + 1;
        fileContext = `The user is currently looking at file '${filePath}' between lines ${start} and ${end}.`;
        try {
          const blameOutput = cp2.execSync(`git blame -w -C -C -L ${start},${end} "${filePath}"`, { cwd, encoding: "utf8" });
...
Output your final answer directly in English.`;
      const channel = vscode36.window.createOutputChannel("Ricwiz AI: Code Context");
      channel.clear();
      channel.show(true);
    ...
    vscode36.window.showErrorMessage("Failed to fetch code context: " + e.message);
  }
}
...
function registerAllCommands(context, webviewProvider2, forceUpdate) {
  context.subscriptions.push(
    ...
    vscode37.commands.registerCommand("ricwiz.codeContext", async (question) => {
