...
exports.generateDestructiveChanges = generateDestructiveChanges;
...
async function generateDestructiveChanges() {
    ...
    await vscode.window.withProgress({
        ...
        try {
            const { stdout } = await (0, git_1.exec)(`git diff --name-only --diff-filter=D origin/${sourceBranch}...HEAD`, { cwd });
            ...
            const addMeta = (type, name) => {
                if (!metadataMap[type])
                    metadataMap[type] = [];
                if (!metadataMap[type].includes(name))
                    metadataMap[type].push(name);
            };
...
//# sourceMappingURL=generateDestructiveChanges.js.map