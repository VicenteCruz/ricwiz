...
</Package>`;let m=Ne.join(t,"destructiveChanges");ge.existsSync(m)||ge.mkdirSync(m);let l=Ne.join(m,"destructiveChanges.xml"),o=Ne.join(m,"package.xml");ge.writeFileSync(l,p,"utf8"),ge.existsSync(o)||ge.writeFileSync(o,`<?xml version="1.0" encoding="UTF-8"?>
<Package xmlns="http://soap.sforce.com/2006/04/metadata">
    <version>58.0</version>
</Package>`,"utf8");let s=await U.workspace.openTextDocument(l);await U.window.showTextDocument(s),U.window.showInformationMessage("Ricwiz: destructiveChanges.xml generated successfully!")}catch(n){U.window.showErrorMessage(`Ricwiz: Error generating destructive changes: ${n.message}`)}})}var U,Ne,ge,pt=k(()=>{"use strict";U=v(require("vscode")),Ne=v(require("path")),ge=v(require("fs"));$()});async function gt(){let t=b();if(!t)return;let i=te.workspace.getConfiguration("ricwiz").get("ticketSourceBranch","main");await te.window.withProgress({location:te.ProgressLocation.Notification,title:"Ricwiz: Finding Apex Tests to run...",cancellable:!1},async()=>{try{let{stdout:n}=await f(`git diff --name-status origin/${i}...HEAD`,{cwd:t}),r=n.split(`
`).map(c=>c.trim()).filter(c=>c.length>0),u=new Set,a=new Set;for(let c of r){let d=c.split(/\s+/);if(d[0].startsWith("D"))continue;let g=d[1];if(g&&g.endsWith(".cls")){let w=g.match(/\/classes\/([^/.]+)\.cls/);if(w){let h=w[1];h.toLowerCase().endsWith("test")?u.add(h):a.add(h)}}}for(let c of a)u.add(`${c}Test`);if(u.size===0){te.window.showInformationMessage("Ricwiz: No Apex Classes were modified in this branch.");return}let p=Array.from(u).map(c=>({label:`$(beaker) ${c}`,description:"Apex Test Class"})),m=await te.window.showQuickPick(p,{canPickMany:!0,title:"Select Test Classes to Run",placeHolder:"Select tests..."});if(!m||m.length===0)return;let o=`sf apex run test -n ${m.map(c=>c.label.replace("$(beaker) ","").trim()).join(",")} -r human -w 30`,s=te.window.createTerminal("Ricwiz: Smart Tests");s.show(),s.sendText(o)}catch(n){te.window.showErrorMessage(`Ricwiz: Error finding tests: ${n.message}`)}})}var te,ft=k(()=>{"use strict";te=v(require("vscode"));$()});var he,wt=k(()=>{"use strict";he=class{static isValidShellArg(e){return e?/^[a-zA-Z0-9\-_/.]+$/.test(e):!1}}});var Pe,ht,Ue,A,re=k(()=>{"use strict";Pe=v(require("vscode")),ht=v(require("path")),Ue=v(require("fs")),A=class t{style;upstreamRemote;originRemote;ticketSourceBranch;ticketPrefix;environments;static baseConfig=Pe.workspace.getConfiguration("ricwiz");activeProfile;profileName;constructor(e){this.activeProfile=e,this.profileName=e?.name;let i=t.baseConfig;this.style=e?.workflowStyle||i.get("workflowStyle","standard"),this.style==="multi-remote"?(this.upstreamRemote=e?.upstreamRemote||i.get("upstreamRemote","salesforce-master"),this.originRemote=e?.originRemote||i.get("originRemote","origin")):(this.upstreamRemote="origin",this.originRemote="origin"),this.ticketSourceBranch=e?.ticketSourceBranch||i.get("ticketSourceBranch","main"),this.ticketPrefix=e?.ticketPrefix||i.get("ticketPrefix","SFPSCA-");let n=[{name:"Qual",sourceBranch:"quality"},{name:"Val",sourceBranch:"validation"},{name:"Prod",sourceBranch:"main"}];this.environments=e?.environments||i.get("environments",n)}getConfig(e,i){return this.activeProfile&&this.activeProfile[e]!==void 0?this.activeProfile[e]:t.baseConfig.get(e,i)}static async initialize(e,i){let n=t.baseConfig.get("profiles",[]),r=ht.join(e,"ricwiz.json");if(Ue.existsSync(r))try{let u=Ue.readFileSync(r,"utf-8"),a=JSON.parse(u);a&&Array.isArray(a.profiles)&&(n=[...n,...a.profiles])}catch(u){Pe.window.showErrorMessage(`Ricwiz: Error parsing ricwiz.json: ${u.message}`)}if(n.length>0){if(!i?.forcePrompt)try{let{exec:m}=($(),ue(Se)),{stdout:l}=await m("git branch --show-current",{cwd:e}),o=l.trim(),s=o;o.includes("-to-")&&(s=o.split("-to-")[0]);let{stdout:c}=await m(`git config branch.${s}.ricwiz-profile`,{cwd:e}),d=c.trim();if(d){let g=n.find(w=>w.name===d);if(g)return new t(g)}}catch{}let u=n.map(m=>m.name),a=await Pe.window.showQuickPick(u,{placeHolder:"Ricwiz: Select Workflow Profile",ignoreFocusOut:!0});if(!a)return;let p=n.find(m=>m.name===a);return new t(p)}return new t}buildUpstreamPath(e){return e.includes("/")?e:`${this.upstreamRemote}/${e}`}getFetchRemote(e){return e.includes("/")?e.split("/")[0]:this.upstreamRemote}getFetchBranch(e){return e.includes("/")?e.substring(e.indexOf("/")+1):e}}});async function vt(t){let e=b();if(!e){B.window.showErrorMessage("Open a folder or workspace that is a Git repository.");return}let i=await A.initialize(e,{forcePrompt:!0});if(!i)return;let n=typeof t=="string"?t:void 0,r=await N(e,{prefix:i.ticketPrefix,suggestedValue:n});if(!r){B.window.showErrorMessage("Branch creation cancelled: Ticket not provided.");return}let{ticketId:u}=r,a=i.environments,p="all",m=[{label:"Create Main Branch & Environments",description:"Creates the main ticket branch and all environment branches",value:"all"},{label:"Create Main Branch Only",description:"Creates only the main ticket branch (skips environments)",value:"mainOnly"},{label:"Create Environments Only",description:"Creates only the environment branches (skip main branch)",value:"envs"}];if(a.length>0){let s=await B.window.showQuickPick(m,{placeHolder:"Ricwiz: What do you want to create?",ignoreFocusOut:!0});if(!s)return;p=s.value}let l=i.ticketSourceBranch;if(p==="all"||p==="mainOnly"){let s=[];try{let{stdout:w}=await f('git branch --all --format="%(refname:short)"',{cwd:e});s=w.split(`
`).map(h=>h.trim()).filter(h=>h&&h!=="origin"),s=[...new Set(s)]}catch{}let c=B.window.createQuickPick();c.title="Ricwiz: Ticket Source Branch",c.placeholder="Confirm or change the source branch for this ticket",c.value=i.ticketSourceBranch,c.ignoreFocusOut=!0;let d=()=>{let w=c.value.trim(),h=[];w&&h.push({label:w,description:"Use typed branch"}),h.push(...s.map(R=>({label:R}))),c.items=h};c.onDidChangeValue(d),d();let g=await new Promise(w=>{c.onDidAccept(()=>{let h=c.selectedItems[0];w(h?h.label:c.value),c.hide()}),c.onDidHide(()=>w(void 0)),c.show()});if(!g){B.window.showInformationMessage("Branch creation cancelled.");return}l=g.trim()}let o=u;if(!he.isValidShellArg(o)){B.window.showErrorMessage(`Invalid format for ticket ID: ${o}`);return}if(!he.isValidShellArg(l)){B.window.showErrorMessage(`Invalid format for ticketSourceBranch in settings: ${l}`);return}for(let s of a){if(!he.isValidShellArg(s.name)){B.window.showErrorMessage(`Invalid format for environment name in settings: ${s.name}`);return}if(!he.isValidShellArg(s.sourceBranch)){B.window.showErrorMessage(`Invalid format for environment sourceBranch in settings: ${s.sourceBranch}`);return}}try{await f("git status",{cwd:e})}catch{B.window.showErrorMessage("The opened folder does not appear to be a valid Git repository.");return}try{await B.window.withProgress({location:B.ProgressLocation.Notification,title:"Ricwiz: Creating Branches",cancellable:!1},async s=>{let c=[];s.report({message:"Checking remote status (git fetch)...",increment:10});try{await f("git fetch",{cwd:e})}catch{}try{if(p==="all"||p==="mainOnly"){if(s.report({message:`Creating main branch ${o}...`,increment:10}),await pe(e,o))B.window.showInformationMessage(`Ricwiz: The branch ${o} already exists. Skipping creation...`),await f(`git checkout ${o}`,{cwd:e});else try{let d=i.getFetchRemote(l),g=i.getFetchBranch(l),w=i.buildUpstreamPath(l);await f(`git fetch ${d} ${g}`,{cwd:e}),await f(`git checkout -b ${o} ${w}`,{cwd:e}),c.push(o)}catch{try{await f(`git checkout -b ${o} ${l}`,{cwd:e}),c.push(o)}catch{throw new Error(`Could not create main branch '${o}' from '${l}'. Does the source branch exist?`)}}try{await f(`git config branch.${o}.ricwiz-source "${l}"`,{cwd:e}),i.profileName&&await f(`git config branch.${o}.ricwiz-profile "${i.profileName}"`,{cwd:e})}catch{}}if(p==="all"||p==="envs"){let d=50/(a.length||1);for(let g of a){let w=`${u}-to-${g.name}`,h=g.sourceBranch;if(s.report({message:`Processing environment branch ${w}...`,increment:d}),!await pe(e,w))try{let R=i.buildUpstreamPath(h);await f(`git checkout -b ${w} ${R}`,{cwd:e}),c.push(w)}catch{try{await f(`git checkout -b ${w} ${h}`,{cwd:e}),c.push(w)}catch{throw new Error(`Could not create environment branch '${w}' from '${h}'. Does the source branch exist?`)}}}}s.report({message:`Publishing branches to ${i.originRemote}...`,increment:10});for(let d of c)try{await f(`git push -u ${i.originRemote} ${d}`,{cwd:e})}catch{B.window.showWarningMessage(`Ricwiz: Branch ${d} was created locally but could not be pushed to ${i.originRemote}.`)}if(p==="all"||p==="mainOnly"){s.report({message:`Switching to ${o}...`,increment:10});try{await f(`git checkout ${o}`,{cwd:e})}catch{}}s.report({increment:100}),B.window.showInformationMessage("Ricwiz: All set! You can start working on your branches! \u{1F680}")}catch(d){if(B.window.showErrorMessage(`Ricwiz Branch Creation Failed: ${d.message}`),c.length>0){try{await f(`git checkout ${l}`,{cwd:e})}catch{}for(let g of c)try{await f(`git branch -D ${g}`,{cwd:e})}catch{}B.window.showWarningMessage(`Ricwiz: Rolled back (deleted) ${c.length} branch(es) locally due to failure.`)}}})}catch(s){B.window.showErrorMessage(`Ricwiz general error: ${s.message}`)}}var B,bt=k(()=>{"use strict";B=v(require("vscode"));$();wt();re()});async function ve(t,e,i,n){n&&n.report({message:"CONFLICT! Resolve & click 'Commit & Continue' in Ricwiz panel."});let r=!1,u=!1,a=async()=>{try{let{stdout:o}=await f("git status --porcelain",{cwd:t});return o.split(`
...
`).map(c=>c.trimRight()).filter(c=>c.length>2).filter(c=>{let d=c.substring(0,2);return["UU","AA","UD","DU","AU","UA","DD"].includes(d)}).map(c=>{let d=c.substring(0,2);return{file:c.substring(3).trim(),state:s(d)}})}catch{return[]}},m=async()=>{if(r)return;let o=await a(),s=await p(),{webviewProvider:c}=(Je(),ue(je));c&&c.setConflictState({isConflict:!0,sourceStr:e,targetStr:i,deletionsCount:o.length,files:s})},l=oe.commands.registerCommand("ricwiz.conflictAction",async o=>{if(o==="abortDeploy")u=!0;else if(o==="resolveDeletions"){try{let c=(await a()).map(g=>({label:g})),d=await oe.window.showQuickPick(c,{canPickMany:!0,placeHolder:"Select conflicted files to DELETE",title:"Ricwiz: Delete Conflicted Files"});if(d&&d.length>0){for(let g of d)try{await f(`git rm --force "${g.label}"`,{cwd:t})}catch{}oe.window.showInformationMessage(`Ricwiz: Deleted ${d.length} conflicted file(s).`)}}catch(s){oe.window.showErrorMessage(`Ricwiz: Error. (${s.message})`)}m()}else if(o==="commitAndContinue")try{let c=(await a()).filter(g=>yt.existsSync(xt.join(t,g)));if(c.length>0&&await oe.window.showWarningMessage(`Wait! There are ${c.length} file(s) with deletion conflicts that are still on your disk.

If you commit now, you will KEEP them in the project.

Are you sure you want to KEEP them?`,{modal:!0},"Yes, KEEP them","No, let me DELETE them")!=="Yes, KEEP them"){m();return}let d=!1;try{let{stdout:g}=await f('git grep -E "^<<<<<<< "',{cwd:t});g.trim().length>0&&(d=!0)}catch{}if(d){oe.window.showErrorMessage("Ricwiz: You still have unresolved conflict markers (<<<<<<<) in your files. Please resolve them first!"),m();return}await f("git add .",{cwd:t}),await f("git commit --no-edit",{cwd:t})}catch(s){oe.window.showErrorMessage(`Ricwiz: Could not commit automatically. (${s.message})`),m()}});for(m();;){if(u){r=!0,l.dispose(),(Je(),ue(je)).webviewProvider?.setConflictState(null);try{await f("git merge --abort",{cwd:t})}catch{}return!1}try{let{stdout:o}=await f("git status --porcelain",{cwd:t});if(o.trim().length===0)return r=!0,l.dispose(),(Je(),ue(je)).webviewProvider?.setConflictState(null),oe.window.showInformationMessage("Ricwiz: Changes committed!"),!0}catch{}await new Promise(o=>setTimeout(o,2e3))}}var oe,yt,xt,We=k(()=>{"use strict";oe=v(require("vscode")),yt=v(require("fs")),xt=v(require("path"));$()});async function kt(){let t=b();if(!t){F.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}try{await f("git status",{cwd:t})}catch{F.window.showErrorMessage("Ricwiz: The opened folder does not appear to be a valid Git repository.");return}let e=await A.initialize(t);if(!e)return;let i=e.environments,n=await N(t,{prefix:e.ticketPrefix});if(!n){F.window.showErrorMessage("Operation cancelled: Ticket not provided.");return}let{ticketId:r,currentBranch:u}=n,a=r;if(!await pe(t,a)){F.window.showErrorMessage(`Ricwiz: Main branch '${a}' does not exist! Prepare deploy is only available when a main ticket branch is used.`);return}let p=e.getConfig("defaultReviewers",""),m="";try{let{stdout:l}=await f(`git config branch.${r}.ricwiz-reviewers`,{cwd:t});m=l.trim()}catch{}if(p.trim()){let l=await F.window.showInputBox({prompt:"Ricwiz: Reviewers for this deploy (optional, comma-separated)",placeHolder:"e.g. @joao, 123456",value:m||p,ignoreFocusOut:!0});if(l===void 0)return;try{l.trim()?await f(`git config branch.${r}.ricwiz-reviewers "${l.trim()}"`,{cwd:t}):m&&await f(`git config --unset branch.${r}.ricwiz-reviewers`,{cwd:t})}catch{}}await F.window.withProgress({location:F.ProgressLocation.Notification,title:"Ricwiz: Preparing Deploy",cancellable:!0},async(l,o)=>{let s=0,c=u,d=!1;o.onCancellationRequested(()=>{d=!0}),l.report({message:"Syncing remote information...",increment:10});try{await f("git fetch --all",{cwd:t});let w=10/(i.length||1);for(let h of i)try{if(d)throw new Error("Aborted");l.report({message:`Fetching ${h.sourceBranch}...`,increment:w});let R=e.getFetchRemote(h.sourceBranch),S=e.getFetchBranch(h.sourceBranch);await f(`git fetch ${R} ${S}:${S}`,{cwd:t})}catch{}}catch{}let g=60/(i.length||1);for(let w of i){if(d)break;let h=`${r}-to-${w.name}`,R=w.sourceBranch;try{l.report({message:`Processing ${h}...`,increment:g/4}),await f(`git checkout ${h}`,{cwd:t});try{await f(`git pull ${e.originRemote} ${h}`,{cwd:t})}catch{}let S=async ee=>{try{await f(`git merge ${ee}`,{cwd:t})}catch(P){let L=!1;try{let{stdout:W}=await f("git ls-files -u",{cwd:t});W.trim().length>0&&(L=!0)}catch{}let J=((P.stdout||"")+(P.stderr||"")+(P.message||"")).toLowerCase();if(L||J.includes("conflict")||J.includes("conflit")){if(!await ve(t,ee,h,l))throw d=!0,new Error("Deploy aborted by user.")}else throw P}};l.report({message:`Merging ${R} into ${h}...`,increment:g/4});let le=e.getFetchRemote(R),fe=e.getFetchBranch(R),we=e.buildUpstreamPath(R);if(await f(`git fetch ${le} ${fe}`,{cwd:t}),await S(we),l.report({message:`Merging ${a} into ${h}...`,increment:g/4}),await S(a),d)break;l.report({message:`Pushing ${h}...`,increment:g/4}),await f(`git push ${e.originRemote} ${h}`,{cwd:t}),s++}catch(S){S.message.includes("aborted")?F.window.showInformationMessage("Ricwiz: Deploy cancelled."):F.window.showErrorMessage(`Ricwiz: Failed to process branch ${h}. Detail: ${S.message}`);return}}if(!d){l.report({message:"Finishing up...",increment:10});let w=c;try{await f(`git show-ref --verify --quiet refs/heads/${a}`,{cwd:t}),w=a}catch{}try{let h=await D(t);w&&w!==h?(await f(`git checkout ${w}`,{cwd:t}),F.window.showInformationMessage(`Ricwiz: Operation complete. Back on branch ${w}.`)):F.window.showInformationMessage("Ricwiz: Operation complete.")}catch{F.window.showInformationMessage("Ricwiz: Operation complete.")}}})}var F,Ct=k(()=>{"use strict";F=v(require("vscode"));$();We();re()});async function $t(t=!1){let e=b();if(!e)return;let i=await A.initialize(e);if(!i)return;let n=await N(e,{prompt:"Enter the full ticket ID for the Merge Requests (e.g., SCPSCA-1234) or just the number"});if(!n)return;let{ticketId:r}=n,a=Y.workspace.getConfiguration("ricwiz").get("gitlabUrlOverride",""),p="";if(a&&a.trim()!=="")p=a.trim();else{let o="";try{let{stdout:s}=await f("git remote get-url origin",{cwd:e});o=s.trim()}catch{Y.window.showErrorMessage("Ricwiz: Could not get the remote origin URL. Please configure the URL manually in the extension settings.");return}p=o,p.endsWith(".git")&&(p=p.slice(0,-4)),p.startsWith("git@")&&(p=p.replace("git@","").replace(":","/"),p=`https://${p}`)}let m=[],l=i.ticketSourceBranch;try{let{stdout:o}=await f(`git config branch.${r}.ricwiz-source`,{cwd:e});o.trim()&&(l=o.trim())}catch{}if(i.environments.length===0)m.push({source:r,target:l});else for(let o of i.environments)m.push({source:`${r}-to-${o.name}`,target:o.sourceBranch});for(let o of m){let s=`${p}/-/merge_requests/new?merge_request[source_branch]=${o.source}&merge_request[target_branch]=${o.target}`;t?Y.commands.executeCommand("simpleBrowser.show",s):Y.env.openExternal(Y.Uri.parse(s))}Y.window.showInformationMessage(`Ricwiz: Opening ${t?"VS Code browser":"external browser"} for Merge Requests!`)}async function zt(){return $t(!1)}async function Rt(){return $t(!0)}var Y,St=k(()=>{"use strict";Y=v(require("vscode"));$();re()});async function Pt(t=!1){let e=b();if(!e)return;let i=Z.workspace.getConfiguration("ricwiz"),n=i.get("jiraUrl","");if(!n||n.trim()===""){Z.window.showErrorMessage("Ricwiz: Jira URL is not configured. Please set it in the extension settings (e.g., https://jira.company.com/browse/).");return}let{getCurrentBranch:r,resolvePrefix:u,extractTicketSuggestion:a}=($(),ue(Se)),p=await r(e),m=i.get("ticketPrefix","SFPSCA-"),l=u(p,m),s=a(p,l,!0);if(s){let{normalizeTicketId:d}=($(),ue(Se));s=d(s,l)}else{let d=await N(e,{prompt:"Ricwiz could not detect the ticket from the branch name. Enter Jira ticket ID (e.g. SFPSCA-1234)",handleToSuffix:!0});if(!d)return;s=d.ticketId}let c=n.trim();c.endsWith("/")||(c+="/"),c+=s,t?Z.commands.executeCommand("simpleBrowser.show",c):Z.env.openExternal(Z.Uri.parse(c)),Z.window.showInformationMessage(`Ricwiz: Opening Jira Ticket ${s} in ${t?"VS Code":"browser"}!`)}async function Bt(){return Pt(!1)}async function Tt(){return Pt(!0)}var Z,Et=k(()=>{"use strict";Z=v(require("vscode"));$()});async function ei(){let t=Dt.workspace.getConfiguration("ricwiz"),e=t.get("jiraUrl",""),i=t.get("jiraEmail","")?.trim(),n=(await nt())?.trim();if(!e||!n)throw new Error('Jira API Token is not securely configured. Please run the "Ricwiz: Set Secure Jira API Token" command.');let r=e;r.includes("/browse")&&(r=r.split("/browse")[0]),r.endsWith("/")&&(r=r.slice(0,-1));let u=i?`Basic ${Buffer.from(`${i}:${n}`).toString("base64")}`:`Bearer ${n}`;return{baseUrl:r,headerAuth:u}}async function Ce(t,e,i){let{baseUrl:n,headerAuth:r}=await ei(),u=new URL(`${n}${e}`);return new Promise((a,p)=>{let m=Mt.request(u,{method:t,headers:{Authorization:r,Accept:"application/json",...i?{"Content-Type":"application/json"}:{}}},l=>{let o="";l.on("data",s=>o+=s),l.on("end",()=>{if(l.statusCode===401||l.statusCode===403)return p(new Error(`Authentication failed (HTTP ${l.statusCode}). Please check your Jira settings.`));if(l.statusCode&&l.statusCode>=400){let s="";try{let c=JSON.parse(o);c.errorMessages&&c.errorMessages.length>0&&(s=c.errorMessages.join(", "))}catch{}return l.statusCode===404||l.statusCode===410?p(new Error(`Ticket not found or deleted (HTTP ${l.statusCode}). ${s}`)):p(new Error(`Jira API returned HTTP status ${l.statusCode}. ${s}`))}if(!o)return a({});try{let s=JSON.parse(o);a(s)}catch{p(new Error("Failed to parse Jira response."))}})});m.on("error",l=>p(new Error(`Network error: ${l.message}`))),i&&m.write(JSON.stringify(i)),m.end()})}async function $e(t){let e=await Ce("GET",`/rest/api/2/issue/${t}`);return e&&e.fields?{summary:e.fields.summary||"",description:e.fields.description||"No description provided."}:null}async function At(t){let e=await Ce("GET",`/rest/api/2/issue/${t}/transitions`);return e&&e.transitions?e.transitions.map(i=>({id:i.id,name:i.name})):[]}async function It(t,e){await Ce("POST",`/rest/api/2/issue/${t}/transitions`,{transition:{id:e}})}async function Lt(t,e){await Ce("POST",`/rest/api/2/issue/${t}/comment`,{body:e})}async function Ft(t,e){await Ce("PUT",`/rest/api/2/issue/${t}`,{update:{labels:[{add:e}]}})}async function Ot(t){let e=await Ce("POST","/rest/api/3/search/jql",{jql:t,maxResults:50,fields:["summary","status","assignee"]});return e&&e.issues?e.issues.map(i=>({key:i.key,summary:i.fields?.summary||"No Title",status:i.fields?.status?.name||"Unknown",assignee:i.fields?.assignee?.displayName||"Unassigned"})):[]}var Mt,Dt,Be=k(()=>{"use strict";Mt=v(require("https")),Dt=v(require("vscode"));xe()});async function Nt(t){let e=b();if(e)try{if(!await A.initialize(e))return;let r=(await D(e)).split("-to-")[0];if(!r){ie.window.showErrorMessage("Ricwiz: You are not currently on a valid ticket branch.");return}await ie.window.withProgress({location:ie.ProgressLocation.Notification,title:`Fetching details for ${r}...`,cancellable:!1},async u=>{let a=await $e(r);a?(t.setJiraData({ticketId:r,...a}),t.setPage("jira")):ie.window.showErrorMessage("Ricwiz: No data found for this ticket.")})}catch(i){i.message.includes("securely configured")?await ie.window.showErrorMessage(i.message,"Set Token Now")==="Set Token Now"&&ie.commands.executeCommand("ricwiz.setJiraToken"):ie.window.showErrorMessage(`Ricwiz Jira Error: ${i.message}`)}}var ie,Ut=k(()=>{"use strict";ie=v(require("vscode"));$();re();Be()});async function jt(t,e){let n=ae.workspace.getConfiguration("ricwiz").get("jiraDashboards",[]);if(e!==void 0&&(be=e),!n||n.length===0){t.setDashboardData({queries:[],selectedIndex:0,results:[],error:"No queries defined in settings. Go to Settings > Ricwiz > Jira Dashboards."}),t.setPage("dashboard");return}be>=n.length&&(be=0);let r=n[be];t.setDashboardData({queries:n,selectedIndex:be,results:[],error:"\u23F3 Loading tickets..."}),t.setPage("dashboard");try{let u=await Ot(r.jql),a=ae.workspace.workspaceFolders?.[0]?.uri.fsPath,p=[];if(a)try{let l=require("child_process"),s=require("util").promisify(l.exec),{stdout:c}=await s("git branch",{cwd:a});p=c.split(`
`).map(d=>d.replace("*","").trim()).filter(d=>d)}catch{}let m=u.map(l=>{let o=p.find(s=>s.includes(l.key));return{...l,branch:o||null}});t.setDashboardData({queries:n,selectedIndex:be,results:m,error:null}),t.setPage("dashboard")}catch(u){let a=u.message;(a.includes("ENOTFOUND")||a.includes("network"))&&(a="No Internet or Invalid URL"),t.setDashboardData({queries:n,selectedIndex:be,results:[],error:a}),t.setPage("dashboard")}}async function Jt(t,e){await ae.window.withProgress({location:ae.ProgressLocation.Notification,title:`Fetching details for ${e}...`,cancellable:!1},async()=>{try{let i=await $e(e);i?(t.setJiraData({ticketId:e,...i}),t.setPage("jira")):ae.window.showErrorMessage(`Ricwiz: No data found for ticket ${e}.`)}catch(i){ae.window.showErrorMessage(`Ricwiz Jira Error: ${i.message}`)}})}var ae,be,Wt=k(()=>{"use strict";ae=v(require("vscode"));Be();be=0});async function Qe(){let t=b();return!t||!await A.initialize(t,{forcePrompt:!1})?void 0:(await D(t)).split("-to-")[0]}async function qt(){try{let t=await Qe();if(!t){z.window.showErrorMessage("Ricwiz: You are not on a valid ticket branch.");return}let e=await z.window.withProgress({location:z.ProgressLocation.Notification,title:`Fetching available status for ${t}...`,cancellable:!1},()=>At(t));if(!e||e.length===0){z.window.showInformationMessage(`Ricwiz: No transitions available for ${t}.`);return}let i=e.map(r=>({label:r.name,id:r.id})),n=await z.window.showQuickPick(i,{placeHolder:`Select new status for ${t}`,ignoreFocusOut:!0});n&&(await z.window.withProgress({location:z.ProgressLocation.Notification,title:`Updating status to ${n.label}...`,cancellable:!1},()=>It(t,n.id)),z.window.showInformationMessage(`Ricwiz: Status for ${t} updated to ${n.label}.`))}catch(t){t.message.includes("securely configured")?z.window.showErrorMessage(t.message,"Set Token Now").then(e=>{e==="Set Token Now"&&z.commands.executeCommand("ricwiz.setJiraToken")}):z.window.showErrorMessage(`Ricwiz Jira Error: ${t.message}`)}}async function Gt(){try{let t=await Qe();if(!t){z.window.showErrorMessage("Ricwiz: You are not on a valid ticket branch.");return}let e=await z.window.showInputBox({prompt:`Add comment to ${t}`,placeHolder:"Type your comment here...",ignoreFocusOut:!0});e&&(await z.window.withProgress({location:z.ProgressLocation.Notification,title:`Adding comment to ${t}...`,cancellable:!1},()=>Lt(t,e)),z.window.showInformationMessage(`Ricwiz: Comment added to ${t}.`))}catch(t){t.message.includes("securely configured")?z.window.showErrorMessage(t.message,"Set Token Now").then(e=>{e==="Set Token Now"&&z.commands.executeCommand("ricwiz.setJiraToken")}):z.window.showErrorMessage(`Ricwiz Jira Error: ${t.message}`)}}async function Ht(){try{let t=await Qe();if(!t){z.window.showErrorMessage("Ricwiz: You are not on a valid ticket branch.");return}let e=await z.window.showInputBox({prompt:`Add a label to ${t}`,placeHolder:"e.g. Needs-Review, Bug, High-Priority",ignoreFocusOut:!0});e&&e.trim()&&(await z.window.withProgress({location:z.ProgressLocation.Notification,title:`Adding label to ${t}...`,cancellable:!1},()=>Ft(t,e.trim())),z.window.showInformationMessage(`Ricwiz: Label '${e.trim()}' added to ${t}.`))}catch(t){t.message.includes("securely configured")?z.window.showErrorMessage(t.message,"Set Token Now").then(e=>{e==="Set Token Now"&&z.commands.executeCommand("ricwiz.setJiraToken")}):z.window.showErrorMessage(`Ricwiz Jira Error: ${t.message}`)}}async function Vt(){let t=await z.window.showInputBox({prompt:"Enter your Jira API Token (or Personal Access Token). It will be securely stored in your OS keychain.",password:!0,ignoreFocusOut:!0});if(t)try{await st(t.trim()),z.window.showInformationMessage("Ricwiz: Jira API Token securely stored!")}catch(e){z.window.showErrorMessage(`Ricwiz: Failed to store token: ${e.message}`)}}var z,_t=k(()=>{"use strict";z=v(require("vscode"));$();re();Be();xe()});async function Qt(){let t=await j.window.showInputBox({prompt:"Enter your GitLab Personal Access Token",placeHolder:"glpat-xxxxxxxxxxxxxxxxxxxx",ignoreFocusOut:!0,password:!0});if(t&&t.trim()){let e=t.trim();await j.window.withProgress({location:j.ProgressLocation.Notification,title:"Ricwiz: Validating GitLab Token...",cancellable:!1},async()=>{try{let n=j.workspace.getConfiguration("ricwiz").get("gitlabUrlOverride","https://gitlab.com");if(j.workspace.workspaceFolders)try{let{exec:m}=($(),ue(Se)),l=j.workspace.workspaceFolders[0].uri.fsPath,{stdout:o}=await m("git remote get-url origin",{cwd:l}),s=o.trim();s.startsWith("git@")&&(s=`https://${s.replace("git@","").replace(":","/")}`),s.endsWith(".git")&&(s=s.slice(0,-4)),n=s}catch{}let r=new URL(n),u=`${r.protocol}//${r.host}`,a=require("https"),p=await new Promise((m,l)=>{let o=a.request(new URL(`${u}/api/v4/user`),{method:"GET",timeout:5e3,headers:{"PRIVATE-TOKEN":e,Accept:"application/json"}},s=>{if(s.statusCode>=400)return l(new Error(`Status ${s.statusCode}`));let c="";s.on("data",d=>c+=d),s.on("end",()=>m(JSON.parse(c||"{}")))});o.on("error",l),o.on("timeout",()=>{o.destroy(),l(new Error("Timeout"))}),o.end()});await rt(e),j.window.showInformationMessage(`Ricwiz: \u2705 GitLab API Token saved and validated successfully for ${p.username||"user"}!`),j.commands.executeCommand("ricwiz.manualRefresh")}catch(i){j.window.showErrorMessage(`Ricwiz: \u274C Invalid token or cannot reach GitLab (${i.message}). Please check the token and try again.`)}})}}var j,Yt=k(()=>{"use strict";j=v(require("vscode"));xe()});async function Zt(){let t=b();if(!t){ce.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=await A.initialize(t);if(!e)return;let i=await N(t,{prefix:e.ticketPrefix,prompt:"Enter the full ticket ID to sync all branches for (e.g., SCPSCA-1234) or just the number"});if(!i)return;let{ticketId:n,currentBranch:r}=i;await ce.window.withProgress({location:ce.ProgressLocation.Notification,title:`Ricwiz: Syncing all branches for ${n}...`,cancellable:!1},async u=>{try{u.report({message:"Fetching from remote..."});try{await f("git fetch --all",{cwd:t})}catch{}let{stdout:a}=await f(`git branch --list "*${n}*"`,{cwd:t}),p=a.split(`
`).map(o=>o.replace("*","").trim()).filter(o=>o.length>0);if(p.length===0){ce.window.showWarningMessage(`Ricwiz: No local branches found for ${n}.`);return}let m=0,l=0;for(let o of p)if(u.report({message:`Syncing ${o}...`}),o===r)try{await f(`git pull ${e.originRemote} ${o}`,{cwd:t}),m++}catch(s){let c=!1;try{let{stdout:g}=await f("git ls-files -u",{cwd:t});g.trim().length>0&&(c=!0)}catch{}let d=((s.stdout||"")+(s.stderr||"")+(s.message||"")).toLowerCase();(c||d.includes("conflict")||d.includes("conflit"))&&await ve(t,`${e.originRemote}/${o}`,o,u)?m++:l++}else try{await f(`git fetch ${e.originRemote} ${o}:${o}`,{cwd:t}),m++}catch{try{await f(`git checkout ${o}`,{cwd:t});try{await f(`git pull ${e.originRemote} ${o}`,{cwd:t}),m++}catch(c){let d=!1;try{let{stdout:w}=await f("git ls-files -u",{cwd:t});w.trim().length>0&&(d=!0)}catch{}let g=((c.stdout||"")+(c.stderr||"")+(c.message||"")).toLowerCase();(d||g.includes("conflict")||g.includes("conflit"))&&await ve(t,`${e.originRemote}/${o}`,o,u)?m++:l++}await f(`git checkout ${r}`,{cwd:t})}catch{try{await f(`git checkout ${r}`,{cwd:t})}catch{}l++}}l>0?ce.window.showWarningMessage(`Ricwiz: Synced ${m}/${p.length} branches. ${l} branch(es) could not be synced (possible conflicts or diverged history).`):ce.window.showInformationMessage(`Ricwiz: \u{1F504} All ${m} branches for ${n} are up to date!`)}catch(a){ce.window.showErrorMessage(`Ricwiz: Sync failed: ${a.message}`)}})}var ce,Kt=k(()=>{"use strict";ce=v(require("vscode"));$();We();re()});async function Xt(){let t=b();if(!t){de.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}try{await f("git status",{cwd:t})}catch{de.window.showErrorMessage("Ricwiz: The opened folder does not appear to be a valid Git repository.");return}let e=await A.initialize(t);if(!e)return;let i=e.environments,n=await N(t,{prefix:e.ticketPrefix});if(!n)return;let{ticketId:r,currentBranch:u}=n;await de.window.withProgress({location:de.ProgressLocation.Notification,title:"Ricwiz: Updating environment branches from their bases",cancellable:!0},async(a,p)=>{let m=0,l=u,o=!1;p.onCancellationRequested(()=>{o=!0}),a.report({message:"Syncing remote information...",increment:10});try{await f("git fetch --all",{cwd:t})}catch{}let s=80/(i.length||1);for(let c of i){if(o)break;let d=`${r}-to-${c.name}`,g=c.sourceBranch;if(await pe(t,d))try{a.report({message:`Processing ${d}...`,increment:s/2}),await f(`git checkout ${d}`,{cwd:t});try{a.report({message:`Merging ${g} into ${d}...`,increment:s/2});let w=e.getFetchRemote(g),h=e.getFetchBranch(g),R=e.buildUpstreamPath(g);await f(`git fetch ${w} ${h}`,{cwd:t}),await f(`git merge ${R}`,{cwd:t})}catch(w){let h=!1;try{let{stdout:S}=await f("git ls-files -u",{cwd:t});S.trim().length>0&&(h=!0)}catch{}let R=((w.stdout||"")+(w.stderr||"")+(w.message||"")).toLowerCase();if(h||R.includes("conflict")||R.includes("conflit")){let S=e.buildUpstreamPath(g);if(!await ve(t,S,d,a))throw o=!0,new Error("Update aborted by user.")}else throw w}if(o)break;m++}catch(w){w.message.includes("aborted")?de.window.showInformationMessage("Ricwiz: Update cancelled."):de.window.showErrorMessage(`Ricwiz: Failed to update branch ${d}. Detail: ${w.message}`);return}}if(!o){a.report({message:"Finishing up...",increment:10});try{let c=await D(t);l&&l!==c&&await f(`git checkout ${l}`,{cwd:t})}catch{}de.window.showInformationMessage(`Ricwiz: Successfully updated ${m} environment branches from their bases!`)}})}var de,eo=k(()=>{"use strict";de=v(require("vscode"));$();We();re()});async function to(){let t=b();if(!t){I.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=await D(t),i=I.workspace.getConfiguration("ricwiz");await I.window.withProgress({location:I.ProgressLocation.Notification,title:"Ricwiz: Scanning for unused local branches...",cancellable:!1},async()=>{try{await f("git fetch --prune",{cwd:t})}catch{}let n=[];try{let{stdout:s}=await f('git branch --format="%(refname:short)"',{cwd:t});n=s.split(`
...
This cannot be undone!`,{modal:!0},"Yes, delete them")!=="Yes, delete them"){I.window.showInformationMessage("Ricwiz: Deletion cancelled.");return}let o=0;for(let s of m){let c=s.label;if(c===e){let d=i.get("ticketSourceBranch","main");try{await f(`git checkout ${d}`,{cwd:t}),e=d}catch{I.window.showWarningMessage(`Ricwiz: Could not switch away from ${c}. Skipping.`);continue}}try{await f(`git branch -D ${c}`,{cwd:t}),o++}catch{I.window.showWarningMessage(`Ricwiz: Could not delete local branch ${c}.`)}}I.window.showInformationMessage(`Ricwiz: \u{1F5D1}\uFE0F Cleaned up ${o} unused local branch(es).`)})}var I,oo=k(()=>{"use strict";I=v(require("vscode"));$()});async function ze(t){let e=b();e&&await K.window.withProgress({location:K.ProgressLocation.Notification,title:`Ricwiz: Switching to ${t}...`,cancellable:!1},async()=>{try{let i=await D(e),n=!1;try{let{stdout:u}=await f("git status --porcelain",{cwd:e});n=u.trim().length>0}catch{}if(n&&i)try{await f(`git stash push --include-untracked -m "ricwiz-auto:${i}"`,{cwd:e}),K.window.showInformationMessage(`Ricwiz: \u{1F4E6} Stashed changes from ${i}`)}catch{K.window.showWarningMessage("Ricwiz: Could not stash changes. Checkout may fail if there are conflicts.")}let r=t;t.includes("/")&&(r=t.split("/").slice(1).join("/"));try{await f(`git checkout ${r}`,{cwd:e})}catch{let a="";if(t.includes("/"))a=t.split("/")[0];else{let{stdout:p}=await f("git branch -r",{cwd:e}),m=p.split(`
`).map(o=>o.trim()).filter(o=>o),l=[];for(let o of m){let s=o.split(" ")[0];s.endsWith(`/${r}`)&&l.push(s.substring(0,s.lastIndexOf("/")))}if(l.length===0){K.window.showErrorMessage(`Ricwiz: A branch "${r}" n\xE3o existe localmente nem em nenhuma remote!`);return}else if(l.length===1)a=l[0];else{let o=await A.initialize(e);l.includes("origin")?a="origin":o&&l.includes(o.upstreamRemote)?a=o.upstreamRemote:a=l[0]}}try{await f(`git fetch ${a} ${r}`,{cwd:e}),await f(`git checkout -b ${r} --track ${a}/${r}`,{cwd:e})}catch{K.window.showErrorMessage(`Ricwiz: Encontrou na remote ${a} mas falhou a fazer checkout.`);return}}try{let{stdout:u}=await f("git stash list",{cwd:e}),a=u.split(`
`);for(let p=0;p<a.length;p++)if(a[p].includes(`ricwiz-auto:${r}`)){let m=a[p].match(/stash@\{(\d+)\}/);m&&(await f(`git stash pop stash@{${m[1]}}`,{cwd:e}),K.window.showInformationMessage(`Ricwiz: \u{1F4E6} Restored stashed changes on ${r}`));break}}catch{K.window.showWarningMessage(`Ricwiz: Could not restore stashed changes on ${r}. You may need to resolve conflicts manually (check git stash list).`)}}catch{K.window.showErrorMessage(`Ricwiz: Could not checkout branch ${t}.`)}})}var K,qe=k(()=>{"use strict";K=v(require("vscode"));$();re()});async function io(){let t=b();if(t)try{let{stdout:e}=await f("git branch --show-current",{cwd:t}),i=e.trim();i&&(await Te.env.clipboard.writeText(i),Te.window.showInformationMessage(`Ricwiz: \u{1F4CB} Copied "${i}" to clipboard`))}catch{Te.window.showErrorMessage("Ricwiz: Could not get the current branch name.")}}var Te,so=k(()=>{"use strict";Te=v(require("vscode"));$()});async function ro(){let t=b();if(!t){G.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=G.workspace.getConfiguration("ricwiz"),i=e.get("ticketSourceBranch","main"),r=e.get("packageXmlCommand",'sf sgd source delta --to "HEAD" --from "origin/{baseBranch}" --output-dir "."').replace("{baseBranch}",i);await G.window.showWarningMessage("Are you sure you want to generate the package.xml? This may overwrite existing local manifest files.",{modal:!0},"Yes, Generate")==="Yes, Generate"&&await G.window.withProgress({location:G.ProgressLocation.Notification,title:"Ricwiz: Generating package.xml using Salesforce CLI...",cancellable:!1},async()=>{try{await f(r,{cwd:t,maxBuffer:10*1024*1024}),G.window.showInformationMessage("Ricwiz: Successfully generated package.xml!");let a=Ge.join(t,"package","package.xml"),p=Ge.join(t,"package.xml"),m=Ge.join(t,"manifest","package.xml");for(let l of[a,p,m])if(no.existsSync(l)){let o=await G.workspace.openTextDocument(l);await G.window.showTextDocument(o);break}}catch(a){G.window.showErrorMessage(`Ricwiz: Error running sf command - ${a.message}`)}})}var G,Ge,no,ao=k(()=>{"use strict";G=v(require("vscode")),Ge=v(require("path")),no=v(require("fs"));$()});async function co(){let t=b();if(!t){H.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let i=H.workspace.getConfiguration("ricwiz").get("deployCommand","sf project deploy start --manifest package/package.xml --post-destructive-changes destructiveChanges/destructiveChanges.xml -c -l NoTestRun -g");await H.window.showWarningMessage("Are you sure you want to deploy to Salesforce? This action modifies your org and cannot be easily undone.",{modal:!0},"Yes, Deploy")==="Yes, Deploy"&&await H.window.withProgress({location:H.ProgressLocation.Notification,title:"Ricwiz: Deploying package...",cancellable:!1},async()=>{try{let{stdout:r,stderr:u}=await f(i,{cwd:t,maxBuffer:52428800}),a=H.window.createOutputChannel("Ricwiz Deploy");a.appendLine(`Executing: ${i}`),a.appendLine(r),u&&(a.appendLine("--- STDERR ---"),a.appendLine(u)),a.show(),H.window.showInformationMessage("Ricwiz: Successfully ran deploy command!")}catch(r){let u=H.window.createOutputChannel("Ricwiz Deploy");u.appendLine(`Error executing: ${i}`),r.stdout&&u.appendLine(r.stdout),r.stderr&&u.appendLine(r.stderr),u.appendLine(r.message),u.show(),H.window.showErrorMessage("Ricwiz: Error running deploy command. See output channel for details.")}})}var H,lo=k(()=>{"use strict";H=v(require("vscode"));$()});async function mo(){let t=b();if(!t){V.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let i=V.workspace.getConfiguration("ricwiz").get("importDataCommand","sfdx force:data:tree:import --plan data/plan.json");await V.window.showWarningMessage("Are you sure you want to import data into Salesforce? This action modifies records in your org.",{modal:!0},"Yes, Import")==="Yes, Import"&&await V.window.withProgress({location:V.ProgressLocation.Notification,title:"Ricwiz: Importing data...",cancellable:!1},async()=>{try{let{stdout:r,stderr:u}=await f(i,{cwd:t,maxBuffer:52428800}),a=V.window.createOutputChannel("Ricwiz Import Data");a.appendLine(`Executing: ${i}`),a.appendLine(r),u&&(a.appendLine("--- STDERR ---"),a.appendLine(u)),a.show(),V.window.showInformationMessage("Ricwiz: Successfully ran import data command!")}catch(r){let u=V.window.createOutputChannel("Ricwiz Import Data");u.appendLine(`Error executing: ${i}`),r.stdout&&u.appendLine(r.stdout),r.stderr&&u.appendLine(r.stderr),u.appendLine(r.message),u.show(),V.window.showErrorMessage("Ricwiz: Error running import data command. See output channel for details.")}})}var V,uo=k(()=>{"use strict";V=v(require("vscode"));$()});async function po(){let t=b();if(!t){_.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e="";try{e=await D(t)}catch{}let n=_.workspace.getConfiguration("ricwiz").get("ticketSourceBranch","main"),r=await _.window.showInputBox({prompt:`Enter the branch name to list modified files (compared to ${n})`,value:e,placeHolder:"SFPSCA-1234"});r&&await _.window.withProgress({location:_.ProgressLocation.Notification,title:`Ricwiz: Finding files for ${r}...`,cancellable:!1},async()=>{try{let u=r.replace(/-to-[a-zA-Z0-9]+$/i,""),a=[];try{let g="";try{let{stdout:w}=await f(`git merge-base origin/${n} ${r}`,{cwd:t});g=w.trim()}catch{let{stdout:w}=await f(`git merge-base ${n} ${r}`,{cwd:t});g=w.trim()}if(g){let{stdout:w}=await f(`git diff --name-only ${g} ${r}`,{cwd:t,maxBuffer:10485760});a=w.split(`
`).map(h=>h.trim()).filter(h=>h.length>0)}}catch{}let p=[];try{let{stdout:g}=await f(`git --no-pager log --grep="\\b${u}\\b" -i -E --name-only -m --first-parent --format=""`,{cwd:t,maxBuffer:10485760});p=g.split(`
`).map(w=>w.trim()).filter(w=>w.length>0)}catch{}let m=[...a,...p];if(m.length===0){_.window.showInformationMessage(`Ricwiz: No modified files found for ${r}.`);return}let l=Array.from(new Set(m)).sort(),o={};for(let g of l){let w=g.match(/default\/([^/]+)/),h=w&&w[1]?w[1].toUpperCase():"OUTROS";o[h]||(o[h]=[]),o[h].push(g)}let s=`Files modified in branch ${r}:
`,c=Object.keys(o).sort();for(let g of c)s+=`
=== ${g} ===
`,s+=o[g].join(`
`)+`
`;let d=await _.workspace.openTextDocument({content:s,language:"plaintext"});await _.window.showTextDocument(d)}catch(u){_.window.showErrorMessage(`Ricwiz: Error running git log - ${u.message}`)}})}var _,go=k(()=>{"use strict";_=v(require("vscode"));$()});async function fo(){let t=b();if(!t){X.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let i=X.workspace.getConfiguration("ricwiz").get("resetTrackingCommand","sf org disable tracking && sf project reset tracking --no-prompt");await X.window.withProgress({location:X.ProgressLocation.Notification,title:"Ricwiz: Resetting tracking...",cancellable:!1},async()=>{try{let{stdout:n,stderr:r}=await f(i,{cwd:t,maxBuffer:52428800}),u=X.window.createOutputChannel("Ricwiz Reset Tracking");u.appendLine(`Executing: ${i}`),u.appendLine(n),r&&(u.appendLine("--- STDERR ---"),u.appendLine(r)),u.show(),X.window.showInformationMessage("Ricwiz: Successfully reset tracking!")}catch(n){let r=X.window.createOutputChannel("Ricwiz Reset Tracking");r.appendLine(`Error executing: ${i}`),n.stdout&&r.appendLine(n.stdout),n.stderr&&r.appendLine(n.stderr),r.appendLine(n.message),r.show(),X.window.showErrorMessage("Ricwiz: Error resetting tracking. See output channel for details.")}})}var X,wo=k(()=>{"use strict";X=v(require("vscode"));$()});async function ho(){let t=b();if(!t){Q.window.showErrorMessage("Ricwiz: Open a workspace first.");return}let e=["ApexClass","ApexTrigger","CustomObject","CustomField","LightningComponentBundle","AuraDefinitionBundle","Flow","CustomLabel","CustomMetadata","StaticResource","Profile","PermissionSet","PermissionSetGroup","Layout","ValidationRule","RecordType","ListView","Report","EmailTemplate","Other (Type manually)..."],i=await Q.window.showQuickPick(e,{placeHolder:"Select Metadata Type to extract (e.g., ApexClass)",ignoreFocusOut:!0});if(!i||i==="Other (Type manually)..."&&(i=await Q.window.showInputBox({prompt:"Enter Metadata Type (e.g., CustomApplication, Queue)",ignoreFocusOut:!0}),!i))return;let n={ApexClass:"**/*.cls",ApexTrigger:"**/*.trigger",CustomObject:"**/*.{object,object-meta.xml}",CustomField:"**/*.field-meta.xml",LightningComponentBundle:"**/lwc/*/*.js",AuraDefinitionBundle:"**/aura/*/*.cmp",Flow:"**/*.flow-meta.xml",CustomLabel:"**/*.labels-meta.xml",CustomMetadata:"**/*.md-meta.xml",StaticResource:"**/*.resource-meta.xml",Profile:"**/*.profile-meta.xml",PermissionSet:"**/*.permissionset-meta.xml",PermissionSetGroup:"**/*.permissionsetgroup-meta.xml",Layout:"**/*.layout-meta.xml",ValidationRule:"**/*.validationRule-meta.xml",RecordType:"**/*.recordType-meta.xml",ListView:"**/*.listView-meta.xml"},r=[],u=n[i];if(u)try{r=(await Q.workspace.findFiles(u,"**/node_modules/**")).map(m=>{let l=m.fsPath.split(/[\\/]/).pop()||"";if(i==="LightningComponentBundle"||i==="AuraDefinitionBundle"){let o=m.fsPath.split(/[\\/]/);return o[o.length-2]||l.split(".")[0]}return l.split(".")[0]}),r=[...new Set(r)].sort()}catch{}let a=await new Promise(p=>{let m=Q.window.createQuickPick();m.title=`Extract ${i}`,m.placeholder="Type name (e.g. MyComponent) or * for all",m.ignoreFocusOut=!0,m.matchOnDescription=!0;let l=()=>{let o=m.value.trim(),s=[];o?s.push({label:`$(cloud-download) Extract "${o}"`,description:"Extract exact name from Salesforce",alwaysShow:!0}):s.push({label:'$(cloud-download) Extract "*" (All)',description:`Extract all ${i}s`,alwaysShow:!0}),r.forEach(c=>{(!o||c.toLowerCase().includes(o.toLowerCase()))&&s.push({label:c,description:"Local workspace component"})}),m.items=s};m.onDidChangeValue(()=>l()),m.onDidAccept(()=>{let o=m.selectedItems[0];if(o){let s=o.label;s.startsWith('$(cloud-download) Extract "')?s=s.replace('$(cloud-download) Extract "',"").replace('" (All)',"").replace('"',""):s==='$(cloud-download) Extract "*" (All)'&&(s="*"),m.hide(),p(s)}}),m.onDidHide(()=>{m.dispose(),p(void 0)}),l(),m.show()});a&&await Q.window.withProgress({location:Q.ProgressLocation.Notification,title:`Ricwiz: Extracting ${i}:${a} from Salesforce...`,cancellable:!0},async(p,m)=>{try{M.show(!0);let l=`${i}:${a}`,{stdout:o,stderr:s}=await f(`sf project retrieve start -m "${l}"`,{cwd:t});o&&M.appendLine(o),s&&M.appendLine(s),Q.window.showInformationMessage(`Ricwiz: Successfully extracted ${l}.`)}catch(l){M.appendLine(`ERROR: ${l.message}`),l.stdout&&M.appendLine(l.stdout),l.stderr&&M.appendLine(l.stderr),Q.window.showErrorMessage("Ricwiz: Extraction failed. See Output channel for details.")}})}var Q,vo=k(()=>{"use strict";Q=v(require("vscode"));$()});async function yo(){let t=O.window.activeTextEditor;if(!t){O.window.showErrorMessage("Ricwiz: Please open the file you want to deploy in the editor first.");return}let e=t.document.uri.fsPath,i=b();if(!i)return;let n="";if(await O.window.withProgress({location:O.ProgressLocation.Notification,title:"Ricwiz: Fetching available Salesforce orgs...",cancellable:!1},async()=>{try{let{stdout:m}=await f("sf org list --json",{cwd:i});n=m}catch(m){n=m.stdout||""}}),!n){O.window.showErrorMessage("Ricwiz: Failed to fetch orgs. Is Salesforce CLI installed?");return}let r=[];try{let m=JSON.parse(n),l=m.result?.nonScratchOrgs||[],o=m.result?.scratchOrgs||[];r=[...l,...o]}catch{O.window.showErrorMessage("Ricwiz: Failed to parse org list.");return}if(r.length===0){O.window.showInformationMessage("Ricwiz: No authenticated orgs found.");return}let u=r.map(m=>({label:m.alias||m.username,description:m.alias?m.username:"",picked:m.isDefaultUsername})),a=await O.window.showQuickPick(u,{placeHolder:"Select the org(s) to deploy this file to",canPickMany:!0,ignoreFocusOut:!0});if(!a||a.length===0)return;let p=bo.basename(e);await O.window.withProgress({location:O.ProgressLocation.Notification,title:`Ricwiz: Deploying ${p} to ${a.length} org(s)...`,cancellable:!1},async()=>{M.show(!0),M.appendLine(`--- Starting Parallel Deploy of ${p} ---`);let m=a.map(async c=>{let d=c.label;M.appendLine(`[${d}] Deploying...`);try{let{stdout:g,stderr:w}=await f(`sf project deploy start -d "${e}" -o "${d}"`,{cwd:i});return M.appendLine(`[${d}] \u2705 Success`),g&&M.appendLine(g),{org:d,success:!0}}catch(g){return M.appendLine(`[${d}] \u274C Failed`),g.stdout&&M.appendLine(g.stdout),g.stderr&&M.appendLine(g.stderr),{org:d,success:!1}}}),l=await Promise.all(m),o=l.filter(c=>c.success).length,s=l.filter(c=>!c.success).length;s===0?O.window.showInformationMessage(`Ricwiz: Successfully deployed to all ${o} orgs!`):O.window.showErrorMessage(`Ricwiz: Deploy finished with errors (${o} success, ${s} failed). Check Output channel.`)})}var O,bo,xo=k(()=>{"use strict";O=v(require("vscode")),bo=v(require("path"));$()});async function ko(){let t=b();if(!t){T.window.showErrorMessage("Ricwiz: Open a folder or workspace that is a Git repository.");return}let e=T.workspace.getConfiguration("ricwiz"),i=e.get("auditUsername",""),n=e.get("auditHours",8),r=await T.window.showInputBox({prompt:"Enter your Salesforce Username to query in SetupAuditTrail",value:i,placeHolder:"admin@tuaorg.com"});if(!r)return;let u=await T.window.showInputBox({prompt:"How many hours back do you want to search?",value:n.toString(),placeHolder:"8"});if(!u)return;let a=parseFloat(u);if(isNaN(a)||a<=0){T.window.showErrorMessage("Ricwiz: Invalid hours specified.");return}let p=new Date(Date.now()-a*60*60*1e3).toISOString(),l=`sf data query -q "${`SELECT Action, Display, Section FROM SetupAuditTrail WHERE CreatedBy.Username = '${r}' AND CreatedDate >= ${p}`}" --json`;await T.window.withProgress({location:T.ProgressLocation.Notification,title:"Ricwiz: Interrogating Setup Audit Trail...",cancellable:!1},async()=>{try{let{stdout:o}=await f(l,{cwd:t,maxBuffer:52428800}),s=JSON.parse(o);if(!s.result||s.result.records.length===0){T.window.showInformationMessage(`Ricwiz: No changes found for ${r} in the last ${a} hours.`);return}let c=s.result.records,d=[],g=new Set;for(let P of c){let L=ti(P.Action,P.Display,P.Section);if(L){let J=`${L.isDelete?"DEL":"ADD"}-${L.metadataFormat}`;if(!g.has(J)){g.add(J);let W=L.isDelete?"$(trash)":"$(plus)";d.push({label:`${W} ${L.metadataFormat}`,description:`${P.Action} -> ${P.Display}`,metadataFormat:L.metadataFormat,isDelete:L.isDelete})}}}if(d.length===0){T.window.showInformationMessage(`Ricwiz: No extractable metadata changes found for ${r} in the last ${a} hours (ignored passwords/logins).`);return}let w=await T.window.showQuickPick(d,{canPickMany:!0,placeHolder:"Select the changes you want to extract to GitLab",ignoreFocusOut:!0});if(!w||w.length===0){T.window.showInformationMessage("Ricwiz: No changes selected.");return}let h=w.filter(P=>P.isDelete),R=w.filter(P=>!P.isDelete),S=T.window.createOutputChannel("Ricwiz Admin Bridge");if(S.show(),h.length>0){let{stdout:P}=await f("git ls-files",{cwd:t}),L=P.split(`
`).map(W=>W.trim()),J=0;for(let W of h){let Ae=W.metadataFormat.split(":"),Ie=Ae[0],Re=Ae[1],Le=Re;Ie==="CustomField"&&(Le=Re.split(".")[1]);let E=L.filter(q=>{let me=Ve.basename(q);return me.startsWith(Le+".")&&me.includes(Ie==="CustomField"?".field":"")});for(let q of E){let me=Ve.join(t,q);He.existsSync(me)&&(He.unlinkSync(me),S.appendLine(`Deleted local file: ${q}`),J++)}}T.window.showInformationMessage(`Ricwiz: Deleted ${J} local files from Git workspace.`)}if(R.length===0)return;let le=R.map(P=>P.metadataFormat).filter(P=>P!=="").join(", "),fe=await T.window.showInputBox({prompt:"Review and adjust the metadata components to retrieve",value:le,ignoreFocusOut:!0});if(!fe)return;let we=`sf project retrieve start -m "${fe}"`;S.appendLine(`Executing: ${we}`),T.window.showInformationMessage(`Ricwiz: Extracting ${R.length} components...`);let ee=await f(we,{cwd:t});S.appendLine(ee.stdout),ee.stderr&&(S.appendLine("--- STDERR ---"),S.appendLine(ee.stderr)),T.window.showInformationMessage("Ricwiz: Changes extracted successfully! Ready for Git Commit.")}catch(o){T.window.showErrorMessage(`Ricwiz: Error capturing changes - ${o.message}`)}})}function ti(t,e,i){if(!t||!e||!i)return null;let n=t.toLowerCase(),r=i.toLowerCase();if(["security controls","network access","session settings","data export","login history","password policies","identity verification","delegated administration"].includes(r)||n.includes("login")||n.includes("password")||n.includes("oauth")||n.includes("session"))return null;let a=n.includes("delete"),p=null;if(n==="permissionsetgroupcomponentadd"||n==="permissionsetgroupcomponentdelete")return null;let m=(l,o=!1)=>{let s=l.replace(/\(.*\)/g,"").trim();s.includes(":")&&!n.includes("calculation")&&(s=s.split(":")[0]);let c=["disabled","deleted","removed","created","changed","updated","from","to","on","assigned","assign","assignment","permission","set","group","apex","class","trigger","custom","field","object","layout","validation","rule","flow","profile"],d=s.split(/\s+/);if(o){for(;d.length>0&&c.includes(d[d.length-1].toLowerCase());)d.pop();for(;d.length>0&&c.includes(d[0].toLowerCase());)d.shift();return d.join(" ").trim().replace(/[^a-zA-Z0-9_ ]/g,"")}else return d.filter(h=>!c.includes(h.toLowerCase())).join("_").replace(/[^a-zA-Z0-9_]/g,"")||s.replace(/\s+/g,"")};if(n.includes("profile"))p=`Profile:${m(e,!0)}`;else if(n.includes("permissionsetgroupcalculation")){let l=e.split(":");p=`PermissionSetGroup:${l.length>1?l[l.length-1].trim():m(e,!1)}`}else if(n.includes("permission set group")||n.includes("permissionsetgroup"))p=`PermissionSetGroup:${m(e,!1)}`;else if(n.includes("permission set")||n.includes("permissionset"))p=`PermissionSet:${m(e,!1)}`;else if(n.includes("apexclass"))p=`ApexClass:${m(e,!1)}`;else if(n.includes("apextrigger")||n.includes("apex trigger"))p=`ApexTrigger:${m(e,!1)}`;else if(n.includes("customfield")){let l=e.match(/([A-Za-z0-9_]+__c)/),o=e.match(/(?:on|na|for)\s+([A-Za-z0-9_]+)/i);l&&o?p=`CustomField:${o[1]}.${l[1]}`:p=`CustomField:${m(e,!1)}`}else if(n.includes("layout"))p=`Layout:${m(e,!0)}`;else if(n.includes("validation"))p=`ValidationRule:${m(e,!1)}`;else if(n.includes("flow"))p=`Flow:${m(e,!1)}`;else if(n.includes("customobject")){let l=e.match(/([A-Za-z0-9_]+__c)/);p=l?`CustomObject:${l[1]}`:`CustomObject:${m(e,!1)}`}else if(!n.includes("created")&&!n.includes("changed")&&!n.includes("deleted"))return null;return p?{metadataFormat:p,isDelete:a}:null}var T,He,Ve,Co=k(()=>{"use strict";T=v(require("vscode")),He=v(require("fs")),Ve=v(require("path"));$()});async function $o(){let t=b();if(t)try{let{stdout:e}=await f('git for-each-ref --sort=-committerdate --format="%(refname:short)|||%(committerdate:relative)|||%(subject)" refs/heads/',{cwd:t}),i=e.split(`
`).filter(r=>r.trim()).map(r=>{let u=r.split("|||");return{label:`$(git-branch) ${u[0]}`,description:u[1],detail:u[2],branchName:u[0]}}),n=await Ye.window.showQuickPick(i,{placeHolder:"Select a branch from history to checkout",matchOnDescription:!0,matchOnDetail:!0});n&&await ze(n.branchName)}catch{Ye.window.showErrorMessage("Ricwiz: Failed to get branch history")}}var Ye,zo=k(()=>{"use strict";Ye=v(require("vscode"));$();qe()});async function Ro(){let t=b();if(!t)return;let e=await Ee.window.showInputBox({prompt:"Enter ticket number or name (e.g., 48934)",placeHolder:"48934"});if(e)try{let{stdout:i}=await f(`git branch --list "*${e}*"`,{cwd:t}),n=i.split(`
`).map(a=>a.replace("*","").trim()).filter(a=>a);if(n.length===0){Ee.window.showInformationMessage(`Ricwiz: No branches found matching "${e}"`);return}let r=n.map(a=>({label:`$(git-branch) ${a}`,branchName:a})),u=await Ee.window.showQuickPick(r,{placeHolder:`Select a branch for ${e}`});u&&await ze(u.branchName)}catch{Ee.window.showErrorMessage("Ricwiz: Failed to search branches")}}var Ee,So=k(()=>{"use strict";Ee=v(require("vscode"));$();qe()});async function Bo(){let t=ye.window.activeTextEditor;if(!t)return ye.window.showErrorMessage("Ricwiz: Please open a file in the editor to check blame."),null;let e=t.document.fileName,i=Po.basename(e),n=b();if(!n)return ye.window.showErrorMessage("Ricwiz: Workspace is not a git repository."),null;let r=[];try{let{stdout:o}=await f(`git log -5 --pretty=format:"%an|%ar|%s|%h" -- "${e}"`,{cwd:n}),s=o.trim().split(`
`);for(let c of s){let d=c.split("|");d.length>=4&&r.push({author:d[0],time:d[1],message:d.slice(2,-1).join("|"),hash:d[d.length-1]})}}catch(o){console.error("Git blame error:",o)}let u="Unknown",a="Unknown",p="Unknown",m=[],l=oi(e);if(l)try{await ye.window.withProgress({location:ye.ProgressLocation.Notification,title:`Ricwiz: Analyzing ${l.name} in Salesforce...`,cancellable:!1},async()=>{let o="";if(l.type==="CustomField"){let s=l.name.split(".");s.length===2&&(o=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM CustomField WHERE DeveloperName = '${s[1].replace("__c","")}' AND TableEnumOrId = '${s[0]}'`)}else l.type==="LightningComponentBundle"?o=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM LightningComponentBundle WHERE DeveloperName = '${l.name}'`:o=`SELECT LastModifiedBy.Name, LastModifiedDate, CreatedBy.Name FROM ${l.type} WHERE Name = '${l.name}'`;if(o)try{let{stdout:s}=await f(`sf data query -t -q "${o}" --json`,{cwd:n,maxBuffer:52428800}),c=JSON.parse(s);if(c&&c.result&&c.result.records&&c.result.records.length>0){let d=c.result.records[0];u=d.LastModifiedBy?d.LastModifiedBy.Name:"Unknown",p=d.CreatedBy?d.CreatedBy.Name:"Unknown",a=new Date(d.LastModifiedDate).toLocaleString()}else u="Not found in Org",a="N/A",p="N/A"}catch{u="Query Error",a="N/A",p="N/A"}try{let s="SELECT Action, Display, CreatedBy.Name, CreatedDate FROM SetupAuditTrail ORDER BY CreatedDate DESC LIMIT 1500",{stdout:c}=await f(`sf data query -q "${s}" --json`,{cwd:n,maxBuffer:52428800}),d=JSON.parse(c);if(d&&d.result&&d.result.records){let g=l.name.replace("__c","");m=d.result.records.filter(h=>h.Display&&h.Display.includes(g)).map(h=>({action:h.Action,display:h.Display,author:h.CreatedBy?h.CreatedBy.Name:"Unknown",time:new Date(h.CreatedDate).toLocaleString()})).slice(0,10)}}catch(s){console.error("Audit trail query error:",s)}})}catch(o){console.error("Salesforce query error:",o)}else u="Unsupported Metadata Type",a="N/A";return{fileName:i,gitHistory:r,sfAuthor:u,sfTime:a,sfCreatedBy:p,auditHistory:m}}function oi(t){let e=t.replace(/\\/g,"/");if(e.includes("/classes/")){let i=e.match(/\/classes\/([^/.]+)\.cls/);if(i)return{type:"ApexClass",name:i[1]}}if(e.includes("/triggers/")){let i=e.match(/\/triggers\/([^/.]+)\.trigger/);if(i)return{type:"ApexTrigger",name:i[1]}}if(e.includes("/lwc/")){let i=e.match(/\/lwc\/([^/]+)\//);if(i)return{type:"LightningComponentBundle",name:i[1]}}if(e.includes("/aura/")){let i=e.match(/\/aura\/([^/]+)\//);if(i)return{type:"AuraDefinitionBundle",name:i[1]}}if(e.includes("/objects/")&&e.includes("/fields/")){let i=e.match(/\/objects\/([^/]+)\//),n=e.match(/\/fields\/([^/.]+)\.field/);if(i&&n)return{type:"CustomField",name:`${i[1]}.${n[1]}`}}return null}var ye,Po,To=k(()=>{"use strict";ye=v(require("vscode")),Po=v(require("path"));$()});function Eo(t,e,i){t.subscriptions.push(x.commands.registerCommand("ricwiz.generateDestructiveChanges",ut),x.commands.registerCommand("ricwiz.runSmartTests",gt),x.commands.registerCommand("ricwiz.refreshWebview",()=>{e&&x.commands.executeCommand("workbench.action.webview.reloadWebviewAction")}),x.commands.registerCommand("ricwiz.createBranches",vt),x.commands.registerCommand("ricwiz.prepareDeploy",kt),x.commands.registerCommand("ricwiz.createMergeRequests",zt),x.commands.registerCommand("ricwiz.createMergeRequestsVSCode",Rt),x.commands.registerCommand("ricwiz.openJiraTicket",Bt),x.commands.registerCommand("ricwiz.openJiraTicketVSCode",Tt),x.commands.registerCommand("ricwiz.showJiraDetails",()=>{e&&Nt(e)}),x.commands.registerCommand("ricwiz.openJiraDashboard",n=>{e&&jt(e,n)}),x.commands.registerCommand("ricwiz.openJiraDetailsForId",n=>{e&&Jt(e,n)}),x.commands.registerCommand("ricwiz.changeJiraStatus",qt),x.commands.registerCommand("ricwiz.addJiraComment",Gt),x.commands.registerCommand("ricwiz.addJiraLabel",Ht),x.commands.registerCommand("ricwiz.setJiraToken",Vt),x.commands.registerCommand("ricwiz.setGitlabToken",Qt),x.commands.registerCommand("ricwiz.syncAll",Zt),x.commands.registerCommand("ricwiz.updateBases",Xt),x.commands.registerCommand("ricwiz.deleteUnusedBranches",to),x.commands.registerCommand("ricwiz.checkoutBranch",ze),x.commands.registerCommand("ricwiz.copyBranchName",io),x.commands.registerCommand("ricwiz.generatePackageXml",ro),x.commands.registerCommand("ricwiz.deployPackage",co),x.commands.registerCommand("ricwiz.importData",mo),x.commands.registerCommand("ricwiz.listTicketFiles",po),x.commands.registerCommand("ricwiz.resetTracking",fo),x.commands.registerCommand("ricwiz.extractComponent",ho),x.commands.registerCommand("ricwiz.deployMultiOrg",yo),x.commands.registerCommand("ricwiz.captureAdminChanges",ko),x.commands.registerCommand("ricwiz.openHistory",$o),x.commands.registerCommand("ricwiz.searchTicket",Ro),x.commands.registerCommand("ricwiz.whoToBlame",async()=>{let n=await Bo();n&&e&&(e.setBlameData(n),e.setPage("blame"))}),x.commands.registerCommand("ricwiz.manualRefresh",()=>{i&&i()}),x.commands.registerCommand("ricwiz.toggleAutoRefresh",()=>{if(e){let n=!e.isAutoRefreshEnabled();e.setAutoRefresh(n),x.workspace.getConfiguration("ricwiz").update("autoRefresh",n,x.ConfigurationTarget.Global)}}),x.commands.registerCommand("ricwiz.openSettings",()=>{x.commands.executeCommand("workbench.action.openSettings","ricwiz")}))}var x,Mo=k(()=>{"use strict";x=v(require("vscode"));pt();ft();bt();Ct();St();Et();Ut();Wt();_t();Yt();Kt();eo();oo();qe();so();ao();lo();uo();go();wo();vo();xo();Co();zo();So();To()});async function Ze(){let t=await _e();return!!(t&&t.trim())}async function Fo(t){let e=Lo.workspace.getConfiguration("ricwiz"),i=(await _e())?.trim();if(!i)throw new Error("No GitLab token");let n=e.get("gitlabUrlOverride","");if(!n||n.trim()==="")try{let{stdout:p}=await f("git remote get-url origin",{cwd:t}),m=p.trim();m.endsWith(".git")&&(m=m.slice(0,-4)),m.startsWith("git@")&&(m=m.replace("git@","").replace(":","/"),m=`https://${m}`),n=m}catch{throw new Error("Could not get remote origin URL.")}let r=new URL(n),u=`${r.protocol}//${r.host}`,a=Do[t];if(!a){let p=r.pathname;p.startsWith("/")&&(p=p.substring(1)),a=encodeURIComponent(p),Do[t]=a}return{baseUrl:u,token:i,projectPath:a}}async function ii(t,e,i){let{baseUrl:n,token:r}=await Fo(t),u=new URL(`${n}${i}`);return new Promise((a,p)=>{let m=Io.request(u,{method:e,timeout:5e3,headers:{"PRIVATE-TOKEN":r,Accept:"application/json"}},l=>{let o="";l.on("data",s=>o+=s),l.on("end",()=>{if(l.statusCode&&l.statusCode>=400)return p(new Error(`GitLab API error: ${l.statusCode}`));if(!o)return a({});try{let s=JSON.parse(o);a(s)}catch{p(new Error("Failed to parse GitLab response."))}})});m.on("timeout",()=>{m.destroy(),p(new Error("GitLab request timed out"))}),m.on("error",l=>p(new Error(`Network error: ${l.message}`))),m.end()})}async function Ke(t,e,i){let n=`${t}:${e}:${i}`,r=Ao.get(n);if(r&&Date.now()-r.timestamp<si)return r.data;try{let{projectPath:u}=await Fo(t),a=`/api/v4/projects/${u}/merge_requests?source_branch=${encodeURIComponent(e)}&target_branch=${encodeURIComponent(i)}&order_by=updated_at&sort=desc`,p=await ii(t,"GET",a);if(p&&p.length>0){let m=p[0],l="none";if(m.head_pipeline&&m.head_pipeline.status){let s=m.head_pipeline.status;s==="success"||s==="failed"||s==="canceled"||s==="skipped"?l=s:l="running"}let o={isMerged:m.state==="merged",isOpen:m.state==="opened",pipelineStatus:l,webUrl:m.web_url};return Ao.set(n,{data:o,timestamp:Date.now()}),o}return null}catch{return null}}var Io,Lo,Do,Ao,si,Oo=k(()=>{"use strict";Io=v(require("https")),Lo=v(require("vscode"));xe();$();Do={};Ao=new Map,si=30*1e3});function No(t,e){return e.find(i=>t.endsWith(`-to-${i.name}`))}async function Uo(t,e,i,n){let r=await Ze();return await Promise.all(e.map(async a=>{let p=No(a,n);if(!p)return{name:a,isMerged:!1};if(r){let m=await Ke(t,a,p.sourceBranch);if(m)return{name:a,isMerged:m.isMerged,pipelineStatus:m.pipelineStatus,mrUrl:m.webUrl}}return{name:a,isMerged:!1}}))}async function jo(t,e,i){let n=No(e,i);if(!n)return!1;if(await Ze()){let r=await Ke(t,e,n.sourceBranch);if(r)return r.isMerged}return!1}async function Jo(t,e=10){try{let{stdout:i}=await f(`git log --oneline -${e} --format="%h|||%s|||%ar"`,{cwd:t});return i.split(`
...
`).map(r=>r.replace("*","").trim()).filter(r=>r&&r!==i)}var Go=k(()=>{"use strict";$();Oo()});function Ho(t,e,i){let n,r=se.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(r),t.subscriptions.push(se.workspace.onDidChangeConfiguration(a=>{if(a.affectsConfiguration("ricwiz.autoRefresh")){let p=se.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(p)}}));async function u(){let a=se.extensions.getExtension("vscode.git");if(a){let l=function(o){let s="",c;async function d(){let w=se.workspace.workspaceFolders;if(!w)return;let h=w[0].uri.fsPath,R=await D(h);if(R&&R!==s){s=R;let S=se.workspace.getConfiguration("ricwiz"),le=S.get("ticketPrefix","SFPSCA-");if(!R.includes(le)){let E=R.match(/([A-Z]+-)\d+/i);E&&(le=E[1].toUpperCase())}let fe=[],we=[],ee=[],P=[],L=S.get("environments",[{name:"Qual",sourceBranch:"quality"},{name:"Val",sourceBranch:"validation"},{name:"Prod",sourceBranch:"main"}]);try{let E=S.get("workspaceCheckoutButtons",["main","quality","validation"]);ee=Array.from(new Set(E))}catch{}let J="",W=R.match(new RegExp(`(${le}\\d+(?:-\\d+)?)`,"i"));if(W){let E=W[1].toUpperCase();J=E;let q=S.get("commitMessageSuffix","- "),me=/^[A-Z]+-\d+(?:-\d+)?\s*(?:-\s*|:\s*|\s+)?/i;me.test(o.inputBox.value)?o.inputBox.value.toUpperCase().startsWith(E)||(o.inputBox.value=o.inputBox.value.replace(me,`${E}${q}`)):o.inputBox.value=`${E}${q}`+o.inputBox.value,i.text=`$(bookmark) ${E}`,i.tooltip=`Branch: ${R}
Click to open Jira ticket`,i.show();try{let Xe=await qo(h,E,R);fe=await Uo(h,Xe,E,L)}catch{}}else{i.hide();try{P=await Wo(h)}catch{}}let[Ae,Ie,Re]=await Promise.all([Jo(h,10),jo(h,R,L),J?$e(J).catch(E=>{let q=E.message;return(q.includes("ENOTFOUND")||q.includes("network"))&&(q="No Internet or Invalid URL"),{summary:`\u26A0\uFE0F Jira Error: ${q}`,description:""}}):Promise.resolve(null)]);we=Ae;let Le=Re?Re.summary:"";e?.updateBranch(R,Ie,fe,we,ee,P,Le)}}function g(){e?.isAutoRefreshEnabled()&&(c&&clearTimeout(c),c=setTimeout(()=>{s="",d()},300))}n=()=>{s="",d()},d(),o.state.onDidChange(()=>g()),se.window.onDidChangeWindowState(w=>{w.focused&&g()})};var p=l;a.isActive||await a.activate();let m=a.exports.getAPI(1);m.repositories.length>0&&m.repositories.forEach(o=>l(o)),m.onDidOpenRepository(o=>l(o))}}return u(),n}var se,Vo=k(()=>{"use strict";se=v(require("vscode"));$();Go();Be()});var je={};et(je,{activate:()=>ni,deactivate:()=>ri,webviewProvider:()=>Me});module.exports=ue(je);function ni(t){it(t),Me=new Oe(t.extensionUri),t.subscriptions.push(De.window.registerWebviewViewProvider("ricwiz-webview",Me));let e=De.window.createStatusBarItem(De.StatusBarAlignment.Left,100);e.command="ricwiz.openJiraTicket",t.subscriptions.push(e);let i=Ho(t,Me,e);Eo(t,Me,i)}function ri(){}var De,Me,Je=k(()=>{De=v(require("vscode"));ot();xe();Mo();Vo()});Je();0&&(module.exports={activate,deactivate,webviewProvider});
