"use strict";var No=Object.create;var _e=Object.defineProperty;var jo=Object.getOwnPropertyDescriptor;var Jo=Object.getOwnPropertyNames;var Wo=Object.getPrototypeOf,Go=Object.prototype.hasOwnProperty;var ct=(t,e,o)=>()=>{if(o)throw o[0];try{return t&&(e=t(t=0)),e}catch(i){throw o=[i],i}};var Bt=(t,e)=>{for(var o in e)_e(t,o,{get:e[o],enumerable:!0})},St=(t,e,o,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let r of Jo(e))!Go.call(t,r)&&r!==o&&_e(t,r,{get:()=>e[r],enumerable:!(i=jo(e,r))||i.enumerable});return t};var k=(t,e,o)=>(o=t!=null?No(Wo(t)):{},St(e||!t||!t.__esModule?_e(o,"default",{value:t,enumerable:!0}):o,t)),Ho=t=>St(_e({},"__esModule",{value:!0}),t);function j(t){let e=new Date().toISOString();Vo.appendLine(`[${e}] ${t}`),console.log(`[Ricwiz] ${t}`)}var It,Vo,dt=ct(()=>{"use strict";It=k(require("vscode")),Vo=It.window.createOutputChannel("Ricwiz Debug")});async function Ft(t){j("initializeSecrets: No longer using SecretStorage. Tokens are read directly from VS Code configuration.")}async function Ut(t){j("storeJiraToken: Storing token in VS Code global configuration..."),await ve.workspace.getConfiguration("ricwiz").update("jiraApiToken",t,ve.ConfigurationTarget.Global),j("storeJiraToken: Successfully stored")}async function Ke(){j("getJiraToken: Reading token from VS Code configuration...");let e=ve.workspace.getConfiguration("ricwiz").get("jiraApiToken","");if(e)return j("getJiraToken: Successfully read Jira Token from configuration."),e;j("getJiraToken: Token not found in configuration.")}async function Ot(t){j("storeGitlabToken: Storing token in VS Code global configuration..."),await ve.workspace.getConfiguration("ricwiz").update("gitlabApiToken",t,ve.ConfigurationTarget.Global),j("storeGitlabToken: Successfully stored")}async function Ce(){j("getGitlabToken: Reading token from VS Code configuration...");let e=ve.workspace.getConfiguration("ricwiz").get("gitlabApiToken","");if(e)return e}var ve,$e=ct(()=>{"use strict";ve=k(require("vscode"));dt()});var ao={};Bt(ao,{addJiraComment:()=>yt,addJiraLabel:()=>bt,extractTextFromADF:()=>kt,fetchJiraIssue:()=>Ee,fetchJiraIssuesBatch:()=>Zo,fetchJiraTransitions:()=>wt,searchJira:()=>xt,transitionJiraIssue:()=>vt});async function so(){j("getJiraAuthAndBaseUrl: Starting...");let t=no.workspace.getConfiguration("ricwiz"),e=t.get("jiraUrl",""),o=t.get("jiraEmail","")?.trim();j("getJiraAuthAndBaseUrl: Calling getJiraToken()...");let i=(await Ke())?.trim();if(!i&&process.env.RICWIZ_JIRA_TOKEN&&(j("getJiraAuthAndBaseUrl: Token not found in secretStorage, using process.env"),i=process.env.RICWIZ_JIRA_TOKEN.trim()),!e||!i)throw j(`getJiraAuthAndBaseUrl: FAILED. URL: "${e}", hasToken: ${!!i}`),new Error(`[v5.2.0] Jira API Token is not securely configured. URL: "${e}", hasToken: ${!!i}`);let r=e;r.includes("/browse")&&(r=r.split("/browse")[0]),r.endsWith("/")&&(r=r.slice(0,-1));let p=o?`Basic ${Buffer.from(`${o}:${i}`).toString("base64")}`:`Bearer ${i}`;return{baseUrl:r,headerAuth:p}}async function De(t,e,o){let{baseUrl:i,headerAuth:r}=await so(),p=new URL(`${i}${e}`);return new Promise((s,c)=>{let d=ro.request(p,{method:t,headers:{Authorization:r,Accept:"application/json",...o?{"Content-Type":"application/json"}:{}}},a=>{let n="";a.on("data",m=>n+=m),a.on("end",()=>{if(a.statusCode===401||a.statusCode===403)return c(new Error(`Authentication failed (HTTP ${a.statusCode}). Please check your Jira settings.`));if(a.statusCode&&a.statusCode>=400){let m="";try{let l=JSON.parse(n);l.errorMessages&&l.errorMessages.length>0&&(m=l.errorMessages.join(", "))}catch{}return a.statusCode===404||a.statusCode===410?c(new Error(`Ticket not found or deleted (HTTP ${a.statusCode}). ${m}`)):c(new Error(`Jira API returned HTTP status ${a.statusCode}. ${m}`))}if(!n)return s({});try{let m=JSON.parse(n);s(m)}catch{c(new Error("Failed to parse Jira response."))}})});d.on("error",a=>c(new Error(`Network error: ${a.message}`))),o&&d.write(JSON.stringify(o)),d.end()})}async function Ee(t){let{baseUrl:e}=await so(),o=await De("GET",`/rest/api/2/issue/${t}`);return o&&o.fields?{summary:o.fields.summary||"",description:o.fields.description||"No description provided.",status:o.fields.status?.name||"Unknown",url:`${e}/browse/${t}`}:null}async function wt(t){let e=await De("GET",`/rest/api/2/issue/${t}/transitions`);return e&&e.transitions?e.transitions.map(o=>({id:o.id,name:o.name})):[]}async function vt(t,e){await De("POST",`/rest/api/2/issue/${t}/transitions`,{transition:{id:e}})}async function yt(t,e){await De("POST",`/rest/api/2/issue/${t}/comment`,{body:e})}async function bt(t,e){await De("PUT",`/rest/api/2/issue/${t}`,{update:{labels:[{add:e}]}})}async function xt(t){let e=await De("POST","/rest/api/3/search/jql",{jql:t,maxResults:50,fields:["summary","status","assignee"]});return e&&e.issues?e.issues.map(o=>({key:o.key,summary:o.fields?.summary||"No Title",status:o.fields?.status?.name||"Unknown",assignee:o.fields?.assignee?.displayName||"Unassigned"})):[]}function kt(t){if(!t||typeof t!="object")return"";let e=t;if(e.type==="text")return typeof e.text=="string"?e.text:"";let o="";if(Array.isArray(e.content))for(let i of e.content){let r=kt(i);r&&(o+=r+" ")}return o.trim()}async function Zo(t){if(t.length===0)return[];let e=`issueKey IN (${t.join(",")})`,o=await De("POST","/rest/api/3/search/jql",{jql:e,maxResults:15,fields:["summary","description","parent","subtasks","issuelinks","issuetype","status","assignee","priority","labels","fixVersions"]});return!o||!o.issues?[]:o.issues.map(i=>{let r=i.fields?.parent,p=r?{key:r.key,title:r.fields?.summary||""}:void 0,s=(i.fields?.subtasks??[]).map(a=>({key:a.key,title:a.fields?.summary||""})),c=(i.fields?.issuelinks??[]).map(a=>a.outwardIssue?{type:a.type?.outward||"relates to",issue:{key:a.outwardIssue.key,title:a.outwardIssue.fields?.summary||""}}:{type:a.type?.inward||"relates to",issue:{key:a.inwardIssue.key,title:a.inwardIssue.fields?.summary||""}}),d=(i.fields?.fixVersions??[]).map(a=>a.name).filter(Boolean);return{key:i.key,title:i.fields?.summary||"",type:i.fields?.issuetype?.name||"",status:i.fields?.status?.name||"",assignee:i.fields?.assignee?.displayName||"",priority:i.fields?.priority?.name||"",labels:i.fields?.labels||[],fixVersions:d,description:kt(i.fields?.description),parent:p,subtasks:s,issueLinks:c}})}var ro,no,Ue=ct(()=>{"use strict";ro=k(require("https")),no=k(require("vscode"));$e();dt()});var ri={};Bt(ri,{activate:()=>oi,deactivate:()=>ii,webviewProvider:()=>ce});module.exports=Ho(ri);var Le=k(require("vscode"));var P=k(require("vscode"));function C(t){return t?t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function Qe(t){let e=(t||"").toLowerCase().trim();return e==="open"?"#888888":e==="in progress"?"#007acc":e==="waiting for deploy"?"#d7a500":e==="close"||e==="done"||e==="closed"?"#238636":"var(--vscode-badge-background)"}function ke(t){return t?t==="running"?"\u{1F7E1}":t==="success"?"\u{1F7E2}":t==="failed"?"\u{1F534}":t==="canceled"||t==="skipped"?"\u26AA":"":""}function q(){return`
        <script>
    ...
    `}function Tt(t,e){let o=_(),i=(e.files||[]).map(r=>`
    ...
    </html>`}function Dt(t){return`<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    </html>`}function Et(t){let e=_(),o=t?.ticketId||"Jira",i=t?.summary||"No Title",r=t?.description||"No description provided.",p=t?.relatedBranches||[];return`<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    </html>`}function Mt(t){let{data:e,showBranches:o}=t,i=_(),r=e||{queries:[],selectedIndex:0,results:[],error:null},p=r.queries.map((c,d)=>`
    ...
    </html>`}function At(){return`<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    </html>`}function Lt(t){let{logoUri:e,currentBranch:o,currentBranchIsMerged:i,relatedBranches:r,commits:p,baseBranches:s,recentTickets:c,ticketTitle:d,ticketStatus:a,autoRefreshEnabled:n}=t,m=_(),l=p.length>0?`
...
`).map(l=>l.trimEnd()).filter(l=>l.length>2).filter(l=>{let u=l.substring(0,2);return["UU","AA","UD","DU","AU","UA","DD"].includes(u)}).map(l=>{let u=l.substring(0,2);return{file:l.substring(3).trim(),state:n(u)}})}catch{return[]}},a=async()=>{if(p)return;let n=await c(),m=await d();ce&&ce.setConflictState({isConflict:!0,sourceStr:e,targetStr:o,deletionsCount:n.length,files:m})};for(mt(async n=>{if(n==="abortDeploy")s=!0;else if(n==="resolveDeletions"){try{let l=(await c()).map(f=>({label:f})),u=await be.window.showQuickPick(l,{canPickMany:!0,placeHolder:"Select conflicted files to DELETE",title:"Ricwiz: Delete Conflicted Files"});if(u&&u.length>0){for(let f of u)try{await g(`git rm --force "${f.label}"`,{cwd:t})}catch{}be.window.showInformationMessage(`Ricwiz: Deleted ${u.length} conflicted file(s).`)}}catch(m){be.window.showErrorMessage(`Ricwiz: Error. (${m.message})`)}a()}else if(n==="commitAndContinue")try{let l=(await c()).filter(f=>je.existsSync(Je.join(t,f)));if(l.length>0&&await be.window.showWarningMessage(`Wait! There are ${l.length} file(s) with deletion conflicts that are still on your disk.

If you commit now, you will KEEP them in the project.

Are you sure you want to KEEP them?`,{modal:!0},"Yes, KEEP them","No, let me DELETE them")!=="Yes, KEEP them"){a();return}let u=!1;try{let{stdout:f}=await g('git grep -E "^<<<<<<< "',{cwd:t});f.trim().length>0&&(u=!0)}catch{}if(u){be.window.showErrorMessage("Ricwiz: You still have unresolved conflict markers (<<<<<<<) in your files. Please resolve them first!"),a();return}await g("git add .",{cwd:t}),await g("git commit --no-edit",{cwd:t})}catch(m){be.window.showErrorMessage(`Ricwiz: Could not commit automatically. (${m.message})`),a()}}),a();;){if(s){p=!0,mt(void 0),ce?.setConflictState(null);try{await g("git merge --abort",{cwd:t})}catch{}return!1}try{let{stdout:n}=await g("git status --porcelain",{cwd:t}),m=n.split(`
`).some(w=>{let $=w.substring(0,2);return["UU","AA","UD","DU","AU","UA","DD"].includes($)}),l=Je.join(t,".git","MERGE_HEAD"),u=Je.join(t,".git","REBASE_HEAD"),f=Je.join(t,".git","CHERRY_PICK_HEAD");if(!(m||je.existsSync(l)||je.existsSync(u)||je.existsSync(f)))return p=!0,mt(void 0),ce?.setConflictState(null),be.window.showInformationMessage("Ricwiz: Changes committed!"),!0}catch{}await new Promise(n=>setTimeout(n,2e3))}}var ot=k(require("https")),Vt=k(require("vscode"));$e();async function ft(){let t=await Ce();return!!(t&&t.trim())}async function Qo(t,e){let o=Vt.workspace.getConfiguration("ricwiz"),i=(await Ce())?.trim();if(!i)throw new Error("No GitLab token");let r=e?e.getConfig("gitlabUrlOverride",""):o.get("gitlabUrlOverride",""),p=[];if(r&&r.trim()!=="")p.push(r.trim());else try{let{stdout:c}=await g("git remote",{cwd:t}),d=c.split(`
`).map(n=>n.trim()).filter(n=>n),a=[];e&&e.upstreamRemote&&d.includes(e.upstreamRemote)&&a.push(e.upstreamRemote),e&&e.originRemote&&e.originRemote!==e.upstreamRemote&&d.includes(e.originRemote)&&a.push(e.originRemote),d.includes("upstream")&&!a.includes("upstream")&&a.push("upstream"),d.includes("origin")&&!a.includes("origin")&&a.push("origin"),a.length===0&&d.length>0&&a.push(...d);for(let n of a)try{let{stdout:m}=await g(`git remote get-url ${n}`,{cwd:t}),l=m.trim();l.endsWith(".git")&&(l=l.slice(0,-4)),l.startsWith("git@")&&(l=l.replace("git@","").replace(":","/"),l=`https://${l}`),p.push(l)}catch(m){R.appendLine(`[GitLab API] Error getting remote URL for ${n}: ${m.message}`)}}catch(c){R.appendLine(`[GitLab API] Error getting remotes: ${c.message}`)}if(p.length===0)throw R.appendLine("[GitLab API] No candidate URLs found in getGitlabTargets!"),new Error("Could not get any remote origin URL.");return p.map(c=>{let d=new URL(c),a=`${d.protocol}//${d.host}`,n=d.pathname;n.startsWith("/")&&(n=n.substring(1)),n.endsWith("/")&&(n=n.slice(0,-1)),n.endsWith(".git")&&(n=n.slice(0,-4));let m=encodeURIComponent(n);return{baseUrl:a,token:i,projectPath:m}})}var Yo=new ot.Agent({keepAlive:!0,maxSockets:10});async function ut(t,e,o,i){let r=new URL(`${t}${i}`);return R.appendLine(`[GitLab API] ${o} ${r.toString()}`),new Promise((p,s)=>{let c=ot.request(r,{method:o,timeout:5e3,agent:Yo,headers:{"PRIVATE-TOKEN":e,Accept:"application/json"}},d=>{let a="";d.on("data",n=>a+=n),d.on("end",()=>{if(R.appendLine(`[GitLab API] Response Code: ${d.statusCode}`),d.statusCode&&d.statusCode>=400)return R.appendLine(`[GitLab API] Error Data: ${a}`),s(new Error(`GitLab API error: ${d.statusCode}`));if(!a)return p({});try{let n=JSON.parse(a);Array.isArray(n)?R.appendLine(`[GitLab API] Returned array with ${n.length} items`):n&&typeof n=="object"&&R.appendLine(`[GitLab API] Returned object with id ${n.id||n.iid||"unknown"}`),p(n)}catch(n){R.appendLine(`[GitLab API] Parse Error: ${n.message}`),s(n)}})});c.on("timeout",()=>{c.destroy(),s(new Error("GitLab request timed out"))}),c.on("error",d=>{R.appendLine(`[GitLab API] Request Failed: ${d.message}`),s(d)}),c.end()})}var gt=new Map,Ko=30*1e3;async function ht(t,e,o,i){R.appendLine(`[GitLab API] fetchMergeRequestStatus called for source: ${e}, target: ${o||"any"}`);let r=`${t}:${e}:${o||"any"}`,p=gt.get(r);if(p&&Date.now()-p.timestamp<Ko)return p.data;try{let s=await Qo(t,i),c=null,d=-1;for(let a of s)try{let n=`/api/v4/projects/${a.projectPath}/merge_requests?source_branch=${encodeURIComponent(e)}&order_by=updated_at&sort=desc`;o&&(n+=`&target_branch=${encodeURIComponent(o)}`);let m=await ut(a.baseUrl,a.token,"GET",n);if(m&&m.length>0){let l=m[0];try{let w=await ut(a.baseUrl,a.token,"GET",`/api/v4/projects/${a.projectPath}/merge_requests/${l.iid}`);w&&(l=w)}catch{}let u="none";if(l.head_pipeline&&l.head_pipeline.status){let w=l.head_pipeline.status;w==="success"||w==="failed"||w==="canceled"||w==="skipped"?u=w:u="running"}let f={isMerged:l.state==="merged",isOpen:l.state==="opened",pipelineStatus:u,webUrl:l.web_url,projectPath:a.projectPath,pipelineId:l.head_pipeline?l.head_pipeline.id:void 0},h=0;f.isOpen?h=2:f.isMerged&&(h=1),h>d&&(c=f,d=h)}}catch(n){R.appendLine(`[GitLab API] Error inside target loop: ${n.message}`)}if(c)return gt.set(r,{data:c,timestamp:Date.now()}),c;for(let a of s)try{let n=`/api/v4/projects/${a.projectPath}/pipelines?ref=${encodeURIComponent(e)}&order_by=updated_at&sort=desc`,m=await ut(a.baseUrl,a.token,"GET",n);if(m&&m.length>0){let l=m[0],u="none";if(l.status){let h=l.status;h==="success"||h==="failed"||h==="canceled"||h==="skipped"?u=h:u="running"}let f={isMerged:!1,isOpen:!1,pipelineStatus:u,webUrl:l.web_url,projectPath:a.projectPath,pipelineId:l.id};return gt.set(r,{data:f,timestamp:Date.now()}),f}}catch{}return null}catch(s){return R.appendLine(`[GitLab API] Failed to fetch MR status: ${s.message}`),null}}function qt(t,e){return e.find(o=>t.endsWith(`-to-${o.name}`))}async function Se(t,e,o,i,r){let p=await ft(),s=e.map(async c=>{let d=qt(c,i);if(p){let a=d?d.sourceBranch:void 0,n=await ht(t,c,a,r);if(n)return{name:c,isMerged:n.isMerged,pipelineStatus:n.pipelineStatus,mrUrl:n.webUrl,projectPath:n.projectPath,pipelineId:n.pipelineId}}else R.appendLine(`[GitLab API] Skipping MR check for ${c} because hasGitlabToken() is false`);return{name:c,isMerged:!1,pipelineStatus:"none"}});return await Promise.all(s)}async function _t(t,e,o,i){let r=qt(e,o);if(!r)return!1;if(await ft()){let p=await ht(t,e,r.sourceBranch,i);if(p)return p.isMerged}else R.appendLine(`[GitLab API] Skipping MR check for current branch ${e} because hasGitlabToken() is false`);return!1}async function Qt(t,e=10){try{let{stdout:o}=await g(`git log --oneline -${e} --format="%h|||%s|||%ar"`,{cwd:t});return o.split(`
`).filter(i=>i.trim()).map(i=>{let r=i.split("|||");return{hash:r[0]||"",message:r.length>=3?r.slice(1,-1).join("|||"):r[1]||"",timeAgo:r.length>=3?r[r.length-1]:""}})}catch{return[]}}async function Yt(t,e=3){try{let{stdout:o}=await g('git for-each-ref --sort=-committerdate --format="%(refname:short)" refs/heads/',{cwd:t}),i=o.split(`
`).map(p=>p.trim()).filter(p=>p),r=/^[A-Z]+-\d+$/i;return i.filter(p=>r.test(p)).slice(0,e)}catch{return[]}}async function Te(t,e,o){let{stdout:i}=await g(`git branch --all --list "*${e}*"`,{cwd:t}),r=new Set,p=new RegExp(`${e}(?!\\d)`,"i");return i.split(`
...
Diff:
${o.slice(0,1e4)}`,s=O.window.createOutputChannel("Ricwiz AI: Commit Message");s.show(!0),s.appendLine("--- Generating Commit Message ---");let c=L(e),d=await ei(p,e,s,r);s.appendLine(`
--- Finished ---`);let a=ti(d);if(!a){O.window.showWarningMessage("Could not extract a valid commit message from Gemini output.");return}let n=O.extensions.getExtension("vscode.git");if(n&&n.isActive){let m=n.exports.getAPI(1);if(m.repositories.length>0){let l=m.repositories[0],u=l.inputBox.value||"",f=/^[A-Z]+-\d+(?:-\d+)?\s*(?:-\s*|:\s*|\s+)?/i,h=u.match(f);if(h)l.inputBox.value=h[0]+a;else{let w=await c,$=O.workspace.getConfiguration("ricwiz"),v=$.get("ticketPrefix","SFPSCA-"),x=te(w,v),b=w.match(new RegExp(`(${x}\\d+(?:-\\d+)?)`,"i"));if(b){let S=$.get("commitMessageSuffix","- ");l.inputBox.value=`${b[1].toUpperCase()}${S}`+a}else l.inputBox.value=a}O.window.showInformationMessage("Commit message generated and prefilled!")}else O.window.showInformationMessage("Generated: "+a)}else O.window.showInformationMessage("Generated: "+a)})}catch(o){O.window.showErrorMessage("Failed to generate commit message: "+o.message)}}function Uo(t,e,o){t.subscriptions.push(y.commands.registerCommand("ricwiz.conflictAction",Ht),y.commands.registerCommand("ricwiz.generateDestructiveChanges",async()=>{try{await Jt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.runSmartTests",async()=>{try{await Wt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.refreshWebview",()=>{e&&y.commands.executeCommand("workbench.action.webview.reloadWebviewAction")}),y.commands.registerCommand("ricwiz.createBranches",async i=>{try{await Gt(i)}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.prepareDeploy",async()=>{try{await Kt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.createMergeRequests",async()=>{try{await Xt()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.createMergeRequestsVSCode",async()=>{try{await eo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.openJiraTicket",async()=>{try{await oo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.openJiraTicketVSCode",async()=>{try{await io()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.showJiraDetails",()=>{e&&co(e)}),y.commands.registerCommand("ricwiz.openJiraDashboard",i=>{e&&lo(e,i)}),y.commands.registerCommand("ricwiz.openJiraDetailsForId",i=>{e&&mo(e,i)}),y.commands.registerCommand("ricwiz.toggleDashboardBranches",i=>{e&&(e.setDashboardShowBranches(i),y.commands.executeCommand("ricwiz.openJiraDashboard"))}),y.commands.registerCommand("ricwiz.changeJiraStatus",async()=>{try{await po()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.addJiraComment",async()=>{try{await uo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.addJiraLabel",async()=>{try{await go()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.setJiraToken",fo),y.commands.registerCommand("ricwiz.setGitlabToken",wo),y.commands.registerCommand("ricwiz.syncAll",async()=>{try{await vo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.updateBases",async()=>{try{await yo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.deleteUnusedBranches",async()=>{try{await bo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.checkoutBranch",async i=>{try{await Oe(i)}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.copyBranchName",async()=>{try{await xo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.generatePackageXml",async()=>{try{await Co()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.deployPackage",async()=>{try{await $o()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.importData",async()=>{try{await Ro()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.listTicketFiles",async()=>{try{await zo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.resetTracking",async()=>{try{await Po()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.extractComponent",async()=>{try{await Bo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.deployMultiOrg",async()=>{try{await To()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.captureAdminChanges",async()=>{try{await Do()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.openHistory",async()=>{try{await Eo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.searchTicket",async()=>{try{await Mo()}finally{y.commands.executeCommand("ricwiz.manualRefresh")}}),y.commands.registerCommand("ricwiz.whoToBlame",async()=>{let i=await Lo();i&&e&&(e.setBlameData(i),e.setPage("blame"))}),y.commands.registerCommand("ricwiz.showPipelineLogs",(i,r)=>Io(i,r)),y.commands.registerCommand("ricwiz.manualRefresh",()=>{o&&o()}),y.commands.registerCommand("ricwiz.toggleAutoRefresh",()=>{if(e){let i=!e.isAutoRefreshEnabled();e.setAutoRefresh(i),y.workspace.getConfiguration("ricwiz").update("autoRefresh",i,y.ConfigurationTarget.Global)}}),y.commands.registerCommand("ricwiz.openSettings",()=>{y.commands.executeCommand("workbench.action.openSettings","ricwiz")}),y.commands.registerCommand("ricwiz.generateCommitMessage",async()=>{await Fo()}),y.commands.registerCommand("ricwiz.getTicketsBatch",async i=>{let{fetchJiraIssuesBatch:r}=await Promise.resolve().then(()=>(Ue(),ao)),p=await r(i);return JSON.stringify(p)}))}var le=k(require("vscode"));Ue();function Oo(t,e,o){let i,r=le.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(r),t.subscriptions.push(le.workspace.onDidChangeConfiguration(s=>{if(s.affectsConfiguration("ricwiz.autoRefresh")){let c=le.workspace.getConfiguration("ricwiz").get("autoRefresh",!0);e?.setAutoRefresh(c)}}));async function p(){let s=le.extensions.getExtension("vscode.git");if(s){let a=function(n){let m="",l;async function u(){let h=le.workspace.workspaceFolders;if(!h)return;let w=h[0].uri.fsPath,$=await L(w);if($&&$!==m){m=$;let v=le.workspace.getConfiguration("ricwiz"),x=v.get("ticketPrefix","SFPSCA-");if(!$.includes(x)){let A=$.match(/([A-Z]+-)\d+/i);A&&(x=A[1].toUpperCase())}let b=[],S=[],E=[],B=[],U=await T.initialize(w,{skipPrompt:!0}),W=U?.environments||v.get("environments",[{name:"Qual",sourceBranch:"quality"},{name:"Val",sourceBranch:"validation"},{name:"Prod",sourceBranch:"main"}]);try{let A=v.get("workspaceCheckoutButtons",["main","quality","validation"]);E=Array.from(new Set(A))}catch{}let me="",Ne=$.match(new RegExp(`(${x}\\d+(?:-\\d+)?)`,"i"));if(Ne){let A=Ne[1].toUpperCase();me=A;let we=v.get("commitMessageSuffix","- "),zt=/^[A-Z]+-\d+(?:-\d+)?\s*(?:-\s*|:\s*|\s+)?/i;zt.test(n.inputBox.value)?n.inputBox.value.toUpperCase().startsWith(A)||(n.inputBox.value=n.inputBox.value.replace(zt,`${A}${we}`)):n.inputBox.value=`${A}${we}`+n.inputBox.value,o.text=`$(bookmark) ${A}`,o.tooltip=`Branch: ${$}
Click to open Jira ticket`,o.show();try{let Pt=await Te(w,A,"");b=await Se(w,Pt,A,W,U)}catch{}}else{o.hide();try{B=await Yt(w)}catch{}}let[Ve,qe,xe]=await Promise.all([Qt(w,10),_t(w,$,W,U),me?Ee(me).catch(A=>{let we=A.message;return we&&(we.includes("ENOTFOUND")||we.includes("network"))&&(we="No Internet or Invalid URL"),{summary:`\u26A0\uFE0F Jira Error: ${we}`,description:"",status:""}}):Promise.resolve(null)]);S=Ve;let at=xe?xe.summary:"",Ie=xe&&xe.status||"";e?.updateBranch($,qe,b,S,E,B,at,Ie)}}function f(){e?.isAutoRefreshEnabled()&&(l&&clearTimeout(l),l=setTimeout(()=>{m="",u()},300))}i=()=>{m="",u()},u(),t.subscriptions.push(n.state.onDidChange(()=>f())),t.subscriptions.push(le.window.onDidChangeWindowState(h=>{h.focused&&f()}))};var c=a;s.isActive||await s.activate();let d=s.exports.getAPI(1);d.repositories.length>0&&d.repositories.forEach(n=>a(n)),d.onDidOpenRepository(n=>a(n))}}return p(),()=>{i&&i()}}var ce;async function oi(t){await Ft(t),ce=new Ye(t.extensionUri),t.subscriptions.push(Le.window.registerWebviewViewProvider("ricwiz-webview",ce));let e=Le.window.createStatusBarItem(Le.StatusBarAlignment.Left,100);e.command="ricwiz.openJiraTicket",t.subscriptions.push(e);let o=Oo(t,ce,e);return Uo(t,ce,o),{getJiraCredentials:async()=>({email:Le.workspace.getConfiguration("ricwiz").get("jiraEmail",""),token:await Ke()}),getGitLabToken:async()=>Ce()}}function ii(){}0&&(module.exports={activate,deactivate,webviewProvider});
